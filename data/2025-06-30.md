<div id=toc></div>

# Table of Contents

- [cs.AI](#cs.AI) [Total: 14]
- [cs.AR](#cs.AR) [Total: 3]
- [cs.CG](#cs.CG) [Total: 5]
- [cs.CL](#cs.CL) [Total: 15]
- [cs.CV](#cs.CV) [Total: 18]
- [cs.ET](#cs.ET) [Total: 6]
- [cs.GR](#cs.GR) [Total: 8]
- [cs.LG](#cs.LG) [Total: 16]
- [cs.LO](#cs.LO) [Total: 8]


<div id='cs.AI'></div>

# cs.AI [[Back]](#toc)

### [1] [SEEA-R1: Tree-Structured Reinforcement Fine-Tuning for Self-Evolving Embodied Agents](https://arxiv.org/abs/2506.21669)
*Wanxin Tian,Shijie Zhang,Kevin Zhang,Xiaowei Chi,Yulin Luo,Junyu Lu,Chunkai Fan,Qiang Zhou,Yiming Zhao,Ning Liu Siyu Lin,Zhiyuan Qin,Xiaozhu Ju,Shanghang Zhang,Jian Tang*

Main category: cs.AI

TL;DR: SEEA-R1是首个针对具身智能体自进化能力的强化微调框架，通过Tree-GRPO解决稀疏奖励问题，并引入MGRM实现奖励泛化，在ALFWorld基准测试中表现优于现有SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 尽管强化微调（RFT）在增强LLM推理方面表现出色，但在具身智能领域，它面临两大挑战：多步推理任务中缺乏可访问的中间奖励信号，以及依赖手工设计奖励函数限制了其对新任务的泛化能力。

Method: 本文提出了SEEA-R1，一个为具身智能体自进化能力设计的RFT框架。为解决奖励稀疏问题，引入了Tree-GRPO（将蒙特卡洛树搜索集成到GRPO中）。为实现奖励估计的泛化，引入了多模态生成奖励模型（MGRM）。

Result: 在ALFWorld基准测试中，SEEA-R1超越了包括GPT-4o在内的现有最先进方法，文本和多模态得分分别为85.07%和36.19%。在没有环境奖励的情况下，SEEA-R1也能达到80.3%的得分，超越所有开源基线。

Conclusion: SEEA-R1的额外实验和定性分析进一步支持了其在可扩展具身智能未来研究中的潜力。

Abstract: Self-evolution, the ability of agents to autonomously improve their reasoning
and behavior, is essential for the embodied domain with long-horizon,
real-world tasks. Despite current advancements in reinforcement fine-tuning
(RFT) showing strong performance in enhancing reasoning in LLMs, its potential
to enable self-evolving embodied intelligence with multi-modal interactions
remains largely unexplored. Specifically, reinforcement fine-tuning faces two
fundamental obstacles in embodied settings: (i) the lack of accessible
intermediate rewards in multi-step reasoning tasks limits effective learning
signals, and (ii) reliance on hand-crafted reward functions restricts
generalization to novel tasks and environments. To address these challenges, we
present Self-Evolving Embodied Agents-R1, SEEA-R1, the first RFT framework
designed for enabling the self-evolving capabilities of embodied agents.
Specifically, to convert sparse delayed rewards into denser intermediate
signals that improve multi-step reasoning, we propose Tree-based group relative
policy optimization (Tree-GRPO), which integrates Monte Carlo Tree Search into
GRPO. To generalize reward estimation across tasks and scenes, supporting
autonomous adaptation and reward-driven self-evolution, we further introduce
Multi-modal Generative Reward Model (MGRM). To holistically evaluate the
effectiveness of SEEA-R1, we evaluate on the ALFWorld benchmark, surpassing
state-of-the-art methods with scores of 85.07% (textual) and 36.19%
(multi-modal), outperforming prior models including GPT-4o. SEEA-R1 also
achieves scores of 80.3% without environmental reward, surpassing all
open-source baselines and highlighting its scalability as a self-evolving
embodied agent. Additional experiments and qualitative analysis further support
the potential of SEEA-R1 for future research in scalable embodied intelligence.

</details>


### [2] [Hierarchical Reasoning Model](https://arxiv.org/abs/2506.21734)
*Guan Wang,Jin Li,Yuhao Sun,Xing Chen,Changling Liu,Yue Wu,Meng Lu,Sen Song,Yasin Abbasi Yadkori*

Main category: cs.AI

TL;DR: 受人脑启发，提出了一种新的分层推理模型（HRM），解决了LLM推理的挑战。它是一个小型高效的循环网络，在数独和迷宫等复杂推理任务上表现出色，超越了大型模型，且无需CoT或预训练。


<details>
  <summary>Details</summary>
Motivation: 推理是人工智能面临的关键挑战，当前大型语言模型（LLMs）主要采用的思维链（CoT）技术存在任务分解脆弱、数据需求量大和延迟高等问题。

Method: 提出了一种名为分层推理模型（HRM）的新型循环架构，灵感来源于人脑的层次化和多时间尺度处理。它通过两个相互依赖的循环模块（一个负责慢速、抽象规划的高级模块和一个处理快速、详细计算的低级模块）在一次前向传播中执行任务，无需中间过程的显式监督，且无需预训练或CoT数据。

Result: HRM 在仅有2700万参数和1000个训练样本的情况下，在复杂推理任务（如复杂数独和大型迷宫中的最优路径寻找）上取得了卓越性能。它在抽象推理语料库（ARC）上超越了拥有更长上下文窗口的更大模型。

Conclusion: HRM 有潜力成为通用计算和通用推理系统方面的一个变革性进展。

Abstract: Reasoning, the process of devising and executing complex goal-oriented action
sequences, remains a critical challenge in AI. Current large language models
(LLMs) primarily employ Chain-of-Thought (CoT) techniques, which suffer from
brittle task decomposition, extensive data requirements, and high latency.
Inspired by the hierarchical and multi-timescale processing in the human brain,
we propose the Hierarchical Reasoning Model (HRM), a novel recurrent
architecture that attains significant computational depth while maintaining
both training stability and efficiency. HRM executes sequential reasoning tasks
in a single forward pass without explicit supervision of the intermediate
process, through two interdependent recurrent modules: a high-level module
responsible for slow, abstract planning, and a low-level module handling rapid,
detailed computations. With only 27 million parameters, HRM achieves
exceptional performance on complex reasoning tasks using only 1000 training
samples. The model operates without pre-training or CoT data, yet achieves
nearly perfect performance on challenging tasks including complex Sudoku
puzzles and optimal path finding in large mazes. Furthermore, HRM outperforms
much larger models with significantly longer context windows on the Abstraction
and Reasoning Corpus (ARC), a key benchmark for measuring artificial general
intelligence capabilities. These results underscore HRM's potential as a
transformative advancement toward universal computation and general-purpose
reasoning systems.

</details>


### [3] [THE-Tree: Can Tracing Historical Evolution Enhance Scientific Verification and Reasoning?](https://arxiv.org/abs/2506.21763)
*Xin Wang,Jiyao Liu,Yulong Xiao,Junzhi Ning,Lihao Liu,Junjun He,Botian Shi,Kaicheng Yu*

Main category: cs.AI

TL;DR: 该论文提出THE-Tree框架，利用LLMs和验证机制从科学文献中构建结构化的科学演化树，显著改善了科学思想评估、未来预测和图谱补全。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）加速了科学思想的生成，但严格评估这些AI生成的提案的新颖性和事实准确性是一个关键瓶颈，因为手动验证速度太慢。现有验证方法不足：LLMs作为独立验证器可能产生幻觉并缺乏领域知识，而传统引用网络缺乏明确的因果关系，叙述性调查则缺乏结构。核心挑战是缺乏结构化、可验证和因果关联的科学演化历史数据。

Method: 该论文引入了THE-Tree（技术历史演化树），一个计算框架，用于从科学文献中构建特定领域的演化树。它采用搜索算法探索演化路径，并使用新颖的“思考-言语化-引用-验证”过程：LLM提出潜在进展并引用支持文献，然后通过自然语言推理机制验证每个提议的演化链接的逻辑连贯性和证据支持。

Result: 构建并验证了88个跨不同领域的THE-Tree，并发布了一个包含71k事实验证和27k论文的基准数据集。实验表明：i) 在图谱补全方面，THE-Tree与传统引用网络相比，将hit@1提高了8%至14%；ii) 在预测未来科学发展方面，将hit@1指标提高了近10%；iii) 与其他方法结合时，评估重要科学论文的性能提升了近100%。

Conclusion: THE-Tree通过构建结构化、可验证和因果关联的科学演化历史数据，有效解决了评估AI生成提案的瓶颈问题，显著提升了图谱补全、未来发展预测和论文评估的性能。

Abstract: Large Language Models (LLMs) are accelerating scientific idea generation, but
rigorously evaluating these numerous, often superficial, AI-generated
propositions for novelty and factual accuracy is a critical bottleneck; manual
verification is too slow.Existing validation methods are inadequate: LLMs as
standalone verifiers may hallucinate and lack domain knowledge (our findings
show ~60\% unawareness of relevant papers in specific domains), while
traditional citation networks lack explicit causality and narrative surveys are
unstructured.This underscores a core challenge: the absence of structured,
verifiable, and causally-linked historical data of scientific evolution.To
address this,we introduce \textbf{THE-Tree} (\textbf{T}echnology
\textbf{H}istory \textbf{E}volution Tree), a computational framework that
constructs such domain-specific evolution trees from scientific
literature.THE-Tree employs a search algorithm to explore evolutionary paths.
During its node expansion, it utilizes a novel "Think-Verbalize-Cite-Verify"
process: an LLM proposes potential advancements and cites supporting
literature. Critically, each proposed evolutionary link is then validated for
logical coherence and evidential support by a recovered natural language
inference mechanism that interrogates the cited literature, ensuring that each
step is grounded.We construct and validate 88 THE-Trees across diverse domains
and release a benchmark dataset including up to 71k fact verifications covering
27k papers to foster further research.Experiments demonstrate that i) in graph
completion, our THE-Tree improves hit@1 by 8\% to 14\% across multiple models
compared to traditional citation networks; ii) for predicting future scientific
developments, it improves hit@1 metric by nearly 10\%; and iii) when combined
with other methods, it boosts the performance of evaluating important
scientific papers by almost 100\%.

</details>


### [4] [MobiVerse: Scaling Urban Mobility Simulation with Hybrid Lightweight Domain-Specific Generator and Large Language Models](https://arxiv.org/abs/2506.21784)
*Yifan Liu,Xishun Liao,Haoxuan Ma,Jonathan Liu,Rohan Jadhav,Jiaqi Ma*

Main category: cs.AI

TL;DR: MobiVerse 是一个混合模拟平台，结合高效的生成器和适应性强的 LLM，用于大规模、真实的人类移动性模拟。它在洛杉矶为 53,000 名代理进行了动态调整，展示了其在城市规划中的计算效率和行为真实性。


<details>
  <summary>Details</summary>
Motivation: 尽管移动性研究取得了显著进展，但在允许大规模算法开发、政策实施和综合评估的模拟平台方面仍存在关键差距。传统模型、机器学习方法和现有基于 LLM 的实现都面临挑战。

Method: 该论文提出了 MobiVerse，一个混合框架，它利用轻量级领域特定生成器的高效率来生成基本活动链，并结合 LLM 的适应性进行上下文感知修改。

Result: 在洛杉矶韦斯特伍德进行的案例研究中，MobiVerse 在一台标准 PC 上高效地为大约 53,000 名代理生成并动态调整了计划。实验表明，MobiVerse 成功使代理能够响应环境反馈，包括道路封闭、大型集会活动（如足球比赛）和拥堵。其模块化设计有助于在交通系统和代理级别测试各种移动性算法。结果显示，该方法在提高行为真实性的同时保持了计算效率。

Conclusion: MobiVerse 通过提供一个可定制的平台和基准算法，弥补了移动性模拟领域的空白，可用于移动系统规划和操作。

Abstract: Understanding and modeling human mobility patterns is crucial for effective
transportation planning and urban development. Despite significant advances in
mobility research, there remains a critical gap in simulation platforms that
allow for algorithm development, policy implementation, and comprehensive
evaluation at scale. Traditional activity-based models require extensive data
collection and manual calibration, machine learning approaches struggle with
adaptation to dynamic conditions, and treding agent-based Large Language Models
(LLMs) implementations face computational constraints with large-scale
simulations. To address these challenges, we propose MobiVerse, a hybrid
framework leverages the efficiency of lightweight domain-specific generator for
generating base activity chains with the adaptability of LLMs for context-aware
modifications. A case study was conducted in Westwood, Los Angeles, where we
efficiently generated and dynamically adjusted schedules for the whole
population of approximately 53,000 agents on a standard PC. Our experiments
demonstrate that MobiVerse successfully enables agents to respond to
environmental feedback, including road closures, large gathering events like
football games, and congestion, through our hybrid framework. Its modular
design facilitates testing various mobility algorithms at both transportation
system and agent levels. Results show our approach maintains computational
efficiency while enhancing behavioral realism. MobiVerse bridges the gap in
mobility simulation by providing a customizable platform for mobility systems
planning and operations with benchmark algorithms. Code and videos are
available at https://github.com/ucla-mobility/MobiVerse.

</details>


### [5] [CitySim: Modeling Urban Behaviors and City Dynamics with Large-Scale LLM-Driven Agent Simulation](https://arxiv.org/abs/2506.21805)
*Nicolas Bougie,Narimasa Watanabe*

Main category: cs.AI

TL;DR: CitySim 利用大型语言模型模拟城市中的人类行为，通过价值驱动方法和长期记忆，实现了更逼真的行为模拟，并可用于预测城市现象。


<details>
  <summary>Details</summary>
Motivation: 现有的人类行为建模方法常依赖僵化、手工设定的规则，限制了其模拟细微意图、计划和适应性行为的能力。本研究旨在解决这一挑战，以更好地模拟城市环境中人类行为。

Method: 该研究提出了一个名为 CitySim 的城市模拟器，它利用大型语言模型来生成逼真的日常时间表。CitySim 中的智能体采用递归的价值驱动方法来平衡强制性活动、个人习惯和情境因素。此外，智能体还被赋予信念、长期目标和空间记忆，以实现长期、逼真的模拟。

Result: 与现有方法相比，CitySim 在微观和宏观层面都与真实人类行为更接近。通过模拟数万个智能体，CitySim 能够评估各种现实世界场景下的集体行为，包括估计人群密度、预测地点受欢迎程度和评估福祉。

Conclusion: CitySim 是一个可扩展、灵活的测试平台，用于理解和预测城市现象。

Abstract: Modeling human behavior in urban environments is fundamental for social
science, behavioral studies, and urban planning. Prior work often rely on
rigid, hand-crafted rules, limiting their ability to simulate nuanced
intentions, plans, and adaptive behaviors. Addressing these challenges, we
envision an urban simulator (CitySim), capitalizing on breakthroughs in
human-level intelligence exhibited by large language models. In CitySim, agents
generate realistic daily schedules using a recursive value-driven approach that
balances mandatory activities, personal habits, and situational factors. To
enable long-term, lifelike simulations, we endow agents with beliefs, long-term
goals, and spatial memory for navigation. CitySim exhibits closer alignment
with real humans than prior work, both at micro and macro levels. Additionally,
we conduct insightful experiments by modeling tens of thousands of agents and
evaluating their collective behaviors under various real-world scenarios,
including estimating crowd density, predicting place popularity, and assessing
well-being. Our results highlight CitySim as a scalable, flexible testbed for
understanding and forecasting urban phenomena.

</details>


### [6] [Interactive Multi-Objective Probabilistic Preference Learning with Soft and Hard Bounds](https://arxiv.org/abs/2506.21887)
*Edward Chen,Sang T. Truong,Natalie Dullerud,Sanmi Koyejo,Carlos Guestrin*

Main category: cs.AI

TL;DR: Active-MoSH是一个交互式框架，通过结合局部偏好学习和全局敏感性分析，帮助决策者在高风险多目标决策中有效权衡，提高决策信任度和效率。


<details>
  <summary>Details</summary>
Motivation: 在高风险决策中，决策者需要在多个相互冲突的目标之间进行权衡，并且每次评估成本高昂。现有方法难以系统地迭代优化多方面的偏好结构，且决策者需要信任他们的最终决策。

Method: 提出了Active-MoSH，一个交互式的局部-全局框架。其局部组件结合了软硬边界和概率偏好学习，通过主动采样策略自适应地细化帕累托子集；其全局组件T-MoSH利用多目标敏感性分析来识别潜在被忽视的高价值点，以建立决策者信任。

Result: Active-MoSH在合成和真实世界应用中展示了性能优势。用户研究验证了其能改善收敛性、增强决策者信任并提供表达性偏好表达。

Conclusion: Active-MoSH框架能够有效帮助决策者在高风险场景下，在复杂多目标权衡中做出更有效、更值得信赖的决策，并通过交互式方法改善收敛性和偏好表达。

Abstract: High-stakes decision-making involves navigating multiple competing objectives
with expensive evaluations. For instance, in brachytherapy, clinicians must
balance maximizing tumor coverage (e.g., an aspirational target or soft bound
of >95% coverage) against strict organ dose limits (e.g., a non-negotiable hard
bound of <601 cGy to the bladder), with each plan evaluation being
resource-intensive. Selecting Pareto-optimal solutions that match implicit
preferences is challenging, as exhaustive Pareto frontier exploration is
computationally and cognitively prohibitive, necessitating interactive
frameworks to guide users. While decision-makers (DMs) often possess domain
knowledge to narrow the search via such soft-hard bounds, current methods often
lack systematic approaches to iteratively refine these multi-faceted preference
structures. Critically, DMs must trust their final decision, confident they
haven't missed superior alternatives; this trust is paramount in
high-consequence scenarios. We present Active-MoSH, an interactive local-global
framework designed for this process. Its local component integrates soft-hard
bounds with probabilistic preference learning, maintaining distributions over
DM preferences and bounds for adaptive Pareto subset refinement. This is guided
by an active sampling strategy optimizing exploration-exploitation while
minimizing cognitive burden. To build DM trust, Active-MoSH's global component,
T-MoSH, leverages multi-objective sensitivity analysis to identify potentially
overlooked, high-value points beyond immediate feedback. We demonstrate
Active-MoSH's performance benefits through diverse synthetic and real-world
applications. A user study on AI-generated image selection further validates
our hypotheses regarding the framework's ability to improve convergence,
enhance DM trust, and provide expressive preference articulation, enabling more
effective DMs.

</details>


### [7] [AlphaBeta is not as good as you think: a new probabilistic model to better analyze deterministic game-solving algorithms](https://arxiv.org/abs/2506.21996)
*Raphaël Boige,Amine Boumaza,Bruno Scherrer*

Main category: cs.AI

TL;DR: 传统的博弈树分析模型过于简单。本文提出了一个新的模型，该模型引入了结构复杂性（祖先依赖性），并推导了AlphaBeta和Scout等算法的平均复杂度。研究表明，尽管算法渐近收敛，但在深度有限树上，AlphaBeta比Scout慢得多。


<details>
  <summary>Details</summary>
Motivation: 传统的确定性博弈求解算法分析模型过于简化，其独立性假设剥夺了博弈的结构复杂性，导致算法面临的挑战微不足道。本研究旨在解决这一局限性。

Method: 引入了一个新的概率模型，该模型使用固定的逐层条件分布逐步构建博弈树，从而强制执行祖先依赖性。在此模型下，为包括AlphaBeta和Scout在内的几种算法推导了表征其平均复杂度。通过这些公式，可以在深度博弈树上严格比较算法。

Result: 渐近地看，所有算法似乎都收敛到相同的分支因子。然而，对于深度有限树，AlphaBeta比Scout等算法的常数乘法因子显著更大，导致实际运行速度大幅下降。

Conclusion: 本框架为理解经典博弈求解算法提供了一个更现实、更具挑战性且易于处理的模型，提供了严谨的证据和分析工具。

Abstract: Deterministic game-solving algorithms are conventionally analyzed in the
light of their average-case complexity against a distribution of random
game-trees, where leaf values are independently sampled from a fixed
distribution. This simplified model enables uncluttered mathematical analysis,
revealing two key properties: root value distributions asymptotically collapse
to a single fixed value for finite-valued trees, and all reasonable algorithms
achieve global optimality. However, these findings are artifacts of the model's
design-its long criticized independence assumption strips games of structural
complexity, producing trivial instances where no algorithm faces meaningful
challenges. To address this limitation, we introduce a new probabilistic model
that incrementally constructs game-trees using a fixed level-wise conditional
distribution. By enforcing ancestor dependency, a critical structural feature
of real-world games, our framework generates problems with adjustable
difficulty while retaining some form of analytical tractability. For several
algorithms, including AlphaBeta and Scout, we derive recursive formulas
characterizing their average-case complexities under this model. These allow us
to rigorously compare algorithms on deep game-trees, where Monte-Carlo
simulations are no longer feasible. While asymptotically, all algorithms seem
to converge to identical branching factor (a result analogous to those of
independence-based models), deep finite trees reveal stark differences:
AlphaBeta incurs a significantly larger constant multiplicative factor compared
to algorithms like Scout, leading to a substantial practical slowdown. Our
framework sheds new light on classical game-solving algorithms, offering
rigorous evidence and analytical tools to advance the understanding of these
methods under a more realistic, challenging, and yet tractable model.

</details>


### [8] [LeanConjecturer: Automatic Generation of Mathematical Conjectures for Theorem Proving](https://arxiv.org/abs/2506.22005)
*Naoto Onda,Kazumi Kasaura,Yuta Oriike,Masaya Taniguchi,Akiyoshi Sannai,Sho Sonoda*

Main category: cs.AI

TL;DR: LeanConjecturer系统利用LLM自动在Lean 4中生成数学猜想，解决了形式定理证明中的数据稀缺问题。它生成了大量新颖且非平凡的猜想，并通过强化学习提升了定理证明能力，展示了在数学发现方面的巨大潜力。


<details>
  <summary>Details</summary>
Motivation: 解决形式定理证明中数据稀缺的挑战，并通过自动化生成数学猜想来提供训练数据。

Method: 引入了LeanConjecturer，一个结合了基于规则的上下文提取和基于LLM的定理陈述生成的管道。它通过迭代生成和评估来应对形式定理证明中的数据稀缺挑战。此外，生成的猜想通过群组相对策略优化（GRPO）用于强化学习，以增强定理证明能力。

Result: LeanConjecturer从40个Mathlib种子文件中生成了12,289个猜想，其中3,776个被认为是语法有效且非平凡的。平均每个种子文件生成103.25个新颖猜想。通过对领域特定猜想的定向训练，增强了定理证明能力。系统成功验证了拓扑学中的几个非平凡定理。

Conclusion: LeanConjecturer系统为定理证明系统提供了可扩展的训练数据生成方案，并展示了在数学发现方面的潜力，超越了现有结果的简单变体。

Abstract: We introduce LeanConjecturer, a pipeline for automatically generating
university-level mathematical conjectures in Lean 4 using Large Language Models
(LLMs). Our hybrid approach combines rule-based context extraction with
LLM-based theorem statement generation, addressing the data scarcity challenge
in formal theorem proving. Through iterative generation and evaluation,
LeanConjecturer produced 12,289 conjectures from 40 Mathlib seed files, with
3,776 identified as syntactically valid and non-trivial, that is, cannot be
proven by \texttt{aesop} tactic. We demonstrate the utility of these generated
conjectures for reinforcement learning through Group Relative Policy
Optimization (GRPO), showing that targeted training on domain-specific
conjectures can enhance theorem proving capabilities. Our approach generates
103.25 novel conjectures per seed file on average, providing a scalable
solution for creating training data for theorem proving systems. Our system
successfully verified several non-trivial theorems in topology, including
properties of semi-open, alpha-open, and pre-open sets, demonstrating its
potential for mathematical discovery beyond simple variations of existing
results.

</details>


### [9] [Universal Retrieval for Multimodal Trajectory Modeling](https://arxiv.org/abs/2506.22056)
*Xuan Zhang,Ziyan Jiang,Rui Meng,Yifei Leng,Zhenbang Xiao,Zora Zhiruo Wang,Yanyi Shang,Dehan Kong*

Main category: cs.AI

TL;DR: 该论文针对 GUI 环境中 AI 智能体轨迹数据表示建模的挑战，提出了多模态轨迹检索方法。他们构建了数据集和基准，并提出了 GAE-Retriever 框架，该框架在检索召回率方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 在轨迹数据爆炸式增长的情况下，如何系统地建模轨迹级数据的表示是一个尚未解决的重大挑战，尤其是在 GUI 环境中增强 AI 智能体能力方面。

Method: 提出了多模态轨迹检索（Multimodal Trajectory Retrieval），构建了统一智能体轨迹数据集（UATD）和基准（GAE-Bench），并提出了 GAE-Retriever 框架，该框架采用视觉-语言模型并通过代币选择和 GradCache 机制优化对比学习。

Result: GAE-Retriever 在多个数据集上的综合评估显示，它在检索召回率方面始终优于强大的基线。

Conclusion: GAE-Retriever 在多模态轨迹检索方面表现出色，有效提升了检索召回率。

Abstract: Trajectory data, capturing human actions and environmental states across
various modalities, holds significant potential for enhancing AI agent
capabilities, particularly in GUI environments. However, how to model the
representation of trajectory-level data presents a significant challenge that
has not been systematically addressed amid explosive trajectory data growth. In
this work, we introduce Multimodal Trajectory Retrieval, bridging the gap
between universal retrieval and agent-centric trajectory modeling. We construct
the Unified Agent Trajectory Dataset (UATD) from annotated demonstrations and
states across diverse real-world scenarios. Based on this, we present
GAE-Bench, a benchmark containing a large number of trajectory-based retrieval
pairs. In addition, we propose GAE-Retriever, a multimodal retrieval framework
that adopts vision-language models and incorporates optimized contrastive
learning through a token selection and the GradCache mechanism. Comprehensive
evaluations across multiple datasets show that GAE-Retriever consistently
outperforms strong baselines in retrieval recall, highlighting its
effectiveness in advancing multimodal trajectory retrieval.

</details>


### [10] [Query as Test: An Intelligent Driving Test and Data Storage Method for Integrated Cockpit-Vehicle-Road Scenarios](https://arxiv.org/abs/2506.22068)
*Shengyue Yao,Runqing Guo,Yangyang Qin,Miangbing Meng,Jipeng Cao,Yilun Lin,Yisheng Lv,Fei-Yue Wang*

Main category: cs.AI

TL;DR: 针对智能交通领域数据碎片化和测试效率低下的问题，本文提出了“查询即测试”（QaT）概念和基于ASP的“可扩展场景符号”（ESN）数据框架，以统一异构数据并实现更灵活、严谨的逻辑查询式测试，最终引入“验证驱动开发”（VDD）以加速开发流程。


<details>
  <summary>Details</summary>
Motivation: 人工智能在交通领域的深入渗透导致智能座舱、自动驾驶和智能路网的数据生态系统日益碎片化且不兼容，现有测试方法依赖数据堆叠，无法覆盖所有边缘案例且缺乏灵活性。

Method: 本文引入了“查询即测试”（QaT）概念，并提出了一种基于Answer Set Programming（ASP）的新型声明性数据框架“可扩展场景符号”（ESN），用于统一表示异构多模态数据。此外，还提出了“验证驱动开发”（VDD）范式。

Result: ESN不仅实现了数据的深度语义融合，还带来了三个核心优势：支持复杂灵活的语义查询、提供决策过程的自然可解释性、允许按需数据抽象以实现细粒度隐私保护。QaT范式显著增强了测试的表达性和形式严谨性。

Conclusion: 本文提出了“验证驱动开发”（VDD）的概念，建议在大模型时代通过逻辑验证而非定量测试来指导开发，以加速迭代和开发过程。

Abstract: With the deep penetration of Artificial Intelligence (AI) in the
transportation sector, intelligent cockpits, autonomous driving, and
intelligent road networks are developing at an unprecedented pace. However, the
data ecosystems of these three key areas are increasingly fragmented and
incompatible. Especially, existing testing methods rely on data stacking, fail
to cover all edge cases, and lack flexibility. To address this issue, this
paper introduces the concept of "Query as Test" (QaT). This concept shifts the
focus from rigid, prescripted test cases to flexible, on-demand logical queries
against a unified data representation. Specifically, we identify the need for a
fundamental improvement in data storage and representation, leading to our
proposal of "Extensible Scenarios Notations" (ESN). ESN is a novel declarative
data framework based on Answer Set Programming (ASP), which uniformly
represents heterogeneous multimodal data from the cockpit, vehicle, and road as
a collection of logical facts and rules. This approach not only achieves deep
semantic fusion of data, but also brings three core advantages: (1) supports
complex and flexible semantic querying through logical reasoning; (2) provides
natural interpretability for decision-making processes; (3) allows for
on-demand data abstraction through logical rules, enabling fine-grained privacy
protection. We further elaborate on the QaT paradigm, transforming the
functional validation and safety compliance checks of autonomous driving
systems into logical queries against the ESN database, significantly enhancing
the expressiveness and formal rigor of the testing. Finally, we introduce the
concept of "Validation-Driven Development" (VDD), which suggests to guide
developments by logical validation rather than quantitative testing in the era
of Large Language Models, in order to accelerating the iteration and
development process.

</details>


### [11] [SEEA-R1: Tree-Structured Reinforcement Fine-Tuning for Self-Evolving Embodied Agents](https://arxiv.org/abs/2506.21669)
*Wanxin Tian,Shijie Zhang,Kevin Zhang,Xiaowei Chi,Yulin Luo,Junyu Lu,Chunkai Fan,Qiang Zhou,Yiming Zhao,Ning Liu Siyu Lin,Zhiyuan Qin,Xiaozhu Ju,Shanghang Zhang,Jian Tang*

Main category: cs.AI

TL;DR: SEEA-R1是首个用于具身智能体自我进化的强化微调（RFT）框架，通过Tree-GRPO解决奖励稀疏性，通过MGRM泛化奖励估计，在ALFWorld基准测试中表现出色，超越GPT-4o等模型。


<details>
  <summary>Details</summary>
Motivation: 尽管强化微调（RFT）在增强LLM推理方面表现出色，但其在实现具有多模态交互的自进化具身智能方面的潜力仍未充分探索。RFT在具身环境中面临两个主要障碍：缺少中间奖励信号和对人工设计奖励函数的依赖。

Method: 提出了SEEA-R1，这是首个为实现具身智能体自我进化能力而设计的RFT框架。具体包括：1. 提出基于树的群体相对策略优化（Tree-GRPO），将蒙特卡洛树搜索集成到GRPO中，以将稀疏延迟奖励转化为更密集的中间信号。2. 引入多模态生成奖励模型（MGRM），以推广跨任务和场景的奖励估计。

Result: 在ALFWorld基准测试中，SEEA-R1超越了最先进的方法，文本得分85.07%，多模态得分36.19%，甚至超越了GPT-4o等模型。在没有环境奖励的情况下，SEEA-R1仍达到80.3%的得分，超越所有开源基线。

Conclusion: SEEA-R1的有效性得到了验证，并展现了其在未来可扩展具身智能研究中的巨大潜力。

Abstract: Self-evolution, the ability of agents to autonomously improve their reasoning
and behavior, is essential for the embodied domain with long-horizon,
real-world tasks. Despite current advancements in reinforcement fine-tuning
(RFT) showing strong performance in enhancing reasoning in LLMs, its potential
to enable self-evolving embodied intelligence with multi-modal interactions
remains largely unexplored. Specifically, reinforcement fine-tuning faces two
fundamental obstacles in embodied settings: (i) the lack of accessible
intermediate rewards in multi-step reasoning tasks limits effective learning
signals, and (ii) reliance on hand-crafted reward functions restricts
generalization to novel tasks and environments. To address these challenges, we
present Self-Evolving Embodied Agents-R1, SEEA-R1, the first RFT framework
designed for enabling the self-evolving capabilities of embodied agents.
Specifically, to convert sparse delayed rewards into denser intermediate
signals that improve multi-step reasoning, we propose Tree-based group relative
policy optimization (Tree-GRPO), which integrates Monte Carlo Tree Search into
GRPO. To generalize reward estimation across tasks and scenes, supporting
autonomous adaptation and reward-driven self-evolution, we further introduce
Multi-modal Generative Reward Model (MGRM). To holistically evaluate the
effectiveness of SEEA-R1, we evaluate on the ALFWorld benchmark, surpassing
state-of-the-art methods with scores of 85.07% (textual) and 36.19%
(multi-modal), outperforming prior models including GPT-4o. SEEA-R1 also
achieves scores of 80.3% without environmental reward, surpassing all
open-source baselines and highlighting its scalability as a self-evolving
embodied agent. Additional experiments and qualitative analysis further support
the potential of SEEA-R1 for future research in scalable embodied intelligence.

</details>


### [12] [SEEA-R1: Tree-Structured Reinforcement Fine-Tuning for Self-Evolving Embodied Agents](https://arxiv.org/abs/2506.21669)
*Wanxin Tian,Shijie Zhang,Kevin Zhang,Xiaowei Chi,Yulin Luo,Junyu Lu,Chunkai Fan,Qiang Zhou,Yiming Zhao,Ning Liu Siyu Lin,Zhiyuan Qin,Xiaozhu Ju,Shanghang Zhang,Jian Tang*

Main category: cs.AI

TL;DR: SEEA-R1是一个新的强化微调框架，通过引入Tree-GRPO和MGRM解决了具身智能体自我进化中稀疏奖励和泛化受限的问题，并在ALFWorld基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有强化微调（RFT）在具身领域面临两大障碍：多步推理任务中缺乏可访问的中间奖励，以及依赖手工奖励函数限制了对新任务和环境的泛化能力。本文旨在解决这些挑战，以实现具身智能体的自我进化能力。

Method: 本文提出了SEEA-R1框架，这是首个专为具身智能体自我进化能力设计的RFT框架。为解决稀疏延迟奖励问题，提出了基于树的群组相对策略优化（Tree-GRPO），将蒙特卡洛树搜索融入GRPO。为泛化奖励估计，引入了多模态生成奖励模型（MGRM）。

Result: SEEA-R1在ALFWorld基准测试中超越了现有最先进的方法，文本模式得分85.07%，多模态模式得分36.19%，表现优于包括GPT-4o在内的先前模型。SEEA-R1在没有环境奖励的情况下也取得了80.3%的分数，超过了所有开源基线。

Conclusion: SEEA-R1框架在没有明确环境奖励的情况下，也展示了实现可扩展具身智能和自我进化具身智能的巨大潜力。

Abstract: Self-evolution, the ability of agents to autonomously improve their reasoning
and behavior, is essential for the embodied domain with long-horizon,
real-world tasks. Despite current advancements in reinforcement fine-tuning
(RFT) showing strong performance in enhancing reasoning in LLMs, its potential
to enable self-evolving embodied intelligence with multi-modal interactions
remains largely unexplored. Specifically, reinforcement fine-tuning faces two
fundamental obstacles in embodied settings: (i) the lack of accessible
intermediate rewards in multi-step reasoning tasks limits effective learning
signals, and (ii) reliance on hand-crafted reward functions restricts
generalization to novel tasks and environments. To address these challenges, we
present Self-Evolving Embodied Agents-R1, SEEA-R1, the first RFT framework
designed for enabling the self-evolving capabilities of embodied agents.
Specifically, to convert sparse delayed rewards into denser intermediate
signals that improve multi-step reasoning, we propose Tree-based group relative
policy optimization (Tree-GRPO), which integrates Monte Carlo Tree Search into
GRPO. To generalize reward estimation across tasks and scenes, supporting
autonomous adaptation and reward-driven self-evolution, we further introduce
Multi-modal Generative Reward Model (MGRM). To holistically evaluate the
effectiveness of SEEA-R1, we evaluate on the ALFWorld benchmark, surpassing
state-of-the-art methods with scores of 85.07% (textual) and 36.19%
(multi-modal), outperforming prior models including GPT-4o. SEEA-R1 also
achieves scores of 80.3% without environmental reward, surpassing all
open-source baselines and highlighting its scalability as a self-evolving
embodied agent. Additional experiments and qualitative analysis further support
the potential of SEEA-R1 for future research in scalable embodied intelligence.

</details>


### [13] [Hierarchical Reasoning Model](https://arxiv.org/abs/2506.21734)
*Guan Wang,Jin Li,Yuhao Sun,Xing Chen,Changling Liu,Yue Wu,Meng Lu,Sen Song,Yasin Abbasi Yadkori*

Main category: cs.AI

TL;DR: 受人脑启发，一种名为HRM的新模型，以极少的参数和数据，高效准确地解决了复杂推理任务，性能超越了大型模型。


<details>
  <summary>Details</summary>
Motivation: 推理是AI中的一个关键挑战，当前大型语言模型（LLM）主要采用思维链（CoT）技术，但这些技术存在任务分解脆弱、数据需求大和延迟高等问题。

Method: 本文提出了分层推理模型（HRM），这是一种新颖的循环架构，通过两个相互依赖的循环模块（一个用于慢速、抽象规划的高级模块和一个用于快速、详细计算的低级模块）在单次前向传播中执行顺序推理任务，无需中间过程的显式监督。

Result: HRM模型仅用2700万参数和1000个训练样本（无需预训练或CoT数据），在复杂推理任务（包括复杂数独谜题和大型迷宫中的最优路径查找）上取得了卓越的性能。此外，HRM在抽象和推理语料库（ARC）上超越了参数更多、上下文窗口更长的模型。

Conclusion: HRM模型在通用计算和通用推理系统方面具有巨大的潜力。

Abstract: Reasoning, the process of devising and executing complex goal-oriented action
sequences, remains a critical challenge in AI. Current large language models
(LLMs) primarily employ Chain-of-Thought (CoT) techniques, which suffer from
brittle task decomposition, extensive data requirements, and high latency.
Inspired by the hierarchical and multi-timescale processing in the human brain,
we propose the Hierarchical Reasoning Model (HRM), a novel recurrent
architecture that attains significant computational depth while maintaining
both training stability and efficiency. HRM executes sequential reasoning tasks
in a single forward pass without explicit supervision of the intermediate
process, through two interdependent recurrent modules: a high-level module
responsible for slow, abstract planning, and a low-level module handling rapid,
detailed computations. With only 27 million parameters, HRM achieves
exceptional performance on complex reasoning tasks using only 1000 training
samples. The model operates without pre-training or CoT data, yet achieves
nearly perfect performance on challenging tasks including complex Sudoku
puzzles and optimal path finding in large mazes. Furthermore, HRM outperforms
much larger models with significantly longer context windows on the Abstraction
and Reasoning Corpus (ARC), a key benchmark for measuring artificial general
intelligence capabilities. These results underscore HRM's potential as a
transformative advancement toward universal computation and general-purpose
reasoning systems.

</details>


### [14] [THE-Tree: Can Tracing Historical Evolution Enhance Scientific Verification and Reasoning?](https://arxiv.org/abs/2506.21763)
*Xin Wang,Jiyao Liu,Yulong Xiao,Junzhi Ning,Lihao Liu,Junjun He,Botian Shi,Kaicheng Yu*

Main category: cs.AI

TL;DR: THE-Tree是一个计算框架，利用LLM和NLI从科学文献中构建可验证的技术演化树，解决AI生成思想评估的瓶颈，显著提升图补全、未来预测和论文评估能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型加速科学思想生成，但评估AI生成命题的新颖性和事实准确性是关键瓶颈，因为手动验证太慢。现有验证方法不足：LLM可能产生幻觉且缺乏领域知识，传统引用网络缺乏明确因果关系，叙述性调查缺乏结构。核心挑战是缺乏结构化、可验证、因果关联的科学演化历史数据。

Method: 引入THE-Tree计算框架，通过搜索算法构建领域特定演化树。该框架采用“思考-表达-引用-验证”过程，LLM提出潜在进展并引用文献，然后通过自然语言推理机制验证每个进化链接的逻辑一致性和证据支持。

Result: 构建并验证了88个THE-Tree，发布了包含7.1万事实验证和2.7万篇论文的基准数据集。实验表明：i) 在图补全方面，THE-Tree比传统引文网络将hit@1提高了8%至14%；ii) 在预测未来科学发展方面，hit@1提升近10%；iii) 与其他方法结合时，评估重要科学论文的性能提升近100%。

Conclusion: THE-Tree为科学思想的结构化、可验证评估提供了一个有效框架，显著提升了图补全、未来预测和重要论文评估的性能。

Abstract: Large Language Models (LLMs) are accelerating scientific idea generation, but
rigorously evaluating these numerous, often superficial, AI-generated
propositions for novelty and factual accuracy is a critical bottleneck; manual
verification is too slow.Existing validation methods are inadequate: LLMs as
standalone verifiers may hallucinate and lack domain knowledge (our findings
show ~60\% unawareness of relevant papers in specific domains), while
traditional citation networks lack explicit causality and narrative surveys are
unstructured.This underscores a core challenge: the absence of structured,
verifiable, and causally-linked historical data of scientific evolution.To
address this,we introduce \textbf{THE-Tree} (\textbf{T}echnology
\textbf{H}istory \textbf{E}volution Tree), a computational framework that
constructs such domain-specific evolution trees from scientific
literature.THE-Tree employs a search algorithm to explore evolutionary paths.
During its node expansion, it utilizes a novel "Think-Verbalize-Cite-Verify"
process: an LLM proposes potential advancements and cites supporting
literature. Critically, each proposed evolutionary link is then validated for
logical coherence and evidential support by a recovered natural language
inference mechanism that interrogates the cited literature, ensuring that each
step is grounded.We construct and validate 88 THE-Trees across diverse domains
and release a benchmark dataset including up to 71k fact verifications covering
27k papers to foster further research.Experiments demonstrate that i) in graph
completion, our THE-Tree improves hit@1 by 8\% to 14\% across multiple models
compared to traditional citation networks; ii) for predicting future scientific
developments, it improves hit@1 metric by nearly 10\%; and iii) when combined
with other methods, it boosts the performance of evaluating important
scientific papers by almost 100\%.

</details>


<div id='cs.AR'></div>

# cs.AR [[Back]](#toc)

### [15] [Power- and Area-Efficient Unary Sorting Architecture Using FSM-Based Unary Number Generator](https://arxiv.org/abs/2506.22107)
*Amir Hossein Jalilvand,M. Hassan Najafi*

Main category: cs.AR

TL;DR: 论文提出了一种基于有限状态机一元数生成器的新型一元排序器，显著降低了面积和功耗，适用于资源受限的硬件系统。


<details>
  <summary>Details</summary>
Motivation: 现有的基于一元计算的无比较器设计因昂贵的一元数生成器而面临显著的面积和功耗开销。

Method: 提出一种新型的升序一元排序模块，该模块采用基于有限状态机的一元数生成器。通过使用两状态有限状态机生成右对齐的一元流，该架构在每个周期迭代识别最小值，无需传统的比较器。

Result: 在45nm技术节点下的综合结果表明，与最先进的一元设计相比，面积减少高达82%，功耗降低70%。

Conclusion: 所提出的排序器为能量受限和资源受限的硬件系统提供了一个有前景的解决方案。

Abstract: Sorting is a fundamental operation in computer systems and is widely used in
applications such as databases, data analytics, and hardware accelerators.
Unary computing has recently emerged as a low-cost and power-efficient paradigm
for implementing hardware sorters by eliminating the need for complex
arithmetic operations. However, existing comparison-free unary computing-based
designs suffer from significant area and power overhead due to costly unary
number generators.
  In this paper, we present a novel ascending-order unary sorting module
featuring a finite-state-machine-based unary number generator that
significantly reduces implementation costs. By generating right-aligned unary
streams using a two-state finite-state machine, our architecture iteratively
identifies the minimum input value in each cycle without conventional
comparators. Synthesis results in a 45nm technology node demonstrate up to 82%
reduction in area and 70% reduction in power consumption compared to
state-of-the-art unary designs. The proposed sorter offers a promising solution
for energy-constrained and resource-limited hardware systems.

</details>


### [16] [Hardware acceleration for ultra-fast Neural Network training on FPGA for MRF map reconstruction](https://arxiv.org/abs/2506.22156)
*Mattia Ricchi,Fabrizio Alfonsi,Camilla Marella,Marco Barbieri,Alessandra Retico,Leonardo Brizi,Alessandro Gabrielli,Claudia Testa*

Main category: cs.AR

TL;DR: 利用FPGA的神经网络加速MRF脑参数重建，显著缩短训练时间，并有望在移动设备上实现实时大脑分析。


<details>
  <summary>Details</summary>
Motivation: MRF是一种快速定量MRI技术，可以单次采集提供多参数图。但神经网络加速重建需要大量训练资源，目标是在移动设备上实现实时大脑分析。

Method: 本文提出了一种基于FPGA的神经网络，用于从MRF数据中进行实时脑参数重建。

Result: 该神经网络的训练时间估计为200秒，比标准的基于CPU的训练快250倍。

Conclusion: 这种方法有望在移动设备上实现实时大脑分析，彻底改变临床决策和远程医疗。

Abstract: Magnetic Resonance Fingerprinting (MRF) is a fast quantitative MR Imaging
technique that provides multi-parametric maps with a single acquisition. Neural
Networks (NNs) accelerate reconstruction but require significant resources for
training. We propose an FPGA-based NN for real-time brain parameter
reconstruction from MRF data. Training the NN takes an estimated 200 seconds,
significantly faster than standard CPU-based training, which can be up to 250
times slower. This method could enable real-time brain analysis on mobile
devices, revolutionizing clinical decision-making and telemedicine.

</details>


### [17] [Power- and Area-Efficient Unary Sorting Architecture Using FSM-Based Unary Number Generator](https://arxiv.org/abs/2506.22107)
*Amir Hossein Jalilvand,M. Hassan Najafi*

Main category: cs.AR

TL;DR: 本文提出了一种基于有限状态机的一元排序模块，显著降低了实现成本，与现有的一元设计相比，面积和功耗分别减少了82%和70%。


<details>
  <summary>Details</summary>
Motivation: 现有的基于无比较一元计算的设计由于成本高昂的一元数生成器而导致显著的面积和功耗开销。

Method: 本文提出了一种新颖的升序一元排序模块，其特点是采用基于有限状态机的一元数生成器，显著降低了实现成本。通过使用两态有限状态机生成右对齐的一元流，我们的架构在每个周期内迭代识别最小值，无需传统的比较器。

Result: 在45纳米技术节点下的综合结果表明，与最先进的一元设计相比，面积减少了82%，功耗降低了70%。

Conclusion: 该排序器为能量受限和资源有限的硬件系统提供了一个有前景的解决方案。

Abstract: Sorting is a fundamental operation in computer systems and is widely used in
applications such as databases, data analytics, and hardware accelerators.
Unary computing has recently emerged as a low-cost and power-efficient paradigm
for implementing hardware sorters by eliminating the need for complex
arithmetic operations. However, existing comparison-free unary computing-based
designs suffer from significant area and power overhead due to costly unary
number generators.
  In this paper, we present a novel ascending-order unary sorting module
featuring a finite-state-machine-based unary number generator that
significantly reduces implementation costs. By generating right-aligned unary
streams using a two-state finite-state machine, our architecture iteratively
identifies the minimum input value in each cycle without conventional
comparators. Synthesis results in a 45nm technology node demonstrate up to 82%
reduction in area and 70% reduction in power consumption compared to
state-of-the-art unary designs. The proposed sorter offers a promising solution
for energy-constrained and resource-limited hardware systems.

</details>


<div id='cs.CG'></div>

# cs.CG [[Back]](#toc)

### [18] [Computing Maximum Cliques in Unit Disk Graphs](https://arxiv.org/abs/2506.21926)
*Anastasiia Tkachenko,Haitao Wang*

Main category: cs.CG

TL;DR: 提供了计算单位圆盘图最大团的更快算法，尤其针对小团和凸位置点集。


<details>
  <summary>Details</summary>
Motivation: 计算单位圆盘图中的最大团问题。现有的最佳算法时间复杂度为$O(n^{7/3+o(1)})$，目标是找到更快的算法，尤其是在最大团大小$K=o(n)$时。

Method: 对于一般情况，提出了一种时间复杂度为$O(n \log n + n K^{4/3+o(1)})$的算法。对于凸位置的点集，提出了一种时间复杂度为$O(n^{15/7+o(1)})$的随机算法。当已知最大团中的一个点且点集呈凸位置时，提出了一种时间复杂度为$O(n^2\log n)$的确定性算法。

Result: 新的算法在$K=o(n)$时比现有算法更快。对于凸位置点集，随机算法能以高概率计算出最大团。对于凸位置且已知最大团中一点的特殊情况，提供了确定性算法。

Conclusion: 本文提出了计算单位圆盘图中最大团的改进算法，尤其在最大团大小K较小或点集呈凸位置时，算法性能有显著提升。

Abstract: Given a set $P$ of $n$ points in the plane, the unit-disk graph $G(P)$ is a
graph with $P$ as its vertex set such that two points of $P$ have an edge if
their Euclidean distance is at most $1$. We consider the problem of computing a
maximum clique in $G(P)$. The previously best algorithm for the problem runs in
$O(n^{7/3+o(1)})$ time. We show that the problem can be solved in $O(n \log n +
n K^{4/3+o(1)})$ time, where $K$ is the maximum clique size. The algorithm is
faster than the previous one when $K=o(n)$. In addition, if $P$ is in convex
position, we give a randomized algorithm that runs in $O(n^{15/7+o(1)})=
O(n^{2.143})$ worst-case time and the algorithm can compute a maximum clique
with high probability. For points in convex position, one special case we solve
is when a point in the maximum clique is given; we present an $O(n^2\log n)$
time (deterministic) algorithm for this special case.

</details>


### [19] [The Walk-Length Filtration for Persistent Homology on Weighted Directed Graphs](https://arxiv.org/abs/2506.22263)
*David E. Muñoz,Elizabeth Munch,Firas A. Khasawneh*

Main category: cs.CG

TL;DR: 本文介绍了一种新的有向图过滤方法——步长过滤，用于持久同调分析，并证明了其在L1距离下的稳定性，同时提供了一个计算算法。


<details>
  <summary>Details</summary>
Motivation: 在许多有向图的应用中，计算持久同调有助于编码输入信息的形状和结构，但将有向图信息转化为标准持久同调框架所需的无向单纯复形过滤的方法很少。

Method: 提出了一种新的从有向图构建的“步长过滤”方法，该方法反映了小步在有向图中访问特定顶点集合的行为。

Result: 研究表明，虽然在通常的L∞型网络距离下持久性不稳定，但在广义L1型距离下是稳定的。文章还提供了一种计算该过滤的算法，并通过实例（包括循环网络和合成海马网络）研究了其行为，并与常用的Dowker过滤进行了比较。

Conclusion: 本文提出了一种新的从有向图构建的过滤方法，即步长过滤，并证明了其在广义L1距离下的稳定性，为有向图的持久同调分析提供了一个新的工具。

Abstract: Directed graphs arise in many applications where computing persistent
homology helps to encode the shape and structure of the input information.
However, there are only a few ways to turn the directed graph information into
an undirected simplicial complex filtration required by the standard persistent
homology framework. In this paper, we present a new filtration constructed from
a directed graph, called the walk-length filtration. This filtration mirrors
the behavior of small walks visiting certain collections of vertices in the
directed graph. We show that, while the persistence is not stable under the
usual $L_\infty$-style network distance, a generalized $L_1$-style distance is,
indeed, stable. We further provide an algorithm for its computation, and
investigate the behavior of this filtration in examples, including cycle
networks and synthetic hippocampal networks with a focus on comparison to the
often used Dowker filtration.

</details>


### [20] [Computing Maximum Cliques in Unit Disk Graphs](https://arxiv.org/abs/2506.21926)
*Anastasiia Tkachenko,Haitao Wang*

Main category: cs.CG

TL;DR: 本文针对单位圆盘图的最大团问题，提出了更快的算法，尤其在特定点集配置（如凸位置）或最大团相对较小时，性能有所提升。


<details>
  <summary>Details</summary>
Motivation: 寻找单位圆盘图中最大团的问题，现有算法效率不高，时间复杂度为 $O(n^{7/3+o(1)})$。

Method: 提出了一种时间复杂度为 $O(n \log n + n K^{4/3+o(1)})$ 的通用算法。对于凸位置的点集，提出了一个随机算法（时间复杂度 $O(n^{15/7+o(1)})$）和一个确定性算法（时间复杂度 $O(n^2\log n)$，当已知最大团中一点时）。

Result: 新的通用算法在最大团大小 $K=o(n)$ 时比现有算法更快。对于凸位置的点集，随机算法能以高概率计算出最大团。

Conclusion: 本文为单位圆盘图中的最大团问题提出了更快的算法，在特定条件下（如点位于凸位置或最大团较小）表现更优。

Abstract: Given a set $P$ of $n$ points in the plane, the unit-disk graph $G(P)$ is a
graph with $P$ as its vertex set such that two points of $P$ have an edge if
their Euclidean distance is at most $1$. We consider the problem of computing a
maximum clique in $G(P)$. The previously best algorithm for the problem runs in
$O(n^{7/3+o(1)})$ time. We show that the problem can be solved in $O(n \log n +
n K^{4/3+o(1)})$ time, where $K$ is the maximum clique size. The algorithm is
faster than the previous one when $K=o(n)$. In addition, if $P$ is in convex
position, we give a randomized algorithm that runs in $O(n^{15/7+o(1)})=
O(n^{2.143})$ worst-case time and the algorithm can compute a maximum clique
with high probability. For points in convex position, one special case we solve
is when a point in the maximum clique is given; we present an $O(n^2\log n)$
time (deterministic) algorithm for this special case.

</details>


### [21] [Computing Maximum Cliques in Unit Disk Graphs](https://arxiv.org/abs/2506.21926)
*Anastasiia Tkachenko,Haitao Wang*

Main category: cs.CG

TL;DR: 本文提出了在单位圆盘图中寻找最大团的改进算法，在最大团较小或点集呈凸位置时性能更优。


<details>
  <summary>Details</summary>
Motivation: 解决在单位圆盘图中计算最大团的问题，并改进现有算法的性能。先前的最佳算法时间复杂度为$O(n^{7/3+o(1)})$。

Method: 1. 对于一般情况，提出了一种时间复杂度为$O(n \log n + n K^{4/3+o(1)})$的算法。2. 对于凸位置点集，提出了一种最坏情况时间复杂度为$O(n^{15/7+o(1)})$的随机算法。3. 对于凸位置点集中给定最大团中一点的特殊情况，提出了一种$O(n^2\log n)$的确定性算法。

Result: 1. 新算法在K=o(n)时比之前的算法($O(n^{7/3+o(1)})$)更快。2. 针对凸位置点集的随机算法能以高概率计算出最大团。3. 针对给定最大团中一点的凸位置点集的特殊情况，提供了一个确定性算法。

Conclusion: 本文提出了在单位圆盘图中计算最大团的改进算法，尤其是在最大团大小K相对于点数n较小时以及在凸位置点集的情况下。

Abstract: Given a set $P$ of $n$ points in the plane, the unit-disk graph $G(P)$ is a
graph with $P$ as its vertex set such that two points of $P$ have an edge if
their Euclidean distance is at most $1$. We consider the problem of computing a
maximum clique in $G(P)$. The previously best algorithm for the problem runs in
$O(n^{7/3+o(1)})$ time. We show that the problem can be solved in $O(n \log n +
n K^{4/3+o(1)})$ time, where $K$ is the maximum clique size. The algorithm is
faster than the previous one when $K=o(n)$. In addition, if $P$ is in convex
position, we give a randomized algorithm that runs in $O(n^{15/7+o(1)})=
O(n^{2.143})$ worst-case time and the algorithm can compute a maximum clique
with high probability. For points in convex position, one special case we solve
is when a point in the maximum clique is given; we present an $O(n^2\log n)$
time (deterministic) algorithm for this special case.

</details>


### [22] [The Walk-Length Filtration for Persistent Homology on Weighted Directed Graphs](https://arxiv.org/abs/2506.22263)
*David E. Muñoz,Elizabeth Munch,Firas A. Khasawneh*

Main category: cs.CG

TL;DR: 提出了一种新的有向图“步长过滤”方法，解决了有向图持久同调计算中过滤方法的不足，并证明了其在广义L1距离下的稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在将有向图信息转换为无向单纯复形过滤以用于标准持久同调框架时存在不足。

Method: 提出了一种新的有向图过滤方法，称为“步长过滤”（walk-length filtration），该过滤反映了小路径在有向图中访问顶点集合的行为。

Result: 证明了该过滤在广义L1距离下是稳定的，尽管在L∞风格网络距离下不稳定。提供了其计算算法，并在循环网络和合成海马网络等示例中研究了其行为，与Dowker过滤进行了比较。

Conclusion: 本文成功提出了一种针对有向图的稳定且可计算的持久同调过滤方法，解决了现有方法的局限性。

Abstract: Directed graphs arise in many applications where computing persistent
homology helps to encode the shape and structure of the input information.
However, there are only a few ways to turn the directed graph information into
an undirected simplicial complex filtration required by the standard persistent
homology framework. In this paper, we present a new filtration constructed from
a directed graph, called the walk-length filtration. This filtration mirrors
the behavior of small walks visiting certain collections of vertices in the
directed graph. We show that, while the persistence is not stable under the
usual $L_\infty$-style network distance, a generalized $L_1$-style distance is,
indeed, stable. We further provide an algorithm for its computation, and
investigate the behavior of this filtration in examples, including cycle
networks and synthetic hippocampal networks with a focus on comparison to the
often used Dowker filtration.

</details>


<div id='cs.CL'></div>

# cs.CL [[Back]](#toc)

### [23] [Efficient Multilingual ASR Finetuning via LoRA Language Experts](https://arxiv.org/abs/2506.21555)
*Jiahong Li,Yiwen Shao,Jianheng Zhuo,Chenda Li,Liliang Tang,Dong Yu,Yanmin Qian*

Main category: cs.CL

TL;DR: 提出一种基于LoRA语言专家对Whisper模型进行高效微调的方法，以解决多语言ASR中语言间干扰的问题，并在识别性能上取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习显著提升了多语言ASR，但多语言ASR仍面临“多语言诅咒”，即不同语言间会相互干扰，使得模型难以有效识别多种语言并共享模型容量。

Method: 提出了一种基于Whisper的、通过预训练LoRA语言专家进行定制化多语言ASR的有效微调框架。该方法通过LoRA专家融合或知识蒸馏实现。

Result: 在目标语言上的识别性能优于标准微调方法。在语言感知和语言无关场景中，分别获得了约10%和15%的相对性能提升。

Conclusion: 通过基于LoRA语言专家的微调框架，可以显著提高多语言ASR的性能，有效缓解语言间的干扰问题。

Abstract: Recent advancements in deep learning have significantly enhanced multilingual
automatic speech recognition (ASR) due to the development of advanced model
architectures and available large-scale multilingual datasets. Despite that,
multilingual ASR still suffers from the curse of multilinguality in that
different languages tend to interfere with each other, making it difficult for
the ASR model to identify multiple languages effectively while sharing model
capacity across them. This paper proposes an efficient finetuning framework for
customized multilingual ASR via prepared LoRA language experts based on
Whisper. Through LoRA expert fusion or knowledge distillation, our approach
achieves better recognition performance on target languages than standard
fine-tuning methods. Experimental results demonstrate that the proposed models
yield approximately 10\% and 15\% relative performance gains in language-aware
and language-agnostic scenarios, respectively.

</details>


### [24] [VAT-KG: Knowledge-Intensive Multimodal Knowledge Graph Dataset for Retrieval-Augmented Generation](https://arxiv.org/abs/2506.21556)
*Hyeongcheol Park,MinHyuk Jang,Ha Dam Baek,Gyusam Chang,Jiyoung Seo,Jiwan Park,Hogun Park,Sangpil Kim*

Main category: cs.CL

TL;DR: 提出了VAT-KG，一个支持视觉、音频和文本的多模态知识图谱，旨在解决现有MMKGs的局限性，并通过新的RAG框架增强MLLMs，并在多模态问答中显示出有效性。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态知识图谱（MMKGs）范围有限，通常通过增强现有知识图谱构建，导致知识过时或不完整，且仅支持文本和视觉等狭窄的模态，限制了其可扩展性和适用性。

Method: 提出了视觉-音频-文本知识图谱（VAT-KG），这是一个以概念为中心、知识密集的多模态知识图谱，涵盖视觉、音频和文本信息。其构建流程确保了多模态数据与细粒度语义之间的跨模态知识对齐。此外，还引入了一种新颖的多模态RAG框架。

Result: 在跨多种模态的问答任务中进行的实验证明了VAT-KG在支持多模态大型语言模型（MLLMs）方面的有效性。

Conclusion: VAT-KG在统一和利用多模态知识方面具有实际价值。

Abstract: Multimodal Knowledge Graphs (MMKGs), which represent explicit knowledge
across multiple modalities, play a pivotal role by complementing the implicit
knowledge of Multimodal Large Language Models (MLLMs) and enabling more
grounded reasoning via Retrieval Augmented Generation (RAG). However, existing
MMKGs are generally limited in scope: they are often constructed by augmenting
pre-existing knowledge graphs, which restricts their knowledge, resulting in
outdated or incomplete knowledge coverage, and they often support only a narrow
range of modalities, such as text and visual information. These limitations
reduce their extensibility and applicability to a broad range of multimodal
tasks, particularly as the field shifts toward richer modalities such as video
and audio in recent MLLMs. Therefore, we propose the Visual-Audio-Text
Knowledge Graph (VAT-KG), the first concept-centric and knowledge-intensive
multimodal knowledge graph that covers visual, audio, and text information,
where each triplet is linked to multimodal data and enriched with detailed
descriptions of concepts. Specifically, our construction pipeline ensures
cross-modal knowledge alignment between multimodal data and fine-grained
semantics through a series of stringent filtering and alignment steps, enabling
the automatic generation of MMKGs from any multimodal dataset. We further
introduce a novel multimodal RAG framework that retrieves detailed
concept-level knowledge in response to queries from arbitrary modalities.
Experiments on question answering tasks across various modalities demonstrate
the effectiveness of VAT-KG in supporting MLLMs, highlighting its practical
value in unifying and leveraging multimodal knowledge.

</details>


### [25] [Debunk and Infer: Multimodal Fake News Detection via Diffusion-Generated Evidence and LLM Reasoning](https://arxiv.org/abs/2506.21557)
*Kaiying Yan,Moyang Liu,Yukun Liu,Ruibo Fu,Zhengqi Wen,Jianhua Tao,Xuefei Liu*

Main category: cs.CL

TL;DR: DIFND框架利用驳斥知识、条件扩散模型和多模态大语言模型来检测假新闻，在多个数据集上显示出更高的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 快速传播的假新闻对信息可信度构成严重挑战。

Method: 提出DIFND框架，结合条件扩散模型和多模态大语言模型（MLLMs），利用驳斥知识生成反驳或验证证据，并通过多智能体MLLM系统进行链式驳斥推理，共同建模多模态特征、生成式驳斥线索和推理丰富的验证。

Result: 在FakeSV和FVC数据集上，DIFND显著提升了检测准确性，优于现有方法。

Conclusion: DIFND不仅超越了现有方法，还提供了可靠的决策。

Abstract: The rapid spread of fake news across multimedia platforms presents serious
challenges to information credibility. In this paper, we propose a
Debunk-and-Infer framework for Fake News Detection(DIFND) that leverages
debunking knowledge to enhance both the performance and interpretability of
fake news detection. DIFND integrates the generative strength of conditional
diffusion models with the collaborative reasoning capabilities of multimodal
large language models (MLLMs). Specifically, debunk diffusion is employed to
generate refuting or authenticating evidence based on the multimodal content of
news videos, enriching the evaluation process with diverse yet semantically
aligned synthetic samples. To improve inference, we propose a chain-of-debunk
strategy where a multi-agent MLLM system produces logic-grounded,
multimodal-aware reasoning content and final veracity judgment. By jointly
modeling multimodal features, generative debunking cues, and reasoning-rich
verification within a unified architecture, DIFND achieves notable improvements
in detection accuracy. Extensive experiments on the FakeSV and FVC datasets
show that DIFND not only outperforms existing approaches but also delivers
trustworthy decisions.

</details>


### [26] [Bench to the Future: A Pastcasting Benchmark for Forecasting Agents](https://arxiv.org/abs/2506.21558)
*FutureSearch,:,Jack Wildman,Nikos I. Bosse,Daniel Hnyk,Peter Mühlbacher,Finn Hambly,Jon Evans,Dan Schwarz,Lawrence Phillips*

Main category: cs.CL

TL;DR: 介绍了一个名为BTF的“回溯预测”基准测试，旨在为LLM提供一个现实、可重复的预测能力评估环境，并证明其结果可与真实预测相媲美。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏为LLM预测器提供真实、密封且可重复环境的预测基准测试。

Method: 引入了“回溯预测”（pastcasting）基准测试BTF，包含数百个已解决的问题，并为每个问题提供大量的离线网页语料库，以模拟现实世界的预测环境。

Result: BTF的回溯预测环境可以产生与使用互联网对当时未解决问题进行预测所获得的可比结果，并能有效地评估代理和思维链预测方法，以及追踪LLM预测能力的进展。

Conclusion: 本文提出的BTF基准测试为LLM预测能力提供了一个现实、密封且可重复的评估环境，并有望成为一个持续更新的基准。

Abstract: Forecasting is a challenging task that offers a clearly measurable way to
study AI systems. Forecasting requires a large amount of research on the
internet, and evaluations require time for events to happen, making the
development of forecasting benchmarks challenging. To date, no forecasting
benchmark provides a realistic, hermetic, and repeatable environment for LLM
forecasters. We introduce Bench To the Future (BTF), a "pastcasting" benchmark
with hundreds of high-quality questions for which the resolution is already
known. Each question is accompanied by a large offline corpus of tens of
thousands of relevant web pages, enabling a way to elicit realistic "forecasts"
on past events from LLMs. Results suggest that our pastcasting environment can
produce results comparable to those based on forecasts using the internet on
at-the-time unresolved questions. We show results benchmarking agent and
chain-of-thought forecasting approaches using several LLMs, including the
recently-released Claude 4 models, and demonstrate BTF's ability to track
steady forecasting capability progress over time. We intend this to be a living
benchmark, with new questions added continually to account for increasing
training data cutoff dates. We invite researchers to contact us at
hello@futuresearch.ai to utilize our benchmark or tooling for their own
research.

</details>


### [27] [FloorPlan-DeepSeek (FPDS): A multimodal approach to floorplan generation using vector-based next room prediction](https://arxiv.org/abs/2506.21562)
*Jun Yin,Pengyu Zeng,Jing Zhong,Peilin Li,Miao Zhang,Ran Luo,Shuai Lu*

Main category: cs.CL

TL;DR: 针对现有平面图生成模型与建筑实践不兼容的问题，本文提出了一种受大型语言模型启发的“下一房间预测”新范式，实验证明其在文本到平面图任务中具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有的平面图生成模型主要是端到端的，与现实世界建筑实践中观察到的增量工作流程不兼容。

Method: 受大型语言模型中自回归“下一词元预测”机制的启发，提出了一种专为建筑平面图建模量身定制的“下一房间预测”新范式。

Result: 实验评估表明，FPDS在文本到平面图任务中与扩散模型和Tell2Design相比表现出竞争力。

Conclusion: FPDS在文本到平面图任务中表现出与扩散模型和Tell2Design相当的竞争力，预示着其在未来智能建筑设计中具有潜在的应用前景。

Abstract: In the architectural design process, floor plan generation is inherently
progressive and iterative. However, existing generative models for floor plans
are predominantly end-to-end generation that produce an entire pixel-based
layout in a single pass. This paradigm is often incompatible with the
incremental workflows observed in real-world architectural practice. To address
this issue, we draw inspiration from the autoregressive 'next token prediction'
mechanism commonly used in large language models, and propose a novel 'next
room prediction' paradigm tailored to architectural floor plan modeling.
Experimental evaluation indicates that FPDS demonstrates competitive
performance in comparison to diffusion models and Tell2Design in the
text-to-floorplan task, indicating its potential applicability in supporting
future intelligent architectural design.

</details>


### [28] [GraphLAMA: Enabling Efficient Adaptation of Graph Language Models with Limited Annotations](https://arxiv.org/abs/2506.21559)
*Junze Chen,Cheng Yang,Shujie Li,Zhiqiang Zhang,Yawen Li,Junping Du,Chuan Shi*

Main category: cs.CL

TL;DR: GraphLAMA通过参数自适应解决了图语言模型中上下文学习的效率问题和指令微调的数据需求问题，实现了更好的预测精度和更快的推理速度。


<details>
  <summary>Details</summary>
Motivation: 图语言模型（GLMs）中的上下文学习（ICL）存在有效性问题（参数固定）和效率问题（长上下文）。同时，指令微调所需的大量标注数据在实际场景中难以获取。因此，本文旨在引入一个额外的参数自适应阶段，以少量标注样本高效地调整GLM，从而提高预测精度和推理速度。

Method: 本文提出了GraphLAMA方法，其模型骨干采用图神经网络（GNN）将节点转换为LLM标记的表示空间，并将任务指令表示为节点和语言标记的混合。在预训练阶段，除了LLM之外的模型参数会针对不同任务进行训练以捕获通用知识。在自适应阶段，仅基于少量样本更新少量预训练参数。

Result: 在少量/零样本节点分类和摘要生成任务上，GraphLAMA实现了最先进的性能，准确率绝对提升了4.91%。在5样本设置下，推理速度比ICL快10倍。

Conclusion: GraphLAMA通过引入高效的参数自适应阶段，成功解决了图语言模型中上下文学习的有效性和效率问题以及指令微调对大量标注数据的需求，显著提高了预测准确性并加快了推理速度。

Abstract: Large language models (LLMs) have demonstrated their strong capabilities in
various domains, and have been recently integrated for graph analysis as graph
language models (GLMs). With LLMs as the predictor, some GLMs can interpret
unseen tasks described by natural language, and learn from a few examples in
the prompts without parameter tuning, known as in-context learning (ICL).
Another subset of GLMs utilizes abundant training labels to enhance model
performance, known as instruction tuning. However, we argue that ICL on graphs
has effectiveness issues due to fixed parameters and efficiency issues due to
long context. Meanwhile, the large amount of labeled data required for
instruction tuning can be difficult to obtain in real-world scenarios. To this
end, we aim to introduce an extra parameter adaptation stage that can
efficiently tailor GLMs to an unseen graph and task with only a few labeled
examples, in exchange for better prediction accuracy and faster inference
speed. For implementation, in this paper we propose GraphLAMA method, with its
model backbone and learning schemes specialized for efficient tuning and
inference. Specifically, for model backbone, we use a graph neural network
(GNN) with several well-designed components to transform nodes into the
representation space of LLM tokens. Task instructions can then be represented
as a mixture of node and language tokens. In the pre-training stage, model
parameters except the LLM will be trained with different tasks to capture
general knowledge. In the adaptation stage, only a few pre-trained parameters
will be updated based on few-shot examples. Extensive experiments on
few/zero-shot node classification and summary generation show that our proposed
GraphLAMA achieves state-of-the-art performance with 4.91% absolution
improvement in accuracy. Compared with ICL, our inference speed can be 10 times
faster under 5-shot setting.

</details>


### [29] [Reinforcement Learning Fine-Tuning of Language Model for Instruction Following and Math Reasoning](https://arxiv.org/abs/2506.21560)
*Yifu Han,Geo Zhang*

Main category: cs.CL

TL;DR: 研究了在小型语言模型（Qwen2.5-0.5B Base）上使用SFT、DPO和RLOO等RL微调技术，用于指令遵循和数学推理任务。结果显示RLOO+DeBERTa表现最佳，DPO表现强劲，数学任务受益于合成数据和外部验证器。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在探讨强化学习（RL）微调技术在紧凑型语言模型（Qwen2.5-0.5B Base）上对指令遵循和数学推理这两个挑战性任务的有效性。

Method: 本研究比较了监督微调（SFT）、使用偏好标记数据的直接偏好优化（DPO）和使用奖励模型的强化学习留一法（RLOO）。对于数学推理任务，还结合了合成数据增强和使用外部验证器的最佳-N采样。

Result: 实验表明，使用DeBERTa奖励模型的RLOO实现了最佳对齐，而DPO提供了强大且一致的结果。对于数学推理任务，合成数据增强和使用外部验证器的最佳-N采样显著提高了准确性。

Conclusion: 本研究强调了训练轻量级、任务对齐的小型语言模型的关键权衡和实用策略。

Abstract: This study investigates the effectiveness of reinforcement learning (RL)
fine-tuning techniques on a compact language model (Qwen2.5-0.5B Base) for two
challenging tasks: instruction following and mathematical reasoning. We compare
supervised fine-tuning (SFT), Direct Preference Optimization (DPO) using
preference-labeled data, and Reinforce Leave-One-Out (RLOO) with reward models.
Our experiments show that RLOO with DeBERTa reward modeling achieves the best
alignment, while DPO provides strong and consistent results. For math reasoing
tasks, synthetic data augmentation and best-of-N sampling with an external
verifier significantly improve accuracy, showing the potential of combining
fine-tuning with inference-time tools. This study highlights key trade-offs and
practical strategies for training lightweight, task-aligned small-scale
language models.

</details>


### [30] [Reasoning Isn't Enough: Examining Truth-Bias and Sycophancy in LLMs](https://arxiv.org/abs/2506.21561)
*Emilio Barkett,Olivia Long,Madhavendra Thakur*

Main category: cs.CL

TL;DR: 研究评估了LLM的真相检测能力。发现推理模型虽然真相偏见较低，但仍高于人类。某些先进模型存在“谄媚”问题，表明仅凭能力提升无法解决LLM的真相检测挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在事实核查和决策中广泛使用，但其作为真相判断者的能力仍不清楚。

Method: 对八个LLM模型进行了4800次真实性判断评估，比较了推理模型和非推理模型。

Result: 推理模型的真相偏见低于非推理模型，但仍高于人类基准。部分先进模型（o4-mini、GPT-4.1、R1）表现出谄媚倾向，即对真话检测准确，但对谎言检测不准确。

Conclusion: LLM能力提升本身并不能解决其在真实性检测方面的根本挑战。

Abstract: Despite their widespread use in fact-checking, moderation, and high-stakes
decision-making, large language models (LLMs) remain poorly understood as
judges of truth. This study presents the largest evaluation to date of LLMs'
veracity detection capabilities and the first analysis of these capabilities in
reasoning models. We had eight LLMs make 4,800 veracity judgments across
several prompts, comparing reasoning and non-reasoning models. We find that
rates of truth-bias, or the likelihood to believe a statement is true,
regardless of whether it is actually true, are lower in reasoning models than
in non-reasoning models, but still higher than human benchmarks. Most
concerning, we identify sycophantic tendencies in several advanced models
(o4-mini and GPT-4.1 from OpenAI, R1 from DeepSeek), which displayed an
asymmetry in detection accuracy, performing well in truth accuracy but poorly
in deception accuracy. This suggests that capability advances alone do not
resolve fundamental veracity detection challenges in LLMs.

</details>


### [31] [FormosanBench: Benchmarking Low-Resource Austronesian Languages in the Era of Large Language Models](https://arxiv.org/abs/2506.21563)
*Kaiying Kevin Lin,Hsiyu Chen,Haopeng Zhang*

Main category: cs.CL

TL;DR: 大型语言模型在濒危的福尔摩沙语言上表现不佳；新的基准测试FORMOSANBENCH揭示了巨大的性能差距，凸显了对低资源语言更好NLP技术的迫切需求。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在低资源和少数民族语言方面的能力尚未得到充分探索，特别是濒危的福尔摩沙语言。

Method: 引入了FORMOSANBENCH，这是第一个用于评估大型语言模型在低资源南岛语上的基准测试。它涵盖了三种濒危的福尔摩沙语言（Atayal、Amis和Paiwan）以及三种核心NLP任务（机器翻译、自动语音识别和文本摘要）。研究评估了零样本、10样本和微调设置下的模型性能。

Result: 高资源语言与福尔摩沙语言之间存在显著的性能差距。现有的大型语言模型在所有任务中表现不佳，10样本学习和微调只提供了有限的改进。

Conclusion: 研究结果强调了对更具包容性的自然语言处理技术的需求，这些技术能够有效支持濒危和代表性不足的语言。

Abstract: While large language models (LLMs) have demonstrated impressive performance
across a wide range of natural language processing (NLP) tasks in high-resource
languages, their capabilities in low-resource and minority languages remain
significantly underexplored. Formosan languages -- a subgroup of Austronesian
languages spoken in Taiwan -- are both linguistically rich and endangered,
largely due to the sociolinguistic dominance of Mandarin. In this work, we
introduce FORMOSANBENCH, the first benchmark for evaluating LLMs on
low-resource Austronesian languages. It covers three endangered Formosan
languages: Atayal, Amis, and Paiwan, across three core NLP tasks: machine
translation, automatic speech recognition (ASR), and text summarization. We
assess model performance in zero-shot, 10-shot, and fine-tuned settings using
FORMOSANBENCH. Our results reveal a substantial performance gap between
high-resource and Formosan languages. Existing LLMs consistently underperform
across all tasks, with 10-shot learning and fine-tuning offering only limited
improvements. These findings underscore the urgent need for more inclusive NLP
technologies that can effectively support endangered and underrepresented
languages. We release our datasets and code to facilitate future research in
this direction.

</details>


### [32] [Team QUST at SemEval-2025 Task 10: Evaluating Large Language Models in Multiclass Multi-label Classification of News Entity Framing](https://arxiv.org/abs/2506.21564)
*Jiyan Liu,Youzheng Liu,Taihang Wang,Xiaoman Xu,Yimin Wang,Ye Jiang*

Main category: cs.CL

TL;DR: 提出一个三阶段检索框架用于事实核查声明检索，在SemEval-2025 Task 7中分别获得单语赛道第5名和跨语言赛道第7名。


<details>
  <summary>Details</summary>
Motivation: 描述QUST_NLP在SemEval-2025 Task 7中的参与，并提出一个专门为事实核查声明检索设计的三阶段检索框架。

Method: 一个三阶段检索框架：首先，评估并选择最佳的检索模型进行候选检索；其次，采用多个重排序模型（每个选择Top-10结果）；最后，使用加权投票确定最终检索结果。

Result: 在单语赛道获得第5名，在跨语言赛道获得第7名。

Conclusion: 本文提出的三阶段检索框架在事实核查声明检索方面表现出色，并在SemEval-2025 Task 7中取得了有竞争力的成绩。

Abstract: This paper describes the participation of QUST_NLP in the SemEval-2025 Task
7. We propose a three-stage retrieval framework specifically designed for
fact-checked claim retrieval. Initially, we evaluate the performance of several
retrieval models and select the one that yields the best results for candidate
retrieval. Next, we employ multiple re-ranking models to enhance the candidate
results, with each model selecting the Top-10 outcomes. In the final stage, we
utilize weighted voting to determine the final retrieval outcomes. Our approach
achieved 5th place in the monolingual track and 7th place in the crosslingual
track. We release our system code at:
https://github.com/warmth27/SemEval2025_Task7.

</details>


### [33] [A Multi-Agent Probabilistic Inference Framework Inspired by Kairanban-Style CoT System with IdoBata Conversation for Debiasing](https://arxiv.org/abs/2506.21565)
*Takato Ueno,Keito Inoshita*

Main category: cs.CL

TL;DR: 受日本传统交流方式启发，本研究提出了一个多LLM框架（KCS+IBC），通过整合多个LLM和引入对话会话，在情感分析中实现了偏见缓解、可解释性提升和概率预测，并能在聚合性和多样性之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 受日本回览板文化和井户端会议（Kairanban culture and Idobata conversations）的启发，本研究旨在通过信息交换过程，在情感分析中实现偏见缓解、提高可解释性和概率预测。

Method: 本研究提出了一个多智能体推理框架 (KCS+IBC)，该框架集成了多个大型语言模型 (LLM)，并通过引入中期随意对话会话和概率情感预测来融合正式推理和个人视角。

Result: 实验结果表明，KCS 在不同数据集上实现了与单一 LLM 相当的准确性，而 KCS+IBC 在推理后期表现出熵的持续下降和方差的逐渐增加，这表明该框架能够平衡预测的聚合性和多样性。

Conclusion: 该框架能够平衡预测的聚合性和多样性。

Abstract: Japan's kairanban culture and idobata conversations have long functioned as
traditional communication practices that foster nuanced dialogue among
community members and contribute to the formation of social balance. Inspired
by these information exchange processes, this study proposes a multi-agent
inference framework (KCS+IBC) that integrates multiple large language models
(LLMs) to achieve bias mitigation, improved explainability, and probabilistic
prediction in sentiment analysis. In addition to sequentially sharing
prediction results, the proposed method incorporates a mid-phase casual
dialogue session to blend formal inference with individual perspectives and
introduces probabilistic sentiment prediction. Experimental results show that
KCS achieves accuracy comparable to that of a single LLM across datasets, while
KCS+IBC exhibits a consistent decrease in entropy and a gradual increase in
variance during the latter stages of inference, suggesting the framework's
ability to balance aggregation and diversity of predictions. Future work will
quantitatively assess the impact of these characteristics on bias correction
and aim to develop more advanced sentiment analysis systems.

</details>


### [34] [Efficient Multilingual ASR Finetuning via LoRA Language Experts](https://arxiv.org/abs/2506.21555)
*Jiahong Li,Yiwen Shao,Jianheng Zhuo,Chenda Li,Liliang Tang,Dong Yu,Yanmin Qian*

Main category: cs.CL

TL;DR: 本论文提出了一种基于LoRA的微调框架，通过LoRA专家融合或知识蒸馏，有效解决了多语言ASR中的语言干扰问题，并在多种场景下取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在多语言ASR方面取得了显著进展，但多语言ASR仍受限于“多语言诅咒”，即不同语言间相互干扰，使得ASR模型在共享模型容量的同时难以有效识别多种语言。

Method: 论文提出了一种基于Whisper的、通过LoRA语言专家进行定制化多语言ASR的有效微调框架。该方法通过LoRA专家融合或知识蒸馏实现。

Result: 该方法在目标语言上取得了比标准微调方法更好的识别性能。实验结果表明，在语言感知和语言无关场景下，所提出的模型分别获得了大约10%和15%的相对性能提升。

Conclusion: 本文提出的基于LoRA的微调框架有效缓解了多语言干扰，并显著提高了多语言自动语音识别（ASR）的性能。

Abstract: Recent advancements in deep learning have significantly enhanced multilingual
automatic speech recognition (ASR) due to the development of advanced model
architectures and available large-scale multilingual datasets. Despite that,
multilingual ASR still suffers from the curse of multilinguality in that
different languages tend to interfere with each other, making it difficult for
the ASR model to identify multiple languages effectively while sharing model
capacity across them. This paper proposes an efficient finetuning framework for
customized multilingual ASR via prepared LoRA language experts based on
Whisper. Through LoRA expert fusion or knowledge distillation, our approach
achieves better recognition performance on target languages than standard
fine-tuning methods. Experimental results demonstrate that the proposed models
yield approximately 10\% and 15\% relative performance gains in language-aware
and language-agnostic scenarios, respectively.

</details>


### [35] [Efficient Multilingual ASR Finetuning via LoRA Language Experts](https://arxiv.org/abs/2506.21555)
*Jiahong Li,Yiwen Shao,Jianheng Zhuo,Chenda Li,Liliang Tang,Dong Yu,Yanmin Qian*

Main category: cs.CL

TL;DR: 该论文提出了一种基于Whisper，利用LoRA语言专家进行微调的多语言ASR框架，通过专家融合或知识蒸馏克服了多语言干扰问题，并在目标语言上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在多语言自动语音识别（ASR）方面取得了显著进展，但多语言ASR仍受困于“多语言诅咒”，即不同语言之间相互干扰，使得ASR模型难以有效识别多种语言并共享模型容量。

Method: 本文提出了一种基于Whisper，通过准备好的LoRA语言专家进行定制化多语言ASR的有效微调框架。该方法通过LoRA专家融合或知识蒸馏实现。

Result: 实验结果表明，所提出的模型在语言感知和语言无关场景下分别带来了大约10%和15%的相对性能提升。

Conclusion: 该方法通过LoRA语言专家融合或知识蒸馏，在目标语言上实现了比标准微调方法更好的识别性能，并在语言感知和语言无关场景下分别获得了约10%和15%的相对性能提升。

Abstract: Recent advancements in deep learning have significantly enhanced multilingual
automatic speech recognition (ASR) due to the development of advanced model
architectures and available large-scale multilingual datasets. Despite that,
multilingual ASR still suffers from the curse of multilinguality in that
different languages tend to interfere with each other, making it difficult for
the ASR model to identify multiple languages effectively while sharing model
capacity across them. This paper proposes an efficient finetuning framework for
customized multilingual ASR via prepared LoRA language experts based on
Whisper. Through LoRA expert fusion or knowledge distillation, our approach
achieves better recognition performance on target languages than standard
fine-tuning methods. Experimental results demonstrate that the proposed models
yield approximately 10\% and 15\% relative performance gains in language-aware
and language-agnostic scenarios, respectively.

</details>


### [36] [VAT-KG: Knowledge-Intensive Multimodal Knowledge Graph Dataset for Retrieval-Augmented Generation](https://arxiv.org/abs/2506.21556)
*Hyeongcheol Park,MinHyuk Jang,Ha Dam Baek,Gyusam Chang,Jiyoung Seo,Jiwan Park,Hogun Park,Sangpil Kim*

Main category: cs.CL

TL;DR: 该论文提出了VAT-KG，首个包含视觉、音频和文本信息的多模态知识图谱，旨在解决现有MMKGs的局限性。它通过创新的构建流程实现跨模态知识对齐，并引入了多模态RAG框架。实验证明其在支持多模态MLLMs方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有多模态知识图谱（MMKGs）范围有限，通常通过增强现有知识图谱构建，导致知识过时或不完整，且仅支持狭窄的模态（如文本和视觉）。这限制了它们的扩展性和在多模态任务中的适用性，尤其是在MLLMs转向更丰富的模态（如视频和音频）的背景下。

Method: 提出了视觉-音频-文本知识图谱（VAT-KG），这是一个以概念为中心、知识密集的多模态知识图谱，涵盖视觉、音频和文本信息。其构建管道通过一系列严格的过滤和对齐步骤，确保多模态数据与细粒度语义之间的跨模态知识对齐，从而能够从任何多模态数据集中自动生成多模态知识图谱。此外，还引入了一个新的多模态RAG框架，用于响应任意模态的查询，检索详细的概念级知识。

Result: 在跨模态问答任务上的实验证明了VAT-KG在支持MLLMs方面的有效性。

Conclusion: VAT-KG在统一和利用多模态知识方面具有实际价值。

Abstract: Multimodal Knowledge Graphs (MMKGs), which represent explicit knowledge
across multiple modalities, play a pivotal role by complementing the implicit
knowledge of Multimodal Large Language Models (MLLMs) and enabling more
grounded reasoning via Retrieval Augmented Generation (RAG). However, existing
MMKGs are generally limited in scope: they are often constructed by augmenting
pre-existing knowledge graphs, which restricts their knowledge, resulting in
outdated or incomplete knowledge coverage, and they often support only a narrow
range of modalities, such as text and visual information. These limitations
reduce their extensibility and applicability to a broad range of multimodal
tasks, particularly as the field shifts toward richer modalities such as video
and audio in recent MLLMs. Therefore, we propose the Visual-Audio-Text
Knowledge Graph (VAT-KG), the first concept-centric and knowledge-intensive
multimodal knowledge graph that covers visual, audio, and text information,
where each triplet is linked to multimodal data and enriched with detailed
descriptions of concepts. Specifically, our construction pipeline ensures
cross-modal knowledge alignment between multimodal data and fine-grained
semantics through a series of stringent filtering and alignment steps, enabling
the automatic generation of MMKGs from any multimodal dataset. We further
introduce a novel multimodal RAG framework that retrieves detailed
concept-level knowledge in response to queries from arbitrary modalities.
Experiments on question answering tasks across various modalities demonstrate
the effectiveness of VAT-KG in supporting MLLMs, highlighting its practical
value in unifying and leveraging multimodal knowledge.

</details>


### [37] [Debunk and Infer: Multimodal Fake News Detection via Diffusion-Generated Evidence and LLM Reasoning](https://arxiv.org/abs/2506.21557)
*Kaiying Yan,Moyang Liu,Yukun Liu,Ruibo Fu,Zhengqi Wen,Jianhua Tao,Xuefei Liu*

Main category: cs.CL

TL;DR: DIFND是一个利用揭穿知识、条件扩散模型和多模态大语言模型来检测假新闻的框架，通过生成证据和链式推理，显著提高了检测准确性和可信度。


<details>
  <summary>Details</summary>
Motivation: 假新闻在多媒体平台上的迅速传播对信息可信度构成了严重挑战。

Method: 提出了一个Debunk-and-Infer框架（DIFND），它结合了条件扩散模型的生成能力和多模态大型语言模型（MLLMs）的协作推理能力。具体而言，采用揭穿扩散来根据新闻视频的多模态内容生成反驳或验证证据；并提出了一种“揭穿链”策略，其中多智能体MLLM系统产生逻辑接地、多模态感知的推理内容和最终的真实性判断。

Result: 在FakeSV和FVC数据集上的大量实验表明，DIFND不仅优于现有方法，而且提供了可信的决策。

Conclusion: DIFND框架通过联合建模多模态特征、生成性揭穿线索和富含推理的验证，显著提高了检测准确性，并且比现有方法更出色，提供值得信赖的决策，增强了性能和可解释性。

Abstract: The rapid spread of fake news across multimedia platforms presents serious
challenges to information credibility. In this paper, we propose a
Debunk-and-Infer framework for Fake News Detection(DIFND) that leverages
debunking knowledge to enhance both the performance and interpretability of
fake news detection. DIFND integrates the generative strength of conditional
diffusion models with the collaborative reasoning capabilities of multimodal
large language models (MLLMs). Specifically, debunk diffusion is employed to
generate refuting or authenticating evidence based on the multimodal content of
news videos, enriching the evaluation process with diverse yet semantically
aligned synthetic samples. To improve inference, we propose a chain-of-debunk
strategy where a multi-agent MLLM system produces logic-grounded,
multimodal-aware reasoning content and final veracity judgment. By jointly
modeling multimodal features, generative debunking cues, and reasoning-rich
verification within a unified architecture, DIFND achieves notable improvements
in detection accuracy. Extensive experiments on the FakeSV and FVC datasets
show that DIFND not only outperforms existing approaches but also delivers
trustworthy decisions.

</details>


<div id='cs.CV'></div>

# cs.CV [[Back]](#toc)

### [38] [Fine-Grained Preference Optimization Improves Spatial Reasoning in VLMs](https://arxiv.org/abs/2506.21656)
*Yifan Shen,Yuanzhe Liu,Jingyuan Zhu,Xu Cao,Xiaofeng Zhang,Yixiao He,Wenming Ye,James Matthew Rehg,Ismini Lourentzou*

Main category: cs.CV

TL;DR: 新的视觉-语言模型SpatialReasoner-R1，结合M3CTS和fDPO，显著提升了细粒度空间推理能力，并在SPATIALRGPT-Bench上取得了SOTA。


<details>
  <summary>Details</summary>
Motivation: 当前的视觉-语言模型（VLMs）在细粒度空间推理方面表现不佳，尤其是在需要多步逻辑和精确空间对齐的场景。

Method: 本文提出了SpatialReasoner-R1视觉-语言推理模型。为构建高质量监督，设计了多模型蒙特卡洛树搜索（M3CTS）方法生成长链式思维推理轨迹。此外，提出了细粒度直接偏好优化（fDPO），引入了段特定偏好粒度，并通过空间奖励机制评估候选响应。

Result: fDPO在空间质量任务中比标准DPO平均提高了4.1%，在空间数量任务中提高了9.0%。SpatialReasoner-R1在SPATIALRGPT-Bench上创造了新的SOTA，平均准确率比最强基线高出9.8%，同时在通用视觉-语言任务上保持了竞争力。

Conclusion: SpatialReasoner-R1模型通过引入M3CTS和fDPO，有效解决了现有视觉-语言模型在细粒度空间推理方面的不足，并在SPATIALRGPT-Bench上达到了新的SOTA。

Abstract: Current Vision-Language Models (VLMs) struggle with fine-grained spatial
reasoning, particularly when multi-step logic and precise spatial alignment are
required. In this work, we introduce SpatialReasoner-R1, a vision-language
reasoning model designed to address these limitations. To construct
high-quality supervision for spatial reasoning, we design a Multi-Model Monte
Carlo Tree Search (M3CTS) method that generates diverse, logically consistent
Long Chain-of-Thought (LongCoT) reasoning trajectories. In addition, we propose
fine-grained Direct Preference Optimization (fDPO), which introduces
segment-specific preference granularity for descriptive grounding and logical
reasoning, guided by a spatial reward mechanism that evaluates candidate
responses based on visual consistency, spatial grounding, and logical
coherence. Experimental results demonstrate that fDPO achieves an average
improvement of 4.1% over standard DPO across spatial quality tasks, and a 9.0%
gain in spatial quantity tasks. SpatialReasoner-R1, trained with fDPO, sets a
new SoTA on SPATIALRGPT-Bench, outperforming the strongest baseline by 9.8% in
average accuracy, while maintaining competitive performance on general
vision-language tasks.

</details>


### [39] [TanDiT: Tangent-Plane Diffusion Transformer for High-Quality 360° Panorama Generation](https://arxiv.org/abs/2506.21681)
*Hakan Çapuk,Andrew Bond,Muhammed Burak Kızıl,Emir Göçen,Erkut Erdem,Aykut Erdem*

Main category: cs.CV

TL;DR: TanDiT是一种新的全景图像生成方法，通过生成切平面图像网格并利用统一扩散模型解决现有模型在全景图生成中的挑战，同时提出了新的评估指标。实验证明其能生成高质量、多样化的全景图像。


<details>
  <summary>Details</summary>
Motivation: 现有的图像生成模型在处理全景图像生成时仍面临挑战，包括几何畸变程度不同和无缝循环一致性要求。

Method: 引入TanDiT，通过生成覆盖整个360度视图的切平面图像网格来合成全景场景。TanDiT利用统一的扩散模型在单次去噪迭代中同时生成这些切平面图像。此外，还提出了一个模型无关的后处理步骤，旨在增强生成全景图的全局一致性。为了准确评估全景图像质量，还提出了TangentIS和TangentFID两种专用度量标准，并提供了包含带字幕全景数据集和标准化评估脚本的综合基准。

Result: TanDiT方法能够有效泛化到训练数据之外，鲁棒地解释详细复杂的文本提示，并与各种生成模型无缝集成，生成高质量、多样化的全景图像。

Conclusion: TanDiT方法能够有效泛化到训练数据之外，鲁棒地解释详细复杂的文本提示，并与各种生成模型无缝集成，生成高质量、多样化的全景图像。

Abstract: Recent advances in image generation have led to remarkable improvements in
synthesizing perspective images. However, these models still struggle with
panoramic image generation due to unique challenges, including varying levels
of geometric distortion and the requirement for seamless loop-consistency. To
address these issues while leveraging the strengths of the existing models, we
introduce TanDiT, a method that synthesizes panoramic scenes by generating
grids of tangent-plane images covering the entire 360$^\circ$ view. Unlike
previous methods relying on multiple diffusion branches, TanDiT utilizes a
unified diffusion model trained to produce these tangent-plane images
simultaneously within a single denoising iteration. Furthermore, we propose a
model-agnostic post-processing step specifically designed to enhance global
coherence across the generated panoramas. To accurately assess panoramic image
quality, we also present two specialized metrics, TangentIS and TangentFID, and
provide a comprehensive benchmark comprising captioned panoramic datasets and
standardized evaluation scripts. Extensive experiments demonstrate that our
method generalizes effectively beyond its training data, robustly interprets
detailed and complex text prompts, and seamlessly integrates with various
generative models to yield high-quality, diverse panoramic images.

</details>


### [40] [FOCUS: Internal MLLM Representations for Efficient Fine-Grained Visual Question Answering](https://arxiv.org/abs/2506.21710)
*Liangyu Zhong,Fabio Rosenthal,Joachim Sicking,Fabian Hüger,Thorsten Bagdonat,Hanno Gottschalk,Leo Schwinn*

Main category: cs.CV

TL;DR: FOCUS是一种免训练的视觉裁剪方法，利用MLLM内部表示来查找最相关的图像区域，从而解决细粒度视觉问答（VQA）的挑战，并在准确性和效率上都表现出色。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在图像-文本输入方面表现出强大的感知和推理能力，但专注于小图像细节的视觉问答（VQA）仍然是一个挑战。现有的视觉裁剪技术存在局限性，例如需要针对特定任务进行微调、由于非知情的穷举搜索导致效率低下，或与高效的注意力实现不兼容。

Method: FOCUS是一种免训练的视觉裁剪方法，利用多模态大语言模型（MLLM）的内部表示来指导最相关图像区域的搜索。具体分四步：1. 识别VQA提示中的目标对象；2. 使用键值（KV）缓存计算对象相关性图；3. 基于相关性图提出并排序相关图像区域；4. 使用排名最高的区域执行细粒度VQA任务。

Result: FOCUS在四个细粒度VQA数据集和两种MLLM上都取得了强大性能。它在准确性和效率上都优于三种流行的视觉裁剪方法，并且与表现最佳的基线ZoomEye相匹配，同时计算量减少了3到6.5倍。

Conclusion: FOCUS是一种有效且高效的免训练视觉裁剪方法，解决了现有方法在细粒度VQA任务上的局限性。

Abstract: While Multimodal Large Language Models (MLLMs) offer strong perception and
reasoning capabilities for image-text input, Visual Question Answering (VQA)
focusing on small image details still remains a challenge. Although visual
cropping techniques seem promising, recent approaches have several limitations:
the need for task-specific fine-tuning, low efficiency due to uninformed
exhaustive search, or incompatibility with efficient attention implementations.
We address these shortcomings by proposing a training-free visual cropping
method, dubbed FOCUS, that leverages MLLM-internal representations to guide the
search for the most relevant image region. This is accomplished in four steps:
first, we identify the target object(s) in the VQA prompt; second, we compute
an object relevance map using the key-value (KV) cache; third, we propose and
rank relevant image regions based on the map; and finally, we perform the
fine-grained VQA task using the top-ranked region. As a result of this informed
search strategy, FOCUS achieves strong performance across four fine-grained VQA
datasets and two types of MLLMs. It outperforms three popular visual cropping
methods in both accuracy and efficiency, and matches the best-performing
baseline, ZoomEye, while requiring 3 - 6.5 x less compute.

</details>


### [41] [CAST: Cross-Attentive Spatio-Temporal feature fusion for Deepfake detection](https://arxiv.org/abs/2506.21711)
*Aryan Thakre,Omkar Nagwekar,Vedang Talekar,Aparna Santra Biswas*

Main category: cs.CV

TL;DR: 该论文提出一种基于交叉注意力的CAST模型，用于深度伪造视频检测，通过有效融合时空特征，显著提升了检测性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 深度伪造对数字媒体真实性构成威胁，现有CNN-Transformer模型独立处理空间和时间特征，且注意力机制简单结合，限制了时空交互深度。

Method: 提出了一种统一的CAST模型，利用交叉注意力机制更集成地融合空间和时间特征，使时间特征能动态关注相关的空间区域。

Result: 模型在数据集内评估中实现99.49%的AUC和97.57%的准确率；在跨数据集测试中，对未见过的DeepfakeDetection数据集取得93.31%的AUC，展现出良好的泛化能力。

Conclusion: 跨注意力机制的特征融合能有效提升深度伪造视频检测的鲁棒性。

Abstract: Deepfakes have emerged as a significant threat to digital media authenticity,
increasing the need for advanced detection techniques that can identify subtle
and time-dependent manipulations. CNNs are effective at capturing spatial
artifacts, and Transformers excel at modeling temporal inconsistencies.
However, many existing CNN-Transformer models process spatial and temporal
features independently. In particular, attention-based methods often use
separate attention mechanisms for spatial and temporal features and combine
them using naive approaches like averaging, addition, or concatenation, which
limits the depth of spatio-temporal interaction. To address this challenge, we
propose a unified CAST model that leverages cross-attention to effectively fuse
spatial and temporal features in a more integrated manner. Our approach allows
temporal features to dynamically attend to relevant spatial regions, enhancing
the model's ability to detect fine-grained, time-evolving artifacts such as
flickering eyes or warped lips. This design enables more precise localization
and deeper contextual understanding, leading to improved performance across
diverse and challenging scenarios. We evaluate the performance of our model
using the FaceForensics++, Celeb-DF, and DeepfakeDetection datasets in both
intra- and cross-dataset settings to affirm the superiority of our approach.
Our model achieves strong performance with an AUC of 99.49 percent and an
accuracy of 97.57 percent in intra-dataset evaluations. In cross-dataset
testing, it demonstrates impressive generalization by achieving a 93.31 percent
AUC on the unseen DeepfakeDetection dataset. These results highlight the
effectiveness of cross-attention-based feature fusion in enhancing the
robustness of deepfake video detection.

</details>


### [42] [Elucidating and Endowing the Diffusion Training Paradigm for General Image Restoration](https://arxiv.org/abs/2506.21722)
*Xin Lu,Xueyang Fu,Jie Xiao,Zihao Fan,Yurui Zhu,Zheng-Jun Zha*

Main category: cs.CV

TL;DR: 本文提出了一种新框架，将扩散训练集成到通用图像恢复网络中，提高了单任务的泛化能力和多任务的性能，并兼容现有架构。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像恢复任务中具有强大的生成能力，但其复杂架构和迭代过程限制了实际应用。现有方法主要关注优化网络架构和扩散路径，却忽视了将扩散训练范式整合到通用图像恢复框架中。

Method: 通过系统分析时间步依赖、网络层级、噪声水平关系和多恢复任务关联性，阐明了将扩散训练范式应用于通用图像恢复的关键原则；提出了一个由扩散训练支持的新图像恢复框架；引入了一系列正则化策略，使扩散目标与图像恢复任务对齐；开发了渐进式训练范式和特定任务适配器，以应对多任务统一图像恢复。

Result: 在单任务图像恢复中显著提高了图像恢复网络的泛化能力；在多任务统一图像恢复中取得了卓越性能；提出的框架可以无缝集成到现有通用图像恢复架构中。

Conclusion: 本文成功地将扩散模型训练范式整合到通用图像恢复框架中，显著提升了单任务泛化能力和多任务统一图像恢复性能，并且能无缝集成到现有架构中。

Abstract: While diffusion models demonstrate strong generative capabilities in image
restoration (IR) tasks, their complex architectures and iterative processes
limit their practical application compared to mainstream reconstruction-based
general ordinary IR networks. Existing approaches primarily focus on optimizing
network architecture and diffusion paths but overlook the integration of the
diffusion training paradigm within general ordinary IR frameworks. To address
these challenges, this paper elucidates key principles for adapting the
diffusion training paradigm to general IR training through systematic analysis
of time-step dependencies, network hierarchies, noise-level relationships, and
multi-restoration task correlations, proposing a new IR framework supported by
diffusion-based training. To enable IR networks to simultaneously restore
images and model generative representations, we introduce a series of
regularization strategies that align diffusion objectives with IR tasks,
improving generalization in single-task scenarios. Furthermore, recognizing
that diffusion-based generation exerts varying influences across different IR
tasks, we develop an incremental training paradigm and task-specific adaptors,
further enhancing performance in multi-task unified IR. Experiments demonstrate
that our method significantly improves the generalization of IR networks in
single-task IR and achieves superior performance in multi-task unified IR.
Notably, the proposed framework can be seamlessly integrated into existing
general IR architectures.

</details>


### [43] [ADNet: Leveraging Error-Bias Towards Normal Direction in Face Alignment](https://arxiv.org/abs/2109.05721)
*Yangyu Huang,Hao Yang,Chong Li,Jongyoo Kim,Fangyun Wei*

Main category: cs.CV

TL;DR: 该论文解决了人脸对齐中的误差偏差问题，提出了各向异性方向损失（ADL）和各向异性注意力模块（AAM），并整合为ADNet，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸对齐工作很少关注地标误差分布的误差偏差问题，这种误差偏差与模糊的地标标注任务密切相关。

Method: 提出了各向异性方向损失（ADL）和各向异性注意力模块（AAM），分别用于坐标和热图回归。ADL在法线方向施加强绑定力，而AAM在切线方向有更强的响应，这两种方法互补地学习面部结构和纹理细节，并被集成到ADNet中。

Result: ADNet在300W、WFLW和COFW数据集上达到了最先进的结果。

Conclusion: 本文提出的ADNet在300W、WFLW和COFW数据集上取得了最先进的结果，证明了其有效性和鲁棒性。

Abstract: The recent progress of CNN has dramatically improved face alignment
performance. However, few works have paid attention to the error-bias with
respect to error distribution of facial landmarks. In this paper, we
investigate the error-bias issue in face alignment, where the distributions of
landmark errors tend to spread along the tangent line to landmark curves. This
error-bias is not trivial since it is closely connected to the ambiguous
landmark labeling task. Inspired by this observation, we seek a way to leverage
the error-bias property for better convergence of CNN model. To this end, we
propose anisotropic direction loss (ADL) and anisotropic attention module (AAM)
for coordinate and heatmap regression, respectively. ADL imposes strong binding
force in normal direction for each landmark point on facial boundaries. On the
other hand, AAM is an attention module which can get anisotropic attention mask
focusing on the region of point and its local edge connected by adjacent
points, it has a stronger response in tangent than in normal, which means
relaxed constraints in the tangent. These two methods work in a complementary
manner to learn both facial structures and texture details. Finally, we
integrate them into an optimized end-to-end training pipeline named ADNet. Our
ADNet achieves state-of-the-art results on 300W, WFLW and COFW datasets, which
demonstrates the effectiveness and robustness.

</details>


### [44] [Asymmetric Dual Self-Distillation for 3D Self-Supervised Representation Learning](https://arxiv.org/abs/2506.21724)
*Remco F. Leijenaar,Hamidreza Kasaei*

Main category: cs.CV

TL;DR: AsymDSD是一个新的自监督框架，通过统一掩码建模和不变性学习，从非结构化3D点云中学习语义表示，并在ScanObjectNN数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 从非结构化3D点云中学习语义上有意义的表示仍然是计算机视觉中的一个核心挑战，尤其是在缺乏大规模标记数据集的情况下。现有的掩码点建模（MPM）方法其基于重建的目标限制了其捕捉高级语义的能力。

Method: 本文提出了AsymDSD，一个非对称双重自蒸馏框架，它通过在潜在空间而非输入空间进行预测来统一掩码建模和不变性学习。该方法包含高效的非对称设置、禁用掩码查询之间的注意力以防止形状泄露、多掩码采样以及多裁剪的点云适应。

Result: AsymDSD在ScanObjectNN上取得了90.53%的最先进结果，并且在93万个形状上进行预训练后进一步提高到93.72%，超过了现有方法。

Conclusion: AsymDSD通过在潜在空间而非输入空间进行预测，有效地统一了掩码建模和不变性学习，从而解决了MPM在捕捉高级语义方面的局限性，并在3D点云表示学习方面取得了最先进的成果。

Abstract: Learning semantically meaningful representations from unstructured 3D point
clouds remains a central challenge in computer vision, especially in the
absence of large-scale labeled datasets. While masked point modeling (MPM) is
widely used in self-supervised 3D learning, its reconstruction-based objective
can limit its ability to capture high-level semantics. We propose AsymDSD, an
Asymmetric Dual Self-Distillation framework that unifies masked modeling and
invariance learning through prediction in the latent space rather than the
input space. AsymDSD builds on a joint embedding architecture and introduces
several key design choices: an efficient asymmetric setup, disabling attention
between masked queries to prevent shape leakage, multi-mask sampling, and a
point cloud adaptation of multi-crop. AsymDSD achieves state-of-the-art results
on ScanObjectNN (90.53%) and further improves to 93.72% when pretrained on 930k
shapes, surpassing prior methods.

</details>


### [45] [FreeEnricher: Enriching Face Landmarks without Additional Cost](https://arxiv.org/abs/2212.09525)
*Yangyu Huang,Xi Chen,Jongyoo Kim,Hao Yang,Chong Li,Jiaolong Yang,Dong Chen*

Main category: cs.CV

TL;DR: 提出一种弱监督框架，通过利用稀疏数据集来丰富人脸标志密度，在多个测试集上无需额外成本便取得了最先进的密集和稀疏人脸对齐效果。


<details>
  <summary>Details</summary>
Motivation: 尽管在美容医学和面部美化等多种场景中，对密集面部标志的需求很高，但大多数工作只考虑稀疏面部对齐。

Method: 提出一个框架，通过现有稀疏标志数据集来丰富标志密度；观察到每个语义轮廓上的局部块在外观上高度相似；提出一种弱监督思想，学习在原始稀疏标志上进行细化的能力，并将这种能力应用于丰富的密集标志；设计并组织了多个操作符来实施这一思想；训练好的模型作为即插即用模块应用于现有的人脸对齐网络。

Result: 在新构建的密集300W测试集以及原始稀疏300W和WFLW测试集上，无需额外成本，本方法均取得了最先进的准确性。

Conclusion: 本文提出的方法能够有效地在现有稀疏人脸标志数据集的基础上生成密集标志，并在多个基准测试中展现出卓越的性能，可作为现有网络的即插即用模块，具有广泛的应用前景。

Abstract: Recent years have witnessed significant growth of face alignment. Though
dense facial landmark is highly demanded in various scenarios, e.g., cosmetic
medicine and facial beautification, most works only consider sparse face
alignment. To address this problem, we present a framework that can enrich
landmark density by existing sparse landmark datasets, e.g., 300W with 68
points and WFLW with 98 points. Firstly, we observe that the local patches
along each semantic contour are highly similar in appearance. Then, we propose
a weakly-supervised idea of learning the refinement ability on original sparse
landmarks and adapting this ability to enriched dense landmarks. Meanwhile,
several operators are devised and organized together to implement the idea.
Finally, the trained model is applied as a plug-and-play module to the existing
face alignment networks. To evaluate our method, we manually label the dense
landmarks on 300W testset. Our method yields state-of-the-art accuracy not only
in newly-constructed dense 300W testset but also in the original sparse 300W
and WFLW testsets without additional cost.

</details>


### [46] [Exploring Image Generation via Mutually Exclusive Probability Spaces and Local Correlation Hypothesis](https://arxiv.org/abs/2506.21731)
*Chenqiu Zhao,Anup Basu*

Main category: cs.CV

TL;DR: 本文提出了MESP和LCH框架，以探讨生成模型中记忆而非生成的问题。MESP导致了BL-AE和ARVM模型，后者在FID分数上表现出色但被认为是记忆化。LCH提出生成能力源于局部相关性。


<details>
  <summary>Details</summary>
Motivation: 探索概率生成模型中存在的潜在局限性，即学习全局分布可能导致模型记忆数据而非产生真正的新数据。这源于对变分自编码器（VAE）中潜在变量分布重叠的观察，这导致了重建损失和KL散度损失之间的优化冲突。

Method: 提出了互斥概率空间（MESP）和局部相关假设（LCH）两个理论框架。基于MESP，提出了二值潜在自编码器（BL-AE）将图像编码为二值潜在表示，并在此基础上提出自回归随机变量模型（ARVM），该模型输出直方图。LCH旨在解决生成模型中记忆而非生成的问题，并认为生成能力源于潜在变量的局部相关性。

Result: 自回归随机变量模型（ARVM）取得了具有竞争力的FID分数，在标准数据集上优于现有最先进的方法。然而，研究指出这些分数反映的是模型的记忆能力而非生成能力。全面的实验和讨论验证了所提出的框架。

Conclusion: 本文提出了MESP和LCH两个理论框架，以探讨概率生成模型中学习全局分布可能导致记忆而非生成行为的局限性。MESP从对VAE的重新思考中产生，并引出了BL-AE和ARVM模型，尽管ARVM取得了良好的FID分数，但作者指出这反映的是记忆而非真正的生成。LCH则提出生成能力源于潜在变量之间的局部相关性。通过实验验证了这些框架。

Abstract: We propose two theoretical frameworks, the Mutually Exclusive Probability
Space (MESP) and the Local Correlation Hypothesis (LCH), to explore a potential
limitation in probabilistic generative models; namely that learning global
distributions leads to memorization rather than generative behavior. MESP
emerges from our rethinking of the Variational Autoencoder (VAE). We observe
that latent variable distributions in VAE exhibit overlap, which leads to an
optimization conflict between the reconstruction loss and KL-divergence loss. A
lower bound based on the overlap coefficient is proposed. We refer to this
phenomenon as Mutually Exclusive Probability Spaces. Based on MESP, a Binary
Latent Autoencoder (BL-AE) is proposed to encode images into binary latent
representations. These binary latents are used as the input to our
Autoregressive Random Variable Model (ARVM), a modified autoregressive model
outputting histograms. Our ARVM achieves competitive FID scores, outperforming
state-of-the-art methods on standard datasets. However, such scores reflect
memorization rather than generation. To address this issue, we propose the
Local Correlation Hypothesis (LCH), which posits that generative capability
arising from local correlations among latent variables. Comprehensive
experiments and discussions are conducted to validate our frameworks.

</details>


### [47] [Equitable Federated Learning with NCA](https://arxiv.org/abs/2506.21735)
*Nick Lemke,Mirko Konstantin,Henry John Krumb,John Kalkhof,Jonathan Stieber,Anirban Mukhopadhyay*

Main category: cs.CV

TL;DR: FedNCA是一种新型联邦学习系统，专为解决低收入和中等收入国家（LMICs）医疗影像分割中面临的资源和连接挑战而设计。它利用轻量级架构，可在低成本设备上运行，降低通信成本，并支持加密。


<details>
  <summary>Details</summary>
Motivation: 联邦学习（FL）在低收入和中等收入国家（LMICs）中非常有价值，因为这些地区受过培训的医疗专业人员有限，但FL在LMICs的推广面临显著障碍，包括高性能计算资源有限和互联网连接不可靠。

Method: 我们引入了FedNCA，这是一种专为医学图像分割任务量身定制的新型联邦学习系统。FedNCA利用轻量级Med-NCA架构，可在智能手机等低成本边缘设备上进行训练，同时最大限度地降低通信成本。此外，支持加密的FedNCA被证明适用于受损的网络通信。

Result: FedNCA克服了基础设施和安全挑战。

Conclusion: FedNCA为资源受限地区的医疗影像解决方案铺平了道路，实现了普惠、高效、轻量级和加密就绪的医疗影像解决方案，从而促进了公平的医疗保健进步。

Abstract: Federated Learning (FL) is enabling collaborative model training across
institutions without sharing sensitive patient data. This approach is
particularly valuable in low- and middle-income countries (LMICs), where access
to trained medical professionals is limited. However, FL adoption in LMICs
faces significant barriers, including limited high-performance computing
resources and unreliable internet connectivity. To address these challenges, we
introduce FedNCA, a novel FL system tailored for medical image segmentation
tasks. FedNCA leverages the lightweight Med-NCA architecture, enabling training
on low-cost edge devices, such as widely available smartphones, while
minimizing communication costs. Additionally, our encryption-ready FedNCA
proves to be suitable for compromised network communication. By overcoming
infrastructural and security challenges, FedNCA paves the way for inclusive,
efficient, lightweight, and encryption-ready medical imaging solutions,
fostering equitable healthcare advancements in resource-constrained regions.

</details>


### [48] [ImplicitQA: Going beyond frames towards Implicit Video Reasoning](https://arxiv.org/abs/2506.21742)
*Sirnam Swetha,Rohit Gupta,Parth Parag Kulkarni,David G Shatwell,Jeffrey A Chan Santiago,Nyle Siddiqui,Joseph Fioresi,Mubarak Shah*

Main category: cs.CV

TL;DR: 现有视频问答模型在处理电影和创意视频中的隐式推理方面表现不佳。本文提出了ImplicitQA，一个新的基准测试来评估模型的隐式推理能力。实验结果表明，领先的模型在该任务上性能下降，突显了隐式推理的挑战性。数据集已发布以促进相关研究。


<details>
  <summary>Details</summary>
Motivation: 当前的视频问答（Video QA）基准测试主要关注可以通过显式视觉内容回答的问题，而忽略了创意和电影视频中需要进行隐式推理（例如推断动机、因果关系和关系）的情况。现有的VideoQA系统和基准未能捕捉人类理解的这一基本维度。

Method: 提出了ImplicitQA，这是一个新颖的基准测试，专门用于测试模型的隐式推理能力。该基准包含1K个精心标注的问答对，来源于320多个高质量创意视频片段，并系统地分为多个关键推理维度，如横向和纵向空间推理、因果和动机推理等。

Result: 对主流VideoQA模型的广泛评估显示，它们的性能显著下降，这突显了它们对表面视觉线索的依赖以及隐式推理的难度。模型间的性能差异进一步说明了ImplicitQA所带来挑战的复杂性和多样性。

Conclusion: 通过发布ImplicitQA数据集及其数据收集框架，旨在促进视频问答领域在隐式推理方面的进一步研究和发展。

Abstract: Video QA has made significant strides by leveraging multimodal learning to
align visual and textual modalities. However, current benchmarks overwhelmingly
focus on questions answerable through explicit visual content - actions,
objects & events directly observable within individual frames or short clips.
In contrast, creative and cinematic videos - such as movies, TV shows, and
narrative-driven content - employ storytelling techniques that deliberately
omit certain depictions, requiring viewers to infer motives, causality, and
relationships across discontinuous frames. Humans naturally excel at such
implicit reasoning, seamlessly integrating information across time and context
to construct coherent narratives. Current VideoQA systems and benchmarks fail
to capture this essential dimension of human-like understanding. To bridge this
gap, we present ImplicitQA, a novel benchmark specifically designed to test
models on implicit reasoning. It comprises 1K meticulously annotated QA pairs
derived from 320+ high-quality creative video clips, systematically categorized
into key reasoning dimensions: lateral and vertical spatial reasoning, depth
and proximity, viewpoint and visibility, motion and trajectory, causal and
motivational reasoning, social interactions, physical context, and inferred
counting. These annotations are deliberately challenging, crafted by authors
ensuring high-quality. Our extensive evaluations on leading VideoQA models
reveals performance degradation, underscoring their reliance on surface-level
visual cues and highlighting the difficulty of implicit reasoning. Performance
variations across models further illustrate the complexity and diversity of the
challenges presented by ImplicitQA. By releasing both the dataset and our data
collection framework, we aim to stimulate further research and development in
the community. https://huggingface.co/datasets/ucf-crcv/ImplicitQA.

</details>


### [49] [Early Glaucoma Detection using Deep Learning with Multiple Datasets of Fundus Images](https://arxiv.org/abs/2506.21770)
*Rishiraj Paul Chowdhury,Nirmit Shekar Karkera*

Main category: cs.CV

TL;DR: 该论文提出了一种基于EfficientNet-B0深度学习模型，并在多个数据集上进行训练和微调，用于从视网膜眼底图像中早期检测青光眼的方法，表现出良好的性能。


<details>
  <summary>Details</summary>
Motivation: 青光眼是导致不可逆失明的主要原因，早期检测可以显著改善治疗结果，而传统诊断方法通常具有侵入性且需要专业设备。

Method: 该研究使用EfficientNet-B0架构的深度学习流程，通过在ACRIMA、ORIGA和RIM-ONE数据集上顺序训练和微调模型来检测视网膜眼底图像中的青光眼。

Result: 实验表明，最少的预处理能获得更高的AUC-ROC，并且模型在未见过的数据集上表现出强大的判别性能。

Conclusion: 该提出的流程为早期青光眼检测提供了一种可重现和可扩展的方法，支持其潜在的临床实用性。

Abstract: Glaucoma is a leading cause of irreversible blindness, but early detection
can significantly improve treatment outcomes. Traditional diagnostic methods
are often invasive and require specialized equipment. In this work, we present
a deep learning pipeline using the EfficientNet-B0 architecture for glaucoma
detection from retinal fundus images. Unlike prior studies that rely on single
datasets, we sequentially train and fine-tune our model across ACRIMA, ORIGA,
and RIM-ONE datasets to enhance generalization. Our experiments show that
minimal preprocessing yields higher AUC-ROC compared to more complex
enhancements, and our model demonstrates strong discriminative performance on
unseen datasets. The proposed pipeline offers a reproducible and scalable
approach to early glaucoma detection, supporting its potential clinical
utility.

</details>


### [50] [Comparing Learning Paradigms for Egocentric Video Summarization](https://arxiv.org/abs/2506.21785)
*Daniel Wen*

Main category: cs.CV

TL;DR: 当前最先进的计算机视觉模型在第一人称视频摘要方面表现不佳，但一个经过提示微调的GPT-4o模型出人意料地超越了专业模型，表明需要改进自我中心视频处理技术。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的模型在处理第一人称视频时效果不如第三人称视频，因此需要在自我中心视频领域取得进一步进展。研究旨在推进计算机视觉技术在第一人称视频中的应用。

Method: 研究通过评估监督学习（Shotluck Holmes）、无监督学习（TAC-SUM）和提示微调（GPT-4o）在理解和解释自我中心视频数据方面的能力，来探讨它们在视频摘要中的有效性。评估基于Ego-Exo4D数据集中的一小部分自我中心视频。

Result: 当前最先进的模型在第一人称视频上的表现不如第三人称视频。值得注意的是，经过提示微调的通用GPT-4o模型超越了这些专业模型。

Conclusion: 现有方法在适应第一人称视角的独特挑战方面存在局限性。未来需要进一步发展以有效处理和解释自我中心视角的模型。

Abstract: In this study, we investigate various computer vision paradigms - supervised
learning, unsupervised learning, and prompt fine-tuning - by assessing their
ability to understand and interpret egocentric video data. Specifically, we
examine Shotluck Holmes (state-of-the-art supervised learning), TAC-SUM
(state-of-the-art unsupervised learning), and GPT-4o (a prompt fine-tuned
pre-trained model), evaluating their effectiveness in video summarization. Our
results demonstrate that current state-of-the-art models perform less
effectively on first-person videos compared to third-person videos,
highlighting the need for further advancements in the egocentric video domain.
Notably, a prompt fine-tuned general-purpose GPT-4o model outperforms these
specialized models, emphasizing the limitations of existing approaches in
adapting to the unique challenges of first-person perspectives. Although our
evaluation is conducted on a small subset of egocentric videos from the
Ego-Exo4D dataset due to resource constraints, the primary objective of this
research is to provide a comprehensive proof-of-concept analysis aimed at
advancing the application of computer vision techniques to first-person videos.
By exploring novel methodologies and evaluating their potential, we aim to
contribute to the ongoing development of models capable of effectively
processing and interpreting egocentric perspectives.

</details>


### [51] [Fine-Grained Preference Optimization Improves Spatial Reasoning in VLMs](https://arxiv.org/abs/2506.21656)
*Yifan Shen,Yuanzhe Liu,Jingyuan Zhu,Xu Cao,Xiaofeng Zhang,Yixiao He,Wenming Ye,James Matthew Rehg,Ismini Lourentzou*

Main category: cs.CV

TL;DR: SpatialReasoner-R1通过M3CTS生成高质量数据和fDPO优化训练，显著提升了视觉语言模型在细粒度空间推理上的能力，并在相关基准测试中达到了新的SOTA。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型（VLMs）在细粒度空间推理方面表现不佳，特别是在需要多步逻辑和精确空间对齐时。

Method: 提出SpatialReasoner-R1模型；设计Multi-Model Monte Carlo Tree Search (M3CTS)方法生成高质量的空间推理监督数据；提出细粒度直接偏好优化（fDPO）方法，引入分段特定的偏好粒度并结合空间奖励机制。

Result: fDPO在空间质量任务上比标准DPO平均提升4.1%，在空间数量任务上提升9.0%。SpatialReasoner-R1在SPATIALRGPT-Bench上刷新了最先进水平，平均准确率超越最强基线9.8%，同时在通用视觉-语言任务上保持竞争力。

Conclusion: SpatialReasoner-R1有效解决了当前VLM在细粒度空间推理方面的局限性，并取得了最先进的性能。

Abstract: Current Vision-Language Models (VLMs) struggle with fine-grained spatial
reasoning, particularly when multi-step logic and precise spatial alignment are
required. In this work, we introduce SpatialReasoner-R1, a vision-language
reasoning model designed to address these limitations. To construct
high-quality supervision for spatial reasoning, we design a Multi-Model Monte
Carlo Tree Search (M3CTS) method that generates diverse, logically consistent
Long Chain-of-Thought (LongCoT) reasoning trajectories. In addition, we propose
fine-grained Direct Preference Optimization (fDPO), which introduces
segment-specific preference granularity for descriptive grounding and logical
reasoning, guided by a spatial reward mechanism that evaluates candidate
responses based on visual consistency, spatial grounding, and logical
coherence. Experimental results demonstrate that fDPO achieves an average
improvement of 4.1% over standard DPO across spatial quality tasks, and a 9.0%
gain in spatial quantity tasks. SpatialReasoner-R1, trained with fDPO, sets a
new SoTA on SPATIALRGPT-Bench, outperforming the strongest baseline by 9.8% in
average accuracy, while maintaining competitive performance on general
vision-language tasks.

</details>


### [52] [TanDiT: Tangent-Plane Diffusion Transformer for High-Quality 360° Panorama Generation](https://arxiv.org/abs/2506.21681)
*Hakan Çapuk,Andrew Bond,Muhammed Burak Kızıl,Emir Göçen,Erkut Erdem,Aykut Erdem*

Main category: cs.CV

TL;DR: TanDiT 提出了一种基于统一扩散模型的方法，通过生成切平面图像网格来解决全景图像生成中的几何畸变和循环一致性问题，并引入了新的评估指标，生成了高质量的全景图像。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型在处理全景图像生成时面临挑战，如几何畸变和无缝循环一致性。

Method: 提出了 TanDiT 方法，通过生成覆盖360度视图的切平面图像网格来合成全景场景。TanDiT 利用统一的扩散模型在一次去噪迭代中同时生成这些切平面图像。此外，还提出了一个模型无关的后处理步骤来增强全局一致性。为了准确评估全景图像质量，引入了 TangentIS 和 TangentFID 两个专用指标，并提供了包含带字幕全景数据集和标准化评估脚本的综合基准。

Result: 实验证明，该方法能有效泛化到训练数据之外，鲁棒地解释详细复杂的文本提示，并与各种生成模型无缝集成，生成高质量、多样化的全景图像。

Conclusion: TanDiT 能够有效生成高质量、多样化的全景图像，并克服了几何畸变和无缝循环一致性等挑战。

Abstract: Recent advances in image generation have led to remarkable improvements in
synthesizing perspective images. However, these models still struggle with
panoramic image generation due to unique challenges, including varying levels
of geometric distortion and the requirement for seamless loop-consistency. To
address these issues while leveraging the strengths of the existing models, we
introduce TanDiT, a method that synthesizes panoramic scenes by generating
grids of tangent-plane images covering the entire 360$^\circ$ view. Unlike
previous methods relying on multiple diffusion branches, TanDiT utilizes a
unified diffusion model trained to produce these tangent-plane images
simultaneously within a single denoising iteration. Furthermore, we propose a
model-agnostic post-processing step specifically designed to enhance global
coherence across the generated panoramas. To accurately assess panoramic image
quality, we also present two specialized metrics, TangentIS and TangentFID, and
provide a comprehensive benchmark comprising captioned panoramic datasets and
standardized evaluation scripts. Extensive experiments demonstrate that our
method generalizes effectively beyond its training data, robustly interprets
detailed and complex text prompts, and seamlessly integrates with various
generative models to yield high-quality, diverse panoramic images.

</details>


### [53] [Fine-Grained Preference Optimization Improves Spatial Reasoning in VLMs](https://arxiv.org/abs/2506.21656)
*Yifan Shen,Yuanzhe Liu,Jingyuan Zhu,Xu Cao,Xiaofeng Zhang,Yixiao He,Wenming Ye,James Matthew Rehg,Ismini Lourentzou*

Main category: cs.CV

TL;DR: SpatialReasoner-R1 模型结合 M3CTS 和 fDPO 显著提升了视觉-语言模型在精细空间推理方面的能力，并在 SPATIALRGPT-Bench 上取得了新的 SOTA 成果。


<details>
  <summary>Details</summary>
Motivation: 当前的视觉-语言模型 (VLMs) 在精细空间推理方面存在困难，尤其是在需要多步逻辑和精确空间对齐时。

Method: 1. 引入了 SpatialReasoner-R1 视觉-语言推理模型。2. 设计了多模型蒙特卡洛树搜索 (M3CTS) 方法，用于生成多样且逻辑一致的长链思维 (LongCoT) 推理轨迹，以构建高质量的空间推理监督数据。3. 提出了细粒度直接偏好优化 (fDPO) 方法，引入了段落特定的偏好粒度，并通过空间奖励机制评估候选响应，确保视觉一致性、空间定位和逻辑连贯性。

Result: 1. fDPO 在空间质量任务上比标准 DPO 平均提高了 4.1%，在空间数量任务上提高了 9.0%。2. 使用 fDPO 训练的 SpatialReasoner-R1 在 SPATIALRGPT-Bench 上取得了新的最先进水平 (SoTA)，平均准确率比最强基线高出 9.8%。3. 在通用视觉-语言任务上保持了有竞争力的性能。

Conclusion: SpatialReasoner-R1 模型通过结合 M3CTS 生成高质量训练数据和 fDPO 优化方法，显著提升了视觉-语言模型在精细空间推理方面的性能，并在相关基准测试中取得了最先进的成果。

Abstract: Current Vision-Language Models (VLMs) struggle with fine-grained spatial
reasoning, particularly when multi-step logic and precise spatial alignment are
required. In this work, we introduce SpatialReasoner-R1, a vision-language
reasoning model designed to address these limitations. To construct
high-quality supervision for spatial reasoning, we design a Multi-Model Monte
Carlo Tree Search (M3CTS) method that generates diverse, logically consistent
Long Chain-of-Thought (LongCoT) reasoning trajectories. In addition, we propose
fine-grained Direct Preference Optimization (fDPO), which introduces
segment-specific preference granularity for descriptive grounding and logical
reasoning, guided by a spatial reward mechanism that evaluates candidate
responses based on visual consistency, spatial grounding, and logical
coherence. Experimental results demonstrate that fDPO achieves an average
improvement of 4.1% over standard DPO across spatial quality tasks, and a 9.0%
gain in spatial quantity tasks. SpatialReasoner-R1, trained with fDPO, sets a
new SoTA on SPATIALRGPT-Bench, outperforming the strongest baseline by 9.8% in
average accuracy, while maintaining competitive performance on general
vision-language tasks.

</details>


### [54] [TanDiT: Tangent-Plane Diffusion Transformer for High-Quality 360° Panorama Generation](https://arxiv.org/abs/2506.21681)
*Hakan Çapuk,Andrew Bond,Muhammed Burak Kızıl,Emir Göçen,Erkut Erdem,Aykut Erdem*

Main category: cs.CV

TL;DR: TanDiT是一种新的全景图像生成方法，通过生成切平面图像网格并在统一扩散模型中处理，解决了现有模型在全景图像生成中的挑战，生成了高质量的全景图像，并引入了新的评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有的图像生成模型在处理全景图像时面临几何失真和无缝循环一致性等挑战。

Method: 提出TanDiT方法，通过生成覆盖360度视角的切平面图像网格来合成全景场景。TanDiT使用统一的扩散模型在单次去噪迭代中同时生成这些切平面图像。此外，还提出了一个模型无关的后处理步骤来增强全局一致性，并引入了TangentIS和TangentFID两个专门指标及全景数据集基准。

Result: TanDiT方法能够有效泛化到训练数据之外，鲁棒地解释详细和复杂的文本提示，并与各种生成模型无缝集成，生成高质量、多样化的全景图像。

Conclusion: TanDiT方法有效地解决了全景图像生成中的挑战，能够生成高质量、多样化的全景图像，并能泛化到训练数据之外。

Abstract: Recent advances in image generation have led to remarkable improvements in
synthesizing perspective images. However, these models still struggle with
panoramic image generation due to unique challenges, including varying levels
of geometric distortion and the requirement for seamless loop-consistency. To
address these issues while leveraging the strengths of the existing models, we
introduce TanDiT, a method that synthesizes panoramic scenes by generating
grids of tangent-plane images covering the entire 360$^\circ$ view. Unlike
previous methods relying on multiple diffusion branches, TanDiT utilizes a
unified diffusion model trained to produce these tangent-plane images
simultaneously within a single denoising iteration. Furthermore, we propose a
model-agnostic post-processing step specifically designed to enhance global
coherence across the generated panoramas. To accurately assess panoramic image
quality, we also present two specialized metrics, TangentIS and TangentFID, and
provide a comprehensive benchmark comprising captioned panoramic datasets and
standardized evaluation scripts. Extensive experiments demonstrate that our
method generalizes effectively beyond its training data, robustly interprets
detailed and complex text prompts, and seamlessly integrates with various
generative models to yield high-quality, diverse panoramic images.

</details>


### [55] [FOCUS: Internal MLLM Representations for Efficient Fine-Grained Visual Question Answering](https://arxiv.org/abs/2506.21710)
*Liangyu Zhong,Fabio Rosenthal,Joachim Sicking,Fabian Hüger,Thorsten Bagdonat,Hanno Gottschalk,Leo Schwinn*

Main category: cs.CV

TL;DR: FOCUS是一种免训练的视觉裁剪方法，它利用MLLM的内部表示来寻找最相关的图像区域，从而显著提升了细粒度视觉问答（VQA）的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 多模态大型语言模型（MLLMs）在图像-文本输入方面具有强大的感知和推理能力，但专注于图像小细节的视觉问答（VQA）仍然是一个挑战。现有视觉裁剪技术存在任务特定微调需求、低效率或与高效注意力实现不兼容等局限性。

Method: 提出了一种名为FOCUS的免训练视觉裁剪方法，利用MLLM内部表示来指导最相关图像区域的搜索。该方法包括四个步骤：识别VQA提示中的目标对象、使用键值（KV）缓存计算对象相关性图、基于相关性图提出并排序相关图像区域，最后使用排名靠前的区域执行细粒度VQA任务。

Result: FOCUS在四个细粒度VQA数据集和两种MLLM上取得了强大性能，在准确性和效率上优于三种流行的视觉裁剪方法，并且与表现最佳的基线ZoomEye相当，同时计算量减少了3至6.5倍。

Conclusion: FOCUS在细粒度VQA任务上表现出色，并在准确性和效率方面优于现有裁剪方法，同时显著降低了计算成本。

Abstract: While Multimodal Large Language Models (MLLMs) offer strong perception and
reasoning capabilities for image-text input, Visual Question Answering (VQA)
focusing on small image details still remains a challenge. Although visual
cropping techniques seem promising, recent approaches have several limitations:
the need for task-specific fine-tuning, low efficiency due to uninformed
exhaustive search, or incompatibility with efficient attention implementations.
We address these shortcomings by proposing a training-free visual cropping
method, dubbed FOCUS, that leverages MLLM-internal representations to guide the
search for the most relevant image region. This is accomplished in four steps:
first, we identify the target object(s) in the VQA prompt; second, we compute
an object relevance map using the key-value (KV) cache; third, we propose and
rank relevant image regions based on the map; and finally, we perform the
fine-grained VQA task using the top-ranked region. As a result of this informed
search strategy, FOCUS achieves strong performance across four fine-grained VQA
datasets and two types of MLLMs. It outperforms three popular visual cropping
methods in both accuracy and efficiency, and matches the best-performing
baseline, ZoomEye, while requiring 3 - 6.5 x less compute.

</details>


<div id='cs.ET'></div>

# cs.ET [[Back]](#toc)

### [56] [Evaluating Redundancy Mitigation in Vulnerable Road User Awareness Messages for Bicycles](https://arxiv.org/abs/2506.22052)
*Nico Ostendorf,Keno Garlichs,Lars Wolf*

Main category: cs.ET

TL;DR: V2X通信中，设备增加导致信道负载高。研究评估冗余缓解（RM）对弱势道路使用者（VRU）通信的影响。发现RM虽降信道负载，但降VRU感知率。标准RM因信息过时有安全隐患。提出改进RM，平衡负载与安全，效果更优。未来需优化RM技术以兼顾V2X通信效率与VRU安全。


<details>
  <summary>Details</summary>
Motivation: V2X通信对提高道路安全至关重要，特别是对行人、骑自行车者等弱势道路使用者。然而，信道上通信设备数量的增加会导致显著的信道负载。为解决此问题，本研究评估了冗余缓解（RM）对弱势道路使用者感知消息（VAM）的有效性。

Method: 本研究通过模拟研究评估了用于弱势道路使用者感知消息（VAM）的冗余缓解（RM）的有效性，重点关注骑自行车的人。在基于德国汉诺威交通数据的城市场景中进行模拟，评估了RM对信道负载（由信道忙碌率CBR衡量）和安全性（由弱势道路使用者感知率VPR衡量）的影响。此外，还分析了自我弱势道路使用者与被认为是冗余的弱势道路使用者之间在位置、速度和航向上的实际差异。研究提出了一种改进的RM机制。

Result: 研究发现，尽管RM可以减少信道拥塞，但也会导致VPR下降。对实际差异的分析表明，ETSI标准化的RM机制常使用过时信息，导致位置、速度和航向存在显著差异，可能导致危险情况。提出的改进RM机制与标准化RM相比，显著降低了最大CBR，且VPR下降幅度较小，并在位置、速度和航向的实际差异方面表现更好，从而提高了整体安全性。

Conclusion: 结果强调需要进一步研究以优化冗余缓解技术，并确保它们在不影响弱势道路使用者安全的情况下有效增强V2X通信。

Abstract: V2X communication has become crucial for enhancing road safety, especially
for Vulnerable Road Users (VRU) such as pedestrians and cyclists. However, the
increasing number of devices communicating on the same channels will lead to
significant channel load. To address this issue this study evaluates the
effectiveness of Redundancy Mitigation (RM) for VRU Awareness Messages (VAM),
focusing specifically on cyclists. The objective of RM is to minimize the
transmission of redundant information. We conducted a simulation study using a
urban scenario with a high bicycle density based on traffic data from Hannover,
Germany. This study assessed the impact of RM on channel load, measured by
Channel Busy Ratio (CBR), and safety, measured by VRU Perception Rate (VPR) in
simulation. To evaluate the accuracy and reliability of the RM mechanisms, we
analyzed the actual differences in position, speed, and heading between the ego
VRU and the VRU, which was assumed to be redundant. Our findings indicate that
while RM can reduce channel congestion, it also leads to a decrease in VPR. The
analysis of actual differences revealed that the RM mechanism standardized by
ETSI often uses outdated information, leading to significant discrepancies in
position, speed, and heading, which could result in dangerous situations. To
address these limitations, we propose an adapted RM mechanism that improves the
balance between reducing channel load and maintaining VRU awareness. The
adapted approach shows a significant reduction in maximum CBR and a less
significant decrease in VPR compared to the standardized RM. Moreover, it
demonstrates better performance in the actual differences in position, speed,
and heading, thereby enhancing overall safety. Our results highlight the need
for further research to optimize RM techniques and ensure they effectively
enhance V2X communication without compromising the safety of VRUs.

</details>


### [57] [Unified Memcapacitor-Memristor Memory for Synaptic Weights and Neuron Temporal Dynamics](https://arxiv.org/abs/2506.22227)
*Simone D'Agostino,Marco Massarotto,Tristan Torchet,Filippo Moro,Niccolò Castellani,Laurent Grenouillet,Yann Beilliard,David Esseni,Melika Payvand,Elisa Vianello*

Main category: cs.ET

TL;DR: 本文提出并实验验证了一种统一忆阻和忆容行为的存储堆栈，利用其双重功能设计了用于同时控制循环尖峰神经网络（RSNNs）空间和时间动态的电路，并通过硬件感知仿真证明了其在高效神经拟态处理中的潜力。


<details>
  <summary>Details</summary>
Motivation: 设计一种能够同时控制循环尖峰神经网络（RSNNs）中空间和时间动态的电路，以实现高效的神经拟态处理。

Method: 本文提出并实验表征了一种统一忆阻和忆容行为的存储堆栈，并利用其双重功能设计了一种能够同时控制循环尖峰神经网络（RSNNs）中空间和时间动态的电路。

Result: 硬件感知仿真突显了其在高效神经拟态处理方面的潜力。

Conclusion: 一种统一忆阻和忆容行为的存储堆栈有望实现高效的神经拟态处理。

Abstract: We present a fabricated and experimentally characterized memory stack that
unifies memristive and memcapacitive behavior. Exploiting this dual
functionality, we design a circuit enabling simultaneous control of spatial and
temporal dynamics in recurrent spiking neural networks (RSNNs). Hardware-aware
simulations highlight its promise for efficient neuromorphic processing.

</details>


### [58] [Evaluating Redundancy Mitigation in Vulnerable Road User Awareness Messages for Bicycles](https://arxiv.org/abs/2506.22052)
*Nico Ostendorf,Keno Garlichs,Lars Wolf*

Main category: cs.ET

TL;DR: 为了解决V2X通信中信道负载过高的问题，研究评估了冗余缓解（RM）机制。发现标准RM虽能降低信道拥堵，但会因信息过时而降低弱势道路使用者安全感知。提出了一种改进的RM机制，能在降低信道负载的同时，更好地保持弱势道路使用者感知和安全性。


<details>
  <summary>Details</summary>
Motivation: V2X通信对于提高弱势道路使用者（如骑行者）的道路安全至关重要，但设备数量的增加导致信道负载显著，因此需要评估冗余缓解（RM）在弱势道路使用者感知消息（VAM）中的有效性。

Method: 使用基于德国汉诺威交通数据的城市高自行车密度场景进行仿真研究，评估冗余缓解（RM）对信道负载（CBR）和弱势道路使用者感知率（VPR）的影响。分析了实际位置、速度和航向差异，并提出了一种改进的RM机制。

Result: 标准化冗余缓解机制可以降低信道拥堵，但会导致弱势道路使用者感知率下降，且常使用过时信息，造成位置、速度和航向的显著差异，可能导致危险情况。本研究提出了一种改进的冗余缓解机制，该机制在显著降低最大信道忙碌率的同时，对弱势道路使用者感知率的影响较小，并在位置、速度和航向的实际差异方面表现更好，从而提高了整体安全性。

Conclusion: 需要进一步研究以优化冗余缓解技术，以在不损害弱势道路使用者安全的前提下，有效增强V2X通信。

Abstract: V2X communication has become crucial for enhancing road safety, especially
for Vulnerable Road Users (VRU) such as pedestrians and cyclists. However, the
increasing number of devices communicating on the same channels will lead to
significant channel load. To address this issue this study evaluates the
effectiveness of Redundancy Mitigation (RM) for VRU Awareness Messages (VAM),
focusing specifically on cyclists. The objective of RM is to minimize the
transmission of redundant information. We conducted a simulation study using a
urban scenario with a high bicycle density based on traffic data from Hannover,
Germany. This study assessed the impact of RM on channel load, measured by
Channel Busy Ratio (CBR), and safety, measured by VRU Perception Rate (VPR) in
simulation. To evaluate the accuracy and reliability of the RM mechanisms, we
analyzed the actual differences in position, speed, and heading between the ego
VRU and the VRU, which was assumed to be redundant. Our findings indicate that
while RM can reduce channel congestion, it also leads to a decrease in VPR. The
analysis of actual differences revealed that the RM mechanism standardized by
ETSI often uses outdated information, leading to significant discrepancies in
position, speed, and heading, which could result in dangerous situations. To
address these limitations, we propose an adapted RM mechanism that improves the
balance between reducing channel load and maintaining VRU awareness. The
adapted approach shows a significant reduction in maximum CBR and a less
significant decrease in VPR compared to the standardized RM. Moreover, it
demonstrates better performance in the actual differences in position, speed,
and heading, thereby enhancing overall safety. Our results highlight the need
for further research to optimize RM techniques and ensure they effectively
enhance V2X communication without compromising the safety of VRUs.

</details>


### [59] [Unified Memcapacitor-Memristor Memory for Synaptic Weights and Neuron Temporal Dynamics](https://arxiv.org/abs/2506.22227)
*Simone D'Agostino,Marco Massarotto,Tristan Torchet,Filippo Moro,Niccolò Castellani,Laurent Grenouillet,Yann Beilliard,David Esseni,Melika Payvand,Elisa Vianello*

Main category: cs.ET

TL;DR: 统一忆阻/忆电容存储堆栈实现了对循环尖峰神经网络动态的有效控制，有望用于高效神经形态处理。


<details>
  <summary>Details</summary>
Motivation: 利用忆阻和忆电容的双重功能，设计一个能够同时控制循环尖峰神经网络（RSNN）中空间和时间动态的电路。

Method: 本文制作并实验表征了一种统一忆阻和忆电容行为的存储堆栈。利用这种双重功能，设计了一种能够同时控制循环尖峰神经网络（RSNN）中空间和时间动态的电路。进行了硬件感知模拟。

Result: 所设计的电路能够同时控制循环尖峰神经网络（RSNN）中的空间和时间动态。硬件感知模拟突显了其在高效神经形态处理方面的潜力。

Conclusion: 统一忆阻和忆电容存储器堆栈及其设计的电路在循环尖峰神经网络（RSNN）中显示出高效神经形态处理的潜力。

Abstract: We present a fabricated and experimentally characterized memory stack that
unifies memristive and memcapacitive behavior. Exploiting this dual
functionality, we design a circuit enabling simultaneous control of spatial and
temporal dynamics in recurrent spiking neural networks (RSNNs). Hardware-aware
simulations highlight its promise for efficient neuromorphic processing.

</details>


### [60] [Evaluating Redundancy Mitigation in Vulnerable Road User Awareness Messages for Bicycles](https://arxiv.org/abs/2506.22052)
*Nico Ostendorf,Keno Garlichs,Lars Wolf*

Main category: cs.ET

TL;DR: 本研究在V2X通信中，通过模拟评估了弱势道路使用者感知消息的冗余缓解（RM）机制，发现标准化RM虽能降低信道负载但会牺牲弱势道路使用者安全，因此提出了一种改进的RM机制，该机制在负载和安全之间取得了更好的平衡。


<details>
  <summary>Details</summary>
Motivation: V2X通信对于增强道路安全至关重要，特别是对于行人、骑自行车者等弱势道路使用者。然而，信道上通信设备数量的增加会导致显著的信道负载。为解决此问题，本研究评估了弱势道路使用者感知消息（VAM）冗余缓解（RM）的有效性。

Method: 本研究使用基于德国汉诺威交通数据的城市场景模拟，评估了冗余缓解（RM）对信道负载（由信道忙碌率CBR衡量）和安全性（由弱势道路使用者感知率VPR衡量）的影响。同时，通过分析位置、速度和航向的实际差异来评估RM机制的准确性和可靠性。针对标准RM机制的局限性，提出了一种改进的RM机制。

Result: 研究发现，冗余缓解（RM）可以减少信道拥塞，但也会导致弱势道路使用者感知率（VPR）下降。ETSI标准化的RM机制常使用过时信息，导致位置、速度和航向存在显著差异，可能造成危险情况。提出的改进RM机制在最大CBR方面有显著降低，VPR下降幅度较小，并且在位置、速度和航向的实际差异方面表现更好，从而提高了整体安全性。

Conclusion: 研究结果强调需要进一步优化冗余缓解技术，以确保其在不影响弱势道路使用者安全的情况下有效增强V2X通信。

Abstract: V2X communication has become crucial for enhancing road safety, especially
for Vulnerable Road Users (VRU) such as pedestrians and cyclists. However, the
increasing number of devices communicating on the same channels will lead to
significant channel load. To address this issue this study evaluates the
effectiveness of Redundancy Mitigation (RM) for VRU Awareness Messages (VAM),
focusing specifically on cyclists. The objective of RM is to minimize the
transmission of redundant information. We conducted a simulation study using a
urban scenario with a high bicycle density based on traffic data from Hannover,
Germany. This study assessed the impact of RM on channel load, measured by
Channel Busy Ratio (CBR), and safety, measured by VRU Perception Rate (VPR) in
simulation. To evaluate the accuracy and reliability of the RM mechanisms, we
analyzed the actual differences in position, speed, and heading between the ego
VRU and the VRU, which was assumed to be redundant. Our findings indicate that
while RM can reduce channel congestion, it also leads to a decrease in VPR. The
analysis of actual differences revealed that the RM mechanism standardized by
ETSI often uses outdated information, leading to significant discrepancies in
position, speed, and heading, which could result in dangerous situations. To
address these limitations, we propose an adapted RM mechanism that improves the
balance between reducing channel load and maintaining VRU awareness. The
adapted approach shows a significant reduction in maximum CBR and a less
significant decrease in VPR compared to the standardized RM. Moreover, it
demonstrates better performance in the actual differences in position, speed,
and heading, thereby enhancing overall safety. Our results highlight the need
for further research to optimize RM techniques and ensure they effectively
enhance V2X communication without compromising the safety of VRUs.

</details>


### [61] [Unified Memcapacitor-Memristor Memory for Synaptic Weights and Neuron Temporal Dynamics](https://arxiv.org/abs/2506.22227)
*Simone D'Agostino,Marco Massarotto,Tristan Torchet,Filippo Moro,Niccolò Castellani,Laurent Grenouillet,Yann Beilliard,David Esseni,Melika Payvand,Elisa Vianello*

Main category: cs.ET

TL;DR: 一种新的存储堆栈结合了忆阻和忆容行为，实现了一个能够同时控制RSNN中空间和时间动态的电路，在高效神经拟态处理方面展现了前景。


<details>
  <summary>Details</summary>
Motivation: 利用忆阻和忆容的双重功能，设计一个能够同时控制循环尖峰神经网络（RSNNs）中空间和时间动态的电路。

Method: 提出并实验表征了一种统一忆阻和忆容行为的存储堆栈，并利用其双重功能设计了一个能够同时控制循环尖峰神经网络（RSNNs）中空间和时间动态的电路。

Result: 硬件感知模拟突出显示了其在高效神经拟态处理方面的潜力。

Conclusion: 该记忆堆栈和电路设计在高效神经拟态处理方面具有前景。

Abstract: We present a fabricated and experimentally characterized memory stack that
unifies memristive and memcapacitive behavior. Exploiting this dual
functionality, we design a circuit enabling simultaneous control of spatial and
temporal dynamics in recurrent spiking neural networks (RSNNs). Hardware-aware
simulations highlight its promise for efficient neuromorphic processing.

</details>


<div id='cs.GR'></div>

# cs.GR [[Back]](#toc)

### [62] [ICP-3DGS: SfM-free 3D Gaussian Splatting for Large-scale Unbounded Scenes](https://arxiv.org/abs/2506.21629)
*Chenhao Zhang,Yezhi Shen,Fengqing Zhu*

Main category: cs.GR

TL;DR: ICP-3DGS结合ICP与体素致密化，解决了大规模户外场景中神经渲染的相机位姿和3D结构先验依赖问题，提升了相机位姿估计和新视角合成的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的神经渲染方法（如NeRFs和3D Gaussian Splatting）严重依赖预处理的相机位姿和来自运动恢复结构（SfM）的3D结构先验，这在室外场景中难以获取。

Method: 提出将迭代最近点（ICP）与基于优化的细化相结合，用于大相机运动下的精确相机位姿估计。同时，引入了一种基于体素的场景致密化方法来引导大规模场景的重建。

Result: 实验证明，所提出的ICP-3DGS方法在相机位姿估计和新视角合成方面均优于现有方法，适用于各种规模的室内外场景。

Conclusion: ICP-3DGS能够有效解决大规模场景中神经渲染对相机位姿和3D结构先验的依赖问题，并在相机位姿估计和新视角合成方面取得更好的性能。

Abstract: In recent years, neural rendering methods such as NeRFs and 3D Gaussian
Splatting (3DGS) have made significant progress in scene reconstruction and
novel view synthesis. However, they heavily rely on preprocessed camera poses
and 3D structural priors from structure-from-motion (SfM), which are
challenging to obtain in outdoor scenarios. To address this challenge, we
propose to incorporate Iterative Closest Point (ICP) with optimization-based
refinement to achieve accurate camera pose estimation under large camera
movements. Additionally, we introduce a voxel-based scene densification
approach to guide the reconstruction in large-scale scenes. Experiments
demonstrate that our approach ICP-3DGS outperforms existing methods in both
camera pose estimation and novel view synthesis across indoor and outdoor
scenes of various scales. Source code is available at
https://github.com/Chenhao-Z/ICP-3DGS.

</details>


### [63] [SkinningGS: Editable Dynamic Human Scene Reconstruction Using Gaussian Splatting Based on a Skinning Model](https://arxiv.org/abs/2506.21632)
*Da Li,Donggang Jia,Markus Hadwiger,Ivan Viola*

Main category: cs.GR

TL;DR: 一种从单目视频重建交互式人体和背景的新方法，通过点云解耦、位置纹理和CNN特征，实现了比HUGS更好的重建质量、更快的渲染速度和更低的资源消耗，并可扩展到动物重建。


<details>
  <summary>Details</summary>
Motivation: 从单目视频中重建交互式人体化身和背景极具挑战性，目标是实现背景和人体的解耦重建，同时保留人体运动的交互性。

Method: 采用点云解耦和联合优化策略。引入位置纹理细分SMPL模型表面并增长人体点云。结合CNN预测基于纹理的人体点云特征，避免了密集化的超参数调整。

Result: 在重建指标上超越了HUGS，同时保持了对新姿势和视角的泛化能力。实现了超过100 FPS的实时渲染，速度是HUGS的约6倍。显著减少了训练期间的GPU资源消耗。该框架可以扩展到动物场景重建。

Conclusion: 本文提出了一种有效的方法，可以从单目视频中重建交互式人像和背景，其在质量、效率和泛化能力上均优于现有的HUGS等方法，并有潜力扩展到动物重建。

Abstract: Reconstructing an interactive human avatar and the background from a
monocular video of a dynamic human scene is highly challenging. In this work we
adopt a strategy of point cloud decoupling and joint optimization to achieve
the decoupled reconstruction of backgrounds and human bodies while preserving
the interactivity of human motion. We introduce a position texture to subdivide
the Skinned Multi-Person Linear (SMPL) body model's surface and grow the human
point cloud. To capture fine details of human dynamics and deformations, we
incorporate a convolutional neural network structure to predict human body
point cloud features based on texture. This strategy makes our approach free of
hyperparameter tuning for densification and efficiently represents human points
with half the point cloud of HUGS. This approach ensures high-quality human
reconstruction and reduces GPU resource consumption during training. As a
result, our method surpasses the previous state-of-the-art HUGS in
reconstruction metrics while maintaining the ability to generalize to novel
poses and views. Furthermore, our technique achieves real-time rendering at
over 100 FPS, $\sim$6$\times$ the HUGS speed using only Linear Blend Skinning
(LBS) weights for human transformation. Additionally, this work demonstrates
that this framework can be extended to animal scene reconstruction when an
accurately-posed model of an animal is available.

</details>


### [64] [SAR-GS: 3D Gaussian Splatting for Synthetic Aperture Radar Target Reconstruction](https://arxiv.org/abs/2506.21633)
*Aobo Li,Zhengxin Lei,Jiangtao Wei,Feng Xu*

Main category: cs.GR

TL;DR: 受3D Gaussian Splatting启发，本文提出SDGR，一种针对SAR图像的3D目标重建方法。它通过优化高斯基元参数重建目标，并利用自定义CUDA加速梯度计算。实验证明该方法在模拟和真实数据上均能有效重建目标几何结构和散射特性。


<details>
  <summary>Details</summary>
Motivation: 合成孔径雷达（SAR）图像中的三维目标重建对于理解SAR数据中复杂的散射信息至关重要，但SAR成像固有的复杂电磁散射机制带来了显著的重建挑战。

Method: 本文提出了一种新颖的SAR可微分高斯溅射光栅化器（SDGR），用于SAR目标重建。该方法结合了高斯溅射与映射投影算法来计算高斯基元的散射强度并生成模拟SAR图像。通过计算渲染图像与真实图像之间的损失函数来优化高斯基元参数，并采用自定义CUDA梯度流代替自动微分以加速梯度计算。

Result: 通过对简化建筑目标和多车辆目标的SAR图像进行渲染实验，验证了SDGR在模拟SAR图像上的成像合理性。该方法在模拟和真实多车辆目标数据集上均展示了目标重建的有效性，并进行了定量评估。实验结果表明，该方法能够有效重建目标的几何结构和散射特性。

Conclusion: 本方法能有效重建目标的几何结构和散射特性，为SAR成像领域的3D重建提供了一种新颖的解决方案。

Abstract: Three-dimensional target reconstruction from synthetic aperture radar (SAR)
imagery is crucial for interpreting complex scattering information in SAR data.
However, the intricate electromagnetic scattering mechanisms inherent to SAR
imaging pose significant reconstruction challenges. Inspired by the remarkable
success of 3D Gaussian Splatting (3D-GS) in optical domain reconstruction, this
paper presents a novel SAR Differentiable Gaussian Splatting Rasterizer (SDGR)
specifically designed for SAR target reconstruction. Our approach combines
Gaussian splatting with the Mapping and Projection Algorithm to compute
scattering intensities of Gaussian primitives and generate simulated SAR images
through SDGR. Subsequently, the loss function between the rendered image and
the ground truth image is computed to optimize the Gaussian primitive
parameters representing the scene, while a custom CUDA gradient flow is
employed to replace automatic differentiation for accelerated gradient
computation. Through experiments involving the rendering of simplified
architectural targets and SAR images of multiple vehicle targets, we validate
the imaging rationality of SDGR on simulated SAR imagery. Furthermore, the
effectiveness of our method for target reconstruction is demonstrated on both
simulated and real-world datasets containing multiple vehicle targets, with
quantitative evaluations conducted to assess its reconstruction performance.
Experimental results indicate that our approach can effectively reconstruct the
geometric structures and scattering properties of targets, thereby providing a
novel solution for 3D reconstruction in the field of SAR imaging.

</details>


### [65] [A Design Space for Visualization Transitions of 3D Spatial Data in Hybrid AR-Desktop Environments](https://arxiv.org/abs/2506.22250)
*Yucheng Lu,Tobias Rau,Benjamin Lee,Andreas Köhn,Michael Sedlmair,Christian Sandor,Tobias Isenberg*

Main category: cs.GR

TL;DR: 论文提出了一个针对混合AR-桌面环境中3D空间数据集动画过渡的设计空间，旨在降低认知负荷并连接不同表示，并通过天文学、放射学和化学领域的案例研究进行了验证。


<details>
  <summary>Details</summary>
Motivation: 在混合增强现实(AR)-桌面环境中，为3D空间数据集的显示转换引入过渡动画，以说明不同表示之间的连接并减少用户认知负荷，同时简化设计决策并为未来设计提供灵感。

Method: 提出并详细阐述了一个针对3D空间数据集在混合AR-桌面环境中进行动画过渡的设计空间。该方法通过空间编码管线视角讨论3D可视化，并专注于在不同管线之间进行平滑插值以实现无缝体验，并通过三个案例研究验证了其应用。

Result: 提出了一个动画过渡的设计空间，并将其应用于天文学、放射学和化学领域的三个案例研究中，讨论了从中吸取的经验教训。

Conclusion: 本文为混合AR-桌面环境中3D空间数据集的动画过渡提供了一个设计空间，旨在简化决策过程并为未来的设计提供启发。

Abstract: We present a design space for animated transitions of the appearance of 3D
spatial datasets in a hybrid Augmented Reality (AR)-desktop context. Such
hybrid interfaces combine both traditional and immersive displays to facilitate
the exploration of 2D and 3D data representations in the environment in which
they are best displayed. One key aspect is to introduce transitional animations
that change between the different dimensionalities to illustrate the connection
between the different representations and to reduce the potential cognitive
load on the user. The specific transitions to be used depend on the type of
data, the needs of the application domain, and other factors. We summarize
these as a transition design space to simplify the decision-making process and
provide inspiration for future designs. First, we discuss 3D visualizations
from a spatial perspective: a spatial encoding pipeline, where 3D data sampled
from the physical world goes through various transformations, being mapped to
visual representations, and then being integrated into a hybrid AR-desktop
environment. The transition design then focuses on interpolating between two
spatial encoding pipelines to provide a smooth experience. To illustrate the
use of our design space, we apply it to three case studies that focus on
applications in astronomy, radiology, and chemistry; we then discuss lessons
learned from these applications.

</details>


### [66] [ICP-3DGS: SfM-free 3D Gaussian Splatting for Large-scale Unbounded Scenes](https://arxiv.org/abs/2506.21629)
*Chenhao Zhang,Yezhi Shen,Fengqing Zhu*

Main category: cs.GR

TL;DR: 针对神经渲染方法在室外场景中相机姿态获取的难题，本文提出ICP-3DGS。该方法结合ICP进行姿态估计和基于体素的场景密集化，在室内外各种规模场景的姿态估计和新视图合成上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的神经渲染方法（如NeRF和3D高斯泼溅）严重依赖预处理的相机姿态和SfM的3D结构先验，这在室外场景中难以获得。

Method: 我们提出了将迭代最近点（ICP）与基于优化的细化相结合，以在大相机运动下实现准确的相机姿态估计。此外，我们引入了一种基于体素的场景密集化方法来指导大规模场景的重建。

Result: 我们的方法ICP-3DGS在相机姿态估计和新视图合成方面均优于现有方法，适用于各种规模的室内和室外场景。

Conclusion: 我们提出了一种名为ICP-3DGS的方法，通过结合ICP和优化以及基于体素的场景密集化，有效地解决了在各种规模场景（特别是室外场景）中相机姿态估计和场景重建的挑战。

Abstract: In recent years, neural rendering methods such as NeRFs and 3D Gaussian
Splatting (3DGS) have made significant progress in scene reconstruction and
novel view synthesis. However, they heavily rely on preprocessed camera poses
and 3D structural priors from structure-from-motion (SfM), which are
challenging to obtain in outdoor scenarios. To address this challenge, we
propose to incorporate Iterative Closest Point (ICP) with optimization-based
refinement to achieve accurate camera pose estimation under large camera
movements. Additionally, we introduce a voxel-based scene densification
approach to guide the reconstruction in large-scale scenes. Experiments
demonstrate that our approach ICP-3DGS outperforms existing methods in both
camera pose estimation and novel view synthesis across indoor and outdoor
scenes of various scales. Source code is available at
https://github.com/Chenhao-Z/ICP-3DGS.

</details>


### [67] [ICP-3DGS: SfM-free 3D Gaussian Splatting for Large-scale Unbounded Scenes](https://arxiv.org/abs/2506.21629)
*Chenhao Zhang,Yezhi Shen,Fengqing Zhu*

Main category: cs.GR

TL;DR: ICP-3DGS通过结合ICP和体素场景致密化，解决了现有神经渲染方法在户外大规模场景中相机姿态和3D结构先验难以获取的问题，并在姿态估计和视图合成方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的神经渲染方法（如NeRFs和3D Gaussian Splatting）高度依赖预处理的相机姿态和SfM的3D结构先验，这在户外场景中难以获取。

Method: 提出结合迭代最近点（ICP）与优化精炼以实现大相机运动下的精确姿态估计，并引入基于体素的场景致密化方法来指导大规模场景重建。

Result: ICP-3DGS在相机姿态估计和新颖视图合成方面均优于现有方法，适用于各种尺度的室内外场景。

Conclusion: ICP-3DGS方法在室内外各种尺度场景中，相机姿态估计和新颖视图合成方面均优于现有方法。

Abstract: In recent years, neural rendering methods such as NeRFs and 3D Gaussian
Splatting (3DGS) have made significant progress in scene reconstruction and
novel view synthesis. However, they heavily rely on preprocessed camera poses
and 3D structural priors from structure-from-motion (SfM), which are
challenging to obtain in outdoor scenarios. To address this challenge, we
propose to incorporate Iterative Closest Point (ICP) with optimization-based
refinement to achieve accurate camera pose estimation under large camera
movements. Additionally, we introduce a voxel-based scene densification
approach to guide the reconstruction in large-scale scenes. Experiments
demonstrate that our approach ICP-3DGS outperforms existing methods in both
camera pose estimation and novel view synthesis across indoor and outdoor
scenes of various scales. Source code is available at
https://github.com/Chenhao-Z/ICP-3DGS.

</details>


### [68] [SkinningGS: Editable Dynamic Human Scene Reconstruction Using Gaussian Splatting Based on a Skinning Model](https://arxiv.org/abs/2506.21632)
*Da Li,Donggang Jia,Markus Hadwiger,Ivan Viola*

Main category: cs.GR

TL;DR: 一种新的单目视频人体和背景重建方法，通过点云解耦和联合优化，实现了比HUGS更高质量、更高效率和实时渲染，并能扩展到动物重建。


<details>
  <summary>Details</summary>
Motivation: 从单目视频中重建交互式人体形象和背景极具挑战性，旨在实现背景和人体的解耦重建同时保持人体运动的交互性。

Method: 采用点云解耦和联合优化的策略，通过引入位置纹理细分SMPL模型表面并扩展人体点云。结合卷积神经网络预测人体点云特征，从而避免了密化超参数调优，并高效表示人体点。

Result: 实现了高质量的人体重建，训练期间GPU资源消耗更少，重建指标超越了HUGS，并能泛化到新的姿态和视角。实现了超过100 FPS的实时渲染速度，是HUGS的约6倍。该框架还可以扩展到动物场景重建。

Conclusion: 本文提出的方法可以有效地从单目视频中重建交互式人类形象和背景，在质量、效率和实时性能方面优于现有的最新技术，并具有扩展到动物重建的潜力。

Abstract: Reconstructing an interactive human avatar and the background from a
monocular video of a dynamic human scene is highly challenging. In this work we
adopt a strategy of point cloud decoupling and joint optimization to achieve
the decoupled reconstruction of backgrounds and human bodies while preserving
the interactivity of human motion. We introduce a position texture to subdivide
the Skinned Multi-Person Linear (SMPL) body model's surface and grow the human
point cloud. To capture fine details of human dynamics and deformations, we
incorporate a convolutional neural network structure to predict human body
point cloud features based on texture. This strategy makes our approach free of
hyperparameter tuning for densification and efficiently represents human points
with half the point cloud of HUGS. This approach ensures high-quality human
reconstruction and reduces GPU resource consumption during training. As a
result, our method surpasses the previous state-of-the-art HUGS in
reconstruction metrics while maintaining the ability to generalize to novel
poses and views. Furthermore, our technique achieves real-time rendering at
over 100 FPS, $\sim$6$\times$ the HUGS speed using only Linear Blend Skinning
(LBS) weights for human transformation. Additionally, this work demonstrates
that this framework can be extended to animal scene reconstruction when an
accurately-posed model of an animal is available.

</details>


### [69] [SAR-GS: 3D Gaussian Splatting for Synthetic Aperture Radar Target Reconstruction](https://arxiv.org/abs/2506.21633)
*Aobo Li,Zhengxin Lei,Jiangtao Wei,Feng Xu*

Main category: cs.GR

TL;DR: 提出了一种基于高斯溅射的新型SAR三维目标重建方法（SDGR），通过可微分光栅器和自定义CUDA梯度流，有效重建了模拟和真实SAR数据中的目标几何结构和散射特性。


<details>
  <summary>Details</summary>
Motivation: 合成孔径雷达（SAR）图像的三维目标重建对于解释复杂的SAR散射信息至关重要，但SAR成像固有的复杂电磁散射机制带来了显著的重建挑战。

Method: 提出了一种新颖的SAR可微分高斯溅射光栅器（SDGR），专门用于SAR目标重建。该方法将高斯溅射与映射投影算法结合，计算高斯基元的散射强度并生成模拟SAR图像。通过计算渲染图像与真实图像之间的损失函数来优化表示场景的高斯基元参数，并采用自定义CUDA梯度流代替自动微分以加速梯度计算。

Result: 在模拟SAR图像上验证了SDGR的成像合理性，并通过模拟和真实数据集上的多车辆目标证明了该方法对目标重建的有效性。

Conclusion: 该方法能够有效重建目标的几何结构和散射特性，为SAR成像领域的三维重建提供了一种新颖的解决方案。

Abstract: Three-dimensional target reconstruction from synthetic aperture radar (SAR)
imagery is crucial for interpreting complex scattering information in SAR data.
However, the intricate electromagnetic scattering mechanisms inherent to SAR
imaging pose significant reconstruction challenges. Inspired by the remarkable
success of 3D Gaussian Splatting (3D-GS) in optical domain reconstruction, this
paper presents a novel SAR Differentiable Gaussian Splatting Rasterizer (SDGR)
specifically designed for SAR target reconstruction. Our approach combines
Gaussian splatting with the Mapping and Projection Algorithm to compute
scattering intensities of Gaussian primitives and generate simulated SAR images
through SDGR. Subsequently, the loss function between the rendered image and
the ground truth image is computed to optimize the Gaussian primitive
parameters representing the scene, while a custom CUDA gradient flow is
employed to replace automatic differentiation for accelerated gradient
computation. Through experiments involving the rendering of simplified
architectural targets and SAR images of multiple vehicle targets, we validate
the imaging rationality of SDGR on simulated SAR imagery. Furthermore, the
effectiveness of our method for target reconstruction is demonstrated on both
simulated and real-world datasets containing multiple vehicle targets, with
quantitative evaluations conducted to assess its reconstruction performance.
Experimental results indicate that our approach can effectively reconstruct the
geometric structures and scattering properties of targets, thereby providing a
novel solution for 3D reconstruction in the field of SAR imaging.

</details>


<div id='cs.LG'></div>

# cs.LG [[Back]](#toc)

### [70] [APO: Enhancing Reasoning Ability of MLLMs via Asymmetric Policy Optimization](https://arxiv.org/abs/2506.21655)
*Minjie Hong,Zirun Guo,Yan Xia,Zehan Wang,Ziang Zhang,Tao Jin,Zhou Zhao*

Main category: cs.LG

TL;DR: MLLMs在复杂推理上表现不佳。本文提出非对称策略优化（APO），包含难度自适应散度塑形（DADS）和次优轨迹复杂度正则化（STCR），以提高推理能力，避免过度思考，并保持通用任务性能。基于此，View-R1-3B在推理上表现出色，甚至超越了更大的模型，同时保持了良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在整合不同数据方面表现出色，但往往难以进行复杂推理。强化学习（RL）可以提升大语言模型（LLMs）的推理能力，但应用于MLLMs时存在挑战，常见问题包括在通用任务上性能下降以及生成过度详细或“过度思考”的推理。本文研究了KL惩罚和过度思考对MLLMs中RL训练的影响。

Method: 提出非对称策略优化（APO），将采样响应分为正负两组。对正样本引入难度自适应散度塑形（DADS），根据难度动态调整KL散度权重，以防止策略熵急剧下降、提高训练稳定性并保留模型知识。对负样本提出次优轨迹复杂度正则化（STCR），惩罚过长响应以减轻过度思考并鼓励简洁推理，同时保留模型的探索能力。将该方法应用于Qwen2.5-VL-3B，创建了View-R1-3B。

Result: View-R1-3B显著增强了推理能力，比基础模型平均提升7%，并在各种推理基准上优于更大的MLLMs（7-11B）。与其它推理微调的MLLMs不同，View-R1-3B在通用任务上保持持续改进，显示出卓越的泛化能力。

Conclusion: DADS和STCR技术在提升多模态大语言模型（MLLMs）的复杂多模态推理能力方面有效且适用范围广。

Abstract: Multimodal Large Language Models (MLLMs) are powerful at integrating diverse
data, but they often struggle with complex reasoning. While Reinforcement
learning (RL) can boost reasoning in LLMs, applying it to MLLMs is tricky.
Common issues include a drop in performance on general tasks and the generation
of overly detailed or "overthinking" reasoning. Our work investigates how the
KL penalty and overthinking affect RL training in MLLMs. We propose Asymmetric
Policy Optimization (APO) to address these issues, which divides the sampled
responses into positive and negative groups. For positive samples,
Difficulty-Adaptive Divergence Shaping (DADS) is introduced to dynamically
adjust the KL divergence weight based on their difficulty. This method prevents
policy entropy from dropping sharply, improves training stability, utilizes
samples better, and preserves the model's existing knowledge. For negative
samples, Suboptimal Trajectory Complexity Regularization (STCR) is proposed to
penalize overly long responses. This helps mitigate overthinking and encourages
more concise reasoning while preserving the model's explorative capacity. We
apply our method to Qwen2.5-VL-3B, creating View-R1-3B. View-R1-3B
significantly enhances reasoning capabilities, showing an average 7\% gain over
the base model and outperforming larger MLLMs (7-11B) on various reasoning
benchmarks. Importantly, unlike other reasoning-tuned MLLMs that often degrade
on general tasks, View-R1-3B maintains consistent improvement, demonstrating
superior generalization. These results highlight the effectiveness and broad
applicability of our DADS and STCR techniques for advancing complex multimodal
reasoning in MLLMs. The code will be made available at
https://github.com/Indolent-Kawhi/View-R1.

</details>


### [71] [Risk-Averse Total-Reward Reinforcement Learning](https://arxiv.org/abs/2506.21683)
*Xihong Su,Jia Lin Hau,Gersi Doko,Kishan Panaganti,Marek Petrik*

Main category: cs.LG

TL;DR: 针对风险厌恶型MDPs中的ERM和EVaR目标，本文提出了一种Q学习算法，克服了现有基于模型方法的局限性，并展现出强大的收敛性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于模型的风险度量（如ERM和EVaR）算法在小规模问题中有效，但需要完全访问转移概率，这在大型问题中是受限的。

Method: 本文提出了一种Q学习算法，用于计算总回报ERM和EVaR目标的最优平稳策略。

Result: 在表格域上的数值结果表明，所提出的Q学习算法能够快速可靠地收敛到最优风险厌恶值函数。

Conclusion: 本文提出了一种Q学习算法，能够有效计算风险厌恶型马尔可夫决策过程（MDPs）中ERM和EVaR目标的最优平稳策略，并具有强大的收敛性和性能保证。

Abstract: Risk-averse total-reward Markov Decision Processes (MDPs) offer a promising
framework for modeling and solving undiscounted infinite-horizon objectives.
Existing model-based algorithms for risk measures like the entropic risk
measure (ERM) and entropic value-at-risk (EVaR) are effective in small
problems, but require full access to transition probabilities. We propose a
Q-learning algorithm to compute the optimal stationary policy for total-reward
ERM and EVaR objectives with strong convergence and performance guarantees. The
algorithm and its optimality are made possible by ERM's dynamic consistency and
elicitability. Our numerical results on tabular domains demonstrate quick and
reliable convergence of the proposed Q-learning algorithm to the optimal
risk-averse value function.

</details>


### [72] [Unimodal Strategies in Density-Based Clustering](https://arxiv.org/abs/2506.21695)
*Oron Nir,Jay Tenenbaum,Ariel Shamir*

Main category: cs.LG

TL;DR: 该研究发现密度聚类中聚类数量与邻域半径之间存在近乎单峰的关系，并基于此提出了一种利用三分搜索算法高效寻找最佳半径值的新策略，解决了大规模高维数据参数调整计算量大的问题，并在多领域验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 密度聚类方法在处理真实世界中带有噪声或任意分布的数据时优于基于中心的方法，但参数调整（特别是对高维大规模数据）计算量大。

Method: 揭示了密度聚类方法中聚类数量与核心点邻域半径之间的近乎单峰的关键特性，并利用此特性基于三分搜索算法设计了更高效的半径值寻找策略。

Result: 通过在各种高维、大规模NLP、音频和计算机视觉任务上的广泛应用，验证了该方法的实用性和鲁棒性。

Conclusion: 这项工作不仅在密度聚类参数控制方面取得了显著进展，而且拓宽了对其指导参数之间关系的理解。

Abstract: Density-based clustering methods often surpass centroid-based counterparts,
when addressing data with noise or arbitrary data distributions common in
real-world problems. In this study, we reveal a key property intrinsic to
density-based clustering methods regarding the relation between the number of
clusters and the neighborhood radius of core points - we empirically show that
it is nearly unimodal, and support this claim theoretically in a specific
setting. We leverage this property to devise new strategies for finding
appropriate values for the radius more efficiently based on the Ternary Search
algorithm. This is especially important for large scale data that is
high-dimensional, where parameter tuning is computationally intensive. We
validate our methodology through extensive applications across a range of
high-dimensional, large-scale NLP, Audio, and Computer Vision tasks,
demonstrating its practical effectiveness and robustness. This work not only
offers a significant advancement in parameter control for density-based
clustering but also broadens the understanding regarding the relations between
their guiding parameters. Our code is available at
https://github.com/oronnir/UnimodalStrategies.

</details>


### [73] [$\textrm{ODE}_t \left(\textrm{ODE}_l \right)$: Shortcutting the Time and Length in Diffusion and Flow Models for Faster Sampling](https://arxiv.org/abs/2506.21714)
*Denis Gudovskiy,Wenzhao Zheng,Tomoyuki Okuno,Yohei Nakata,Kurt Keutzer*

Main category: cs.LG

TL;DR: 该论文提出了一种新的$	extrm{ODE}_t 	extrm{ODE}_l$ 方法，通过同时控制时间步长和网络深度来动态平衡连续归一化流和扩散模型的质量与复杂度，从而显著降低采样延迟并提高图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有连续归一化流（CNFs）和扩散模型（DMs）的采样过程计算成本高昂，需要多次迭代解决常微分方程（ODE），且多数方法仅关注减少时间步长以提高效率。

Method: 通过重新连接基于Transformer架构中的模块来解决其长度上的内部离散ODE，并在流匹配训练期间采用时间维度和长度维度的一致性项，使得采样可以使用任意数量的时间步和Transformer块进行。

Result: 与现有技术相比，在CelebA-HQ和ImageNet上的图像生成实验表明，在最有效的采样模式下，延迟降低了高达3倍，高质量采样的FID分数提高了高达3.5点，同时降低了延迟和内存使用。

Conclusion: 该研究提出了一种新颖的$	extrm{ODE}_t 	extrm{ODE}_l$ 方法，通过在时间和网络长度维度上动态控制质量-复杂度权衡，显著降低了采样延迟并提高了生成质量，为连续归一化流和扩散模型的效率和性能带来了改进。

Abstract: Recently, continuous normalizing flows (CNFs) and diffusion models (DMs) have
been studied using the unified theoretical framework. Although such models can
generate high-quality data points from a noise distribution, the sampling
demands multiple iterations to solve an ordinary differential equation (ODE)
with high computational complexity. Most existing methods focus on reducing the
number of time steps during the sampling process to improve efficiency. In this
work, we explore a complementary direction in which the quality-complexity
tradeoff can be dynamically controlled in terms of time steps and in the length
of the neural network. We achieve this by rewiring the blocks in the
transformer-based architecture to solve an inner discretized ODE w.r.t. its
length. Then, we employ time- and length-wise consistency terms during flow
matching training, and as a result, the sampling can be performed with an
arbitrary number of time steps and transformer blocks. Unlike others, our
$\textrm{ODE}_t \left(\textrm{ODE}_l \right)$ approach is solver-agnostic in
time dimension and decreases both latency and memory usage. Compared to the
previous state of the art, image generation experiments on CelebA-HQ and
ImageNet show a latency reduction of up to $3\times$ in the most efficient
sampling mode, and a FID score improvement of up to $3.5$ points for
high-quality sampling. We release our code and model weights with fully
reproducible experiments.

</details>


### [74] [Performance Prediction for Large Systems via Text-to-Text Regression](https://arxiv.org/abs/2506.21718)
*Yash Akhauri,Bryan Lewandowski,Cheng-Hsi Lin,Adrian N. Reyes,Grant C. Forbes,Arissa Wongpanich,Bangding Yang,Mohamed S. Abdelfattah,Sagi Perel,Xingyou Song*

Main category: cs.LG

TL;DR: 针对传统回归方法在复杂系统数据上的局限性，本文提出了一种文本到文本的回归方法，使用60M参数的编码器-解码器模型，在预测资源效率上取得了显著优于传统方法的性能，并能很好地适应新任务。


<details>
  <summary>Details</summary>
Motivation: 传统的表格回归方法在处理复杂的系统数据（如配置文件或系统日志）时面临困难，因为特征工程通常不可行。

Method: 提出了文本到文本回归作为一种通用、可扩展的替代方案，具体是一个60M参数的编码器-解码器模型。

Result: 该模型在Google的Borg系统上预测资源效率，达到了近乎完美的0.99（平均0.9）的等级相关性，并且MSE比表格方法低100倍。该模型还可以通过500个少量样本轻松适应新任务，并捕获复杂结果分布的密度。消融研究强调了使用编码器、增加序列长度以及模型固有的不确定性量化的重要性。

Conclusion: 这些发现为现实世界的通用模拟器铺平了道路。

Abstract: In many industries, predicting metric outcomes of large systems is a
fundamental problem, driven largely by traditional tabular regression. However,
such methods struggle on complex systems data in the wild such as configuration
files or system logs, where feature engineering is often infeasible. We propose
text-to-text regression as a general, scalable alternative. For predicting
resource efficiency on Borg, Google's massive compute cluster scheduling
system, a 60M parameter encoder-decoder, trained from random initialization,
achieves up to a near perfect 0.99 (0.9 average) rank correlation across the
entire fleet, and 100x lower MSE than tabular approaches. The model also easily
adapts to new tasks in only 500 few-shot examples and captures the densities of
complex outcome distributions. Ablation studies highlight the importance of
using encoders, increasing sequence length, and the model's inherent
uncertainty quantification. These findings pave the way for universal
simulators of real-world outcomes.

</details>


### [75] [Federated Item Response Theory Models](https://arxiv.org/abs/2506.21744)
*Biying Zhou,Nanyu Luo,Feng Ji*

Main category: cs.LG

TL;DR: 传统的IRT模型在集中式数据处理中存在隐私问题。本文提出了FedIRT框架，它利用联邦学习在分布式环境中进行IRT模型估计，实现了与传统方法相似的准确性，同时保护了隐私并降低了通信成本。


<details>
  <summary>Details</summary>
Motivation: 传统的项目反应理论（IRT）估计需要将所有个人原始响应数据集中在一个地方，这可能导致隐私问题。

Method: 我们提出了一个新颖的框架——联邦项目反应理论（FedIRT），它通过集成联邦学习的优势，实现了在不损失估计准确性的前提下，以分布式方式估计传统IRT模型，并额外增加了隐私保护功能。

Result: 我们的数值实验证实，FedIRT在统计准确性上与使用流行R包的标准IRT估计相似，同时提供了关键优势：隐私保护和降低通信成本。通过真实世界的考试数据集验证了FedIRT的实用性，证明了其在现实教育环境中的有效性。

Conclusion: FedIRT框架扩展了IRT在分布式设置（如多校评估）中的适用性，且不牺牲准确性或安全性。为支持实际应用，我们提供了开源R软件包FedIRT，实现了双参数逻辑（2PL）和部分信用模型（PCM）的框架。

Abstract: Item Response Theory (IRT) models have been widely used to estimate
respondents' latent abilities and calibrate items' difficulty. Traditional IRT
estimation requires all individual raw response data to be centralized in one
place, thus potentially causing privacy issues. Federated learning is an
emerging field in computer science and machine learning with added features of
privacy protection and distributed computing. To integrate the advances from
federated learning with modern psychometrics, we propose a novel framework,
Federated Item Response Theory (IRT), to enable estimating traditional IRT
models with additional privacy, allowing estimation in a distributed manner
without losing estimation accuracy.
  Our numerical experiments confirm that FedIRT achieves statistical accuracy
similar to standard IRT estimation using popular R packages, while offering
critical advantages: privacy protection and reduced communication costs. We
also validate FedIRT's utility through a real-world exam dataset, demonstrating
its effectiveness in realistic educational contexts. This new framework extends
IRT's applicability to distributed settings, such as multi-school assessments,
without sacrificing accuracy or security. To support practical adoption, we
provide an open-ource R package, FedIRT, implementing the framework for the
two-parameter logistic (2PL) and partial credit models (PCM).

</details>


### [76] [Gradient-Based Neuroplastic Adaptation for Concurrent Optimization of Neuro-Fuzzy Networks](https://arxiv.org/abs/2506.21771)
*John Wesley Hostetter,Min Chi*

Main category: cs.LG

TL;DR: 提出了一种新的梯度神经可塑性适应方法，用于同时优化神经模糊网络的参数和结构，使其能够应用于在线强化学习等基于视觉的任务，并通过在DOOM游戏中进行验证。


<details>
  <summary>Details</summary>
Motivation: 神经模糊网络（NFN）虽然具有透明、符号化和通用的函数逼近能力，并且性能与传统神经网络相当，但其系统设计过程仍然是一个挑战。现有工作通常通过低效地隔离参数和结构识别来顺序构建NFN，导致过早地确定脆性且次优的架构。

Method: 我们提出了一种新颖的、与应用无关的方法，称为基于梯度的神经可塑性适应，用于同时优化神经模糊网络的参数和结构。

Result: 通过同时优化NFN的参数和结构，以前NFN无法解决的设置现在变得可行，例如NFN在基于视觉任务中的在线强化学习。通过在线强化学习对NFN进行训练，使其能够熟练地玩名为DOOM的基于视觉的视频游戏中的挑战场景，从而凭经验证明了同时优化NFN的有效性。

Conclusion: 本文通过在基于视觉的任务中，如在线强化学习，展示了神经模糊网络在DOOM游戏中的应用，证明了同时优化神经模糊网络的有效性，从而使其能够应用于以前无法解决的场景。

Abstract: Neuro-fuzzy networks (NFNs) are transparent, symbolic, and universal function
approximations that perform as well as conventional neural architectures, but
their knowledge is expressed as linguistic IF-THEN rules. Despite these
advantages, their systematic design process remains a challenge. Existing work
will often sequentially build NFNs by inefficiently isolating parametric and
structural identification, leading to a premature commitment to brittle and
subpar architecture. We propose a novel application-independent approach called
gradient-based neuroplastic adaptation for the concurrent optimization of NFNs'
parameters and structure. By recognizing that NFNs' parameters and structure
should be optimized simultaneously as they are deeply conjoined, settings
previously unapproachable for NFNs are now accessible, such as the online
reinforcement learning of NFNs for vision-based tasks. The effectiveness of
concurrently optimizing NFNs is empirically shown as it is trained by online
reinforcement learning to proficiently play challenging scenarios from a
vision-based video game called DOOM.

</details>


### [77] [M3PO: Massively Multi-Task Model-Based Policy Optimization](https://arxiv.org/abs/2506.21782)
*Aditya Narendra,Dmitry Makarov,Aleksandr Panov*

Main category: cs.LG

TL;DR: M3PO是一个新的基于模型的强化学习框架，它通过结合隐式世界模型和混合探索策略，提高了多任务环境下的样本效率和泛化能力，并取得了最先进的成果。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法存在问题：单任务设置中样本效率低下，多任务领域中泛化能力差。具体来说，像DreamerV3这样的基于模型的方法忽视了以控制为中心的表示，而像PPO这样的模型无关方法则面临高样本复杂性和弱探索的问题。

Method: M3PO整合了一个隐式世界模型（无需观测重建即可预测任务结果）和一种混合探索策略（结合了基于模型的规划和基于不确定性的模型无关奖励）。它利用基于模型和模型无关的价值估计之间的差异来指导探索，并通过信任域优化器维持稳定的策略更新。

Result: M3PO消除了先前方法中的偏差-方差权衡，保持了策略更新的稳定性，提供了一种高效且稳健的替代方案，并在多个基准测试中达到了最先进的性能。

Conclusion: M3PO是一种可扩展的基于模型的强化学习框架，它提供了一种高效、稳健的替代方案，并克服了现有方法的局限性，在多个基准测试中取得了最先进的性能。

Abstract: We introduce Massively Multi-Task Model-Based Policy Optimization (M3PO), a
scalable model-based reinforcement learning (MBRL) framework designed to
address sample inefficiency in single-task settings and poor generalization in
multi-task domains. Existing model-based approaches like DreamerV3 rely on
pixel-level generative models that neglect control-centric representations,
while model-free methods such as PPO suffer from high sample complexity and
weak exploration. M3PO integrates an implicit world model, trained to predict
task outcomes without observation reconstruction, with a hybrid exploration
strategy that combines model-based planning and model-free uncertainty-driven
bonuses. This eliminates the bias-variance trade-off in prior methods by using
discrepancies between model-based and model-free value estimates to guide
exploration, while maintaining stable policy updates through a trust-region
optimizer. M3PO provides an efficient and robust alternative to existing
model-based policy optimization approaches and achieves state-of-the-art
performance across multiple benchmarks.

</details>


### [78] [Multi-task parallelism for robust pre-training of graph foundation models on multi-source, multi-fidelity atomistic modeling data](https://arxiv.org/abs/2506.21788)
*Massimiliano Lupo Pasini,Jong Youl Choi,Pei Zhang,Kshitij Mehta,Rylie Weaver,Ashwin M. Aji,Karl W. Schulz,Jorda Polo,Prasanna Balaprakash*

Main category: cs.LG

TL;DR: 本文提出了一种多任务并行方法HydraGNN，解决了图神经网络在处理大规模多样化数据时的可扩展性问题，并在超级计算机上实现了高效扩展。


<details>
  <summary>Details</summary>
Motivation: 图神经网络的图基础模型在预训练过程中处理多源、多保真度数据时面临挑战，现有方法在更大、更多样化的数据集上的泛化性和在超级计算机上的可扩展性存在疑问。

Method: 提出了一种多任务并行方法，将每个头部任务分布到具有GPU加速的计算资源上。该方法在开源的HydraGNN架构中实现。

Result: 该方法在超过2400万个结构上进行了训练，并在Perlmutter、Aurora和Frontier超级计算机上进行了测试，展示了在所有三种高度异构的超级计算架构上的高效扩展性。

Conclusion: 本文提出的多任务并行方法成功解决了图基础模型在多源、多保真度数据集上的可扩展性和泛化性问题，并在多种异构超级计算架构上实现了高效扩展。

Abstract: Graph foundation models using graph neural networks promise sustainable,
efficient atomistic modeling. To tackle challenges of processing multi-source,
multi-fidelity data during pre-training, recent studies employ multi-task
learning, in which shared message passing layers initially process input
atomistic structures regardless of source, then route them to multiple decoding
heads that predict data-specific outputs. This approach stabilizes pre-training
and enhances a model's transferability to unexplored chemical regions.
Preliminary results on approximately four million structures are encouraging,
yet questions remain about generalizability to larger, more diverse datasets
and scalability on supercomputers. We propose a multi-task parallelism method
that distributes each head across computing resources with GPU acceleration.
Implemented in the open-source HydraGNN architecture, our method was trained on
over 24 million structures from five datasets and tested on the Perlmutter,
Aurora, and Frontier supercomputers, demonstrating efficient scaling on all
three highly heterogeneous super-computing architectures.

</details>


### [79] [Why Neural Network Can Discover Symbolic Structures with Gradient-based Training: An Algebraic and Geometric Foundation for Neurosymbolic Reasoning](https://arxiv.org/abs/2506.21797)
*Peihao Wang,Zhangyang Wang*

Main category: cs.LG

TL;DR: 一个理论框架，解释了离散符号结构如何从连续神经网络训练中产生。通过将参数提升到测度空间并建模为Wasserstein梯度流，揭示了在群不变性下，训练导致梯度流解耦和自由度收缩，最终形成符合代数运算的组合表示。该框架为神经符号系统提供了基础，并建立了数据缩放定律。


<details>
  <summary>Details</summary>
Motivation: 解释离散符号结构如何从连续神经网络训练动态中自然产生。

Method: 开发了一个理论框架，通过将神经参数提升到测度空间并将训练建模为Wasserstein梯度流。在几何约束（如群不变性）下进行分析。

Result: 在几何约束下，参数测度μ_t会经历两个并发现象：(1)梯度流解耦为独立优化轨迹，(2)自由度逐渐收缩。这些势能编码与任务相关的代数约束。训练过程中，网络从高维探索转向符合代数运算的组合表示。进一步建立了实现符号任务的数据缩放定律。

Conclusion: 该框架为理解和设计集成连续学习与离散代数推理的神经符号系统奠定了原则性基础。

Abstract: We develop a theoretical framework that explains how discrete symbolic
structures can emerge naturally from continuous neural network training
dynamics. By lifting neural parameters to a measure space and modeling training
as Wasserstein gradient flow, we show that under geometric constraints, such as
group invariance, the parameter measure $\mu_t$ undergoes two concurrent
phenomena: (1) a decoupling of the gradient flow into independent optimization
trajectories over some potential functions, and (2) a progressive contraction
on the degree of freedom. These potentials encode algebraic constraints
relevant to the task and act as ring homomorphisms under a commutative
semi-ring structure on the measure space. As training progresses, the network
transitions from a high-dimensional exploration to compositional
representations that comply with algebraic operations and exhibit a lower
degree of freedom. We further establish data scaling laws for realizing
symbolic tasks, linking representational capacity to the group invariance that
facilitates symbolic solutions. This framework charts a principled foundation
for understanding and designing neurosymbolic systems that integrate continuous
learning with discrete algebraic reasoning.

</details>


### [80] [The Cost of Avoiding Backpropagation](https://arxiv.org/abs/2506.21833)
*Kunjal Panchal,Sunav Choudhary,Yuriy Brun,Hui Guan*

Main category: cs.LG

TL;DR: 在内存受限的深度学习中，带有检查点的反向传播（BP）优于前向模式自动微分（FmAD）和零阶（ZO）方法。


<details>
  <summary>Details</summary>
Motivation: 正向模式自动微分（FmAD）和零阶（ZO）优化被认为是反向传播（BP）的内存高效替代方案，但由于缺乏与内存高效BP变体的比较以及缺乏统一的理论分析，其实际益处仍不明确。

Method: 本文对反向传播（BP）、前向模式自动微分（FmAD）和零阶（ZO）方法进行了全面的理论和实证比较。

Result: 理论分析表明，FmAD和ZO虽然可以减少内存使用，但与带有检查点的BP相比，它们在精度、收敛速度和计算方面会产生显著的成本。实证实验表明，在内存使用相当的情况下，带有检查点的BP在大型语言和视觉-语言模型上表现优于FmAD和ZO变体，精度提高高达31.1%，收敛速度提高34.8%，计算量减少3.8倍。

Conclusion: 在内存受限的设置下，带有检查点的反向传播（BP）是模型训练最有效的策略。

Abstract: Forward-mode automatic differentiation (FmAD) and zero-order (ZO)
optimization have been proposed as memory-efficient alternatives to
backpropagation (BP) for gradient computation, especially in low-resource
settings. However, their practical benefits remain unclear due to two key gaps:
a lack of comparison against memory-efficient BP variants, such as activation
checkpointing, and a lack of a unified theoretical analysis. This work presents
a comprehensive theoretical and empirical comparison of BP, FmAD, and ZO
methods. Our theoretical analysis shows that while FmAD, and ZO can reduce
memory usage, they incur significant costs in accuracy, convergence speed, and
computation compared to BP with checkpointing. These drawbacks worsen with
larger models or constrained perturbation budgets. Empirical experiments on
large language and vision-language models show that BP with checkpointing
outperforms FmAD and ZO variants, including those enhanced with variance
reduction, achieving up to 31.1% higher accuracy, 34.8% faster convergence, and
3.8x fewer computations at comparable memory usage. Our results highlight
fundamental limitations of FmAD and ZO, and reaffirm BP with checkpointing as
the most effective strategy for model training under memory-constrained
settings. Our code is available at
https://github.com/Astuary/The_Cost_of_Avoiding_Backpropagation.

</details>


### [81] [APO: Enhancing Reasoning Ability of MLLMs via Asymmetric Policy Optimization](https://arxiv.org/abs/2506.21655)
*Minjie Hong,Zirun Guo,Yan Xia,Zehan Wang,Ziang Zhang,Tao Jin,Zhou Zhao*

Main category: cs.LG

TL;DR: 本文提出非对称策略优化（APO），包含DADS和STCR，以解决多模态大语言模型（MLLMs）在强化学习训练中复杂推理的挑战。新模型View-R1-3B在推理上表现出色，同时保持通用任务性能，优于现有大型模型。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在整合不同数据方面表现强大，但在复杂推理方面存在困难。强化学习（RL）虽能提升LLMs的推理能力，但应用于MLLMs时面临通用任务性能下降和生成“过度思考”推理等问题。

Method: 提出了非对称策略优化（APO），该方法将采样响应分为正负两组。对于正样本，引入了难度自适应散度塑形（DADS）来动态调整KL散度权重。对于负样本，提出了次优轨迹复杂度正则化（STCR）来惩罚过长的响应。

Result: 将所提出的方法应用于Qwen2.5-VL-3B，创建了View-R1-3B。View-R1-3B显著增强了推理能力，比基础模型平均提升7%，并在各种推理基准上优于更大的MLLMs（7-11B）。View-R1-3B在通用任务上保持了一致的改进，没有性能下降。

Conclusion: DADS和STCR技术对于推进多模态大语言模型（MLLMs）中的复杂多模态推理是有效且具有广泛适用性的。

Abstract: Multimodal Large Language Models (MLLMs) are powerful at integrating diverse
data, but they often struggle with complex reasoning. While Reinforcement
learning (RL) can boost reasoning in LLMs, applying it to MLLMs is tricky.
Common issues include a drop in performance on general tasks and the generation
of overly detailed or "overthinking" reasoning. Our work investigates how the
KL penalty and overthinking affect RL training in MLLMs. We propose Asymmetric
Policy Optimization (APO) to address these issues, which divides the sampled
responses into positive and negative groups. For positive samples,
Difficulty-Adaptive Divergence Shaping (DADS) is introduced to dynamically
adjust the KL divergence weight based on their difficulty. This method prevents
policy entropy from dropping sharply, improves training stability, utilizes
samples better, and preserves the model's existing knowledge. For negative
samples, Suboptimal Trajectory Complexity Regularization (STCR) is proposed to
penalize overly long responses. This helps mitigate overthinking and encourages
more concise reasoning while preserving the model's explorative capacity. We
apply our method to Qwen2.5-VL-3B, creating View-R1-3B. View-R1-3B
significantly enhances reasoning capabilities, showing an average 7\% gain over
the base model and outperforming larger MLLMs (7-11B) on various reasoning
benchmarks. Importantly, unlike other reasoning-tuned MLLMs that often degrade
on general tasks, View-R1-3B maintains consistent improvement, demonstrating
superior generalization. These results highlight the effectiveness and broad
applicability of our DADS and STCR techniques for advancing complex multimodal
reasoning in MLLMs. The code will be made available at
https://github.com/Indolent-Kawhi/View-R1.

</details>


### [82] [Risk-Averse Total-Reward Reinforcement Learning](https://arxiv.org/abs/2506.21683)
*Xihong Su,Jia Lin Hau,Gersi Doko,Kishan Panaganti,Marek Petrik*

Main category: cs.LG

TL;DR: 针对风险规避总回报马尔可夫决策过程（MDPs），提出了一种Q-learning算法，用于计算熵风险度量（ERM）和熵风险价值（EVaR）的最优策略，并展示了快速可靠的收敛性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于模型的风险度量算法（如ERM和EVaR）在小问题中有效，但需要完全访问转移概率。

Method: 提出了一种Q-learning算法，用于计算总回报ERM和EVaR目标的最优平稳策略。

Result: 在表格域上的数值结果表明，所提出的Q-learning算法能够快速可靠地收敛到最优风险规避价值函数。

Conclusion: ERM的动态一致性和可提取性使得该Q-learning算法及其最优性成为可能，并具有强大的收敛性和性能保证。

Abstract: Risk-averse total-reward Markov Decision Processes (MDPs) offer a promising
framework for modeling and solving undiscounted infinite-horizon objectives.
Existing model-based algorithms for risk measures like the entropic risk
measure (ERM) and entropic value-at-risk (EVaR) are effective in small
problems, but require full access to transition probabilities. We propose a
Q-learning algorithm to compute the optimal stationary policy for total-reward
ERM and EVaR objectives with strong convergence and performance guarantees. The
algorithm and its optimality are made possible by ERM's dynamic consistency and
elicitability. Our numerical results on tabular domains demonstrate quick and
reliable convergence of the proposed Q-learning algorithm to the optimal
risk-averse value function.

</details>


### [83] [APO: Enhancing Reasoning Ability of MLLMs via Asymmetric Policy Optimization](https://arxiv.org/abs/2506.21655)
*Minjie Hong,Zirun Guo,Yan Xia,Zehan Wang,Ziang Zhang,Tao Jin,Zhou Zhao*

Main category: cs.LG

TL;DR: MLLMs在复杂推理和RL应用中存在困难（性能下降、过度思考）。本论文提出了APO，通过DADS（正样本）和STCR（负样本）来解决这些问题。创建的View-R1-3B在推理能力上显著提升（平均7%），并保持了在通用任务上的性能，超越了更大的MLLMs。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在整合多样化数据方面表现强大，但在复杂推理方面往往力不从心。虽然强化学习（RL）可以提升LLMs的推理能力，但将其应用于MLLMs却很棘手，常见问题包括通用任务性能下降和生成过于详细或“过度思考”的推理。

Method: 提出非对称策略优化（APO），将采样响应分为正负两组。对正样本引入难度自适应散度塑形（DADS），动态调整KL散度权重；对负样本引入次优轨迹复杂度正则化（STCR），惩罚过长的响应。

Result: 将所提出的方法应用于Qwen2.5-VL-3B，创建了View-R1-3B。View-R1-3B显著增强了推理能力，比基础模型平均提升7%，并在各种推理基准测试中优于更大的MLLMs（7-11B）。此外，View-R1-3B在通用任务上保持了一致的改进，表现出卓越的泛化能力。

Conclusion: 本研究结果突出了DADS和STCR技术在推进MLLMs中复杂多模态推理方面的有效性和广泛适用性。

Abstract: Multimodal Large Language Models (MLLMs) are powerful at integrating diverse
data, but they often struggle with complex reasoning. While Reinforcement
learning (RL) can boost reasoning in LLMs, applying it to MLLMs is tricky.
Common issues include a drop in performance on general tasks and the generation
of overly detailed or "overthinking" reasoning. Our work investigates how the
KL penalty and overthinking affect RL training in MLLMs. We propose Asymmetric
Policy Optimization (APO) to address these issues, which divides the sampled
responses into positive and negative groups. For positive samples,
Difficulty-Adaptive Divergence Shaping (DADS) is introduced to dynamically
adjust the KL divergence weight based on their difficulty. This method prevents
policy entropy from dropping sharply, improves training stability, utilizes
samples better, and preserves the model's existing knowledge. For negative
samples, Suboptimal Trajectory Complexity Regularization (STCR) is proposed to
penalize overly long responses. This helps mitigate overthinking and encourages
more concise reasoning while preserving the model's explorative capacity. We
apply our method to Qwen2.5-VL-3B, creating View-R1-3B. View-R1-3B
significantly enhances reasoning capabilities, showing an average 7\% gain over
the base model and outperforming larger MLLMs (7-11B) on various reasoning
benchmarks. Importantly, unlike other reasoning-tuned MLLMs that often degrade
on general tasks, View-R1-3B maintains consistent improvement, demonstrating
superior generalization. These results highlight the effectiveness and broad
applicability of our DADS and STCR techniques for advancing complex multimodal
reasoning in MLLMs. The code will be made available at
https://github.com/Indolent-Kawhi/View-R1.

</details>


### [84] [Risk-Averse Total-Reward Reinforcement Learning](https://arxiv.org/abs/2506.21683)
*Xihong Su,Jia Lin Hau,Gersi Doko,Kishan Panaganti,Marek Petrik*

Main category: cs.LG

TL;DR: 本文提出了一种新的Q-learning算法，用于解决使用熵风险度量（ERM）和熵风险价值（EVaR）的风险规避马尔可夫决策过程（MDPs），该算法无需完全访问转移概率，并能快速可靠地收敛。


<details>
  <summary>Details</summary>
Motivation: 现有的基于模型的风险度量（如熵风险度量ERM和熵风险价值EVaR）算法在小问题中有效，但需要完全访问转移概率。这限制了它们在大规模问题中的应用。因此，需要一种无需完全访问转移概率的新算法来解决风险规避的总回报马尔可夫决策过程（MDP）中未贴现的无限 horizon 目标。

Method: 本文提出了一种Q-learning算法，用于计算总回报熵风险度量（ERM）和熵风险价值（EVaR）目标的最优平稳策略。该算法的优化得益于ERM的动态一致性和可引出性。

Result: 数值结果表明，在表格域中，所提出的Q-learning算法能够快速可靠地收敛到最优的风险规避值函数。

Conclusion: 本文提出的Q-learning算法对于风险规避的总回报马尔可夫决策过程（MDP），特别是熵风险度量（ERM）和熵风险价值（EVaR）目标，具有强大的收敛性和性能保证，即使在没有完全访问转移概率的情况下也能有效。其成功核心在于ERM的动态一致性和可引出性。

Abstract: Risk-averse total-reward Markov Decision Processes (MDPs) offer a promising
framework for modeling and solving undiscounted infinite-horizon objectives.
Existing model-based algorithms for risk measures like the entropic risk
measure (ERM) and entropic value-at-risk (EVaR) are effective in small
problems, but require full access to transition probabilities. We propose a
Q-learning algorithm to compute the optimal stationary policy for total-reward
ERM and EVaR objectives with strong convergence and performance guarantees. The
algorithm and its optimality are made possible by ERM's dynamic consistency and
elicitability. Our numerical results on tabular domains demonstrate quick and
reliable convergence of the proposed Q-learning algorithm to the optimal
risk-averse value function.

</details>


### [85] [Unimodal Strategies in Density-Based Clustering](https://arxiv.org/abs/2506.21695)
*Oron Nir,Jay Tenenbaum,Ariel Shamir*

Main category: cs.LG

TL;DR: 针对密度聚类在高维大规模数据中参数调优计算成本高的问题，本研究发现聚类数量与邻域半径之间存在近似单峰关系，并基于此提出了使用三分搜索算法高效寻找半径的新策略，经验证有效且鲁棒。


<details>
  <summary>Details</summary>
Motivation: 密度聚类方法在处理带噪声或任意分布的数据时表现优于基于中心的聚类方法，但在高维大规模数据中，参数（特别是邻域半径）调优计算成本高昂。

Method: 本研究揭示了密度聚类方法中，聚类数量与核心点邻域半径之间存在一个近似单峰的关键特性，并利用此特性，基于三分搜索算法，设计了更高效的半径寻找策略。

Result: 该方法通过对高维、大规模NLP、音频和计算机视觉任务的广泛应用进行验证，证明了其在实际中的有效性和鲁棒性。

Conclusion: 本研究不仅在密度聚类参数控制方面取得了显著进展，而且拓宽了对指导参数之间关系的理解。

Abstract: Density-based clustering methods often surpass centroid-based counterparts,
when addressing data with noise or arbitrary data distributions common in
real-world problems. In this study, we reveal a key property intrinsic to
density-based clustering methods regarding the relation between the number of
clusters and the neighborhood radius of core points - we empirically show that
it is nearly unimodal, and support this claim theoretically in a specific
setting. We leverage this property to devise new strategies for finding
appropriate values for the radius more efficiently based on the Ternary Search
algorithm. This is especially important for large scale data that is
high-dimensional, where parameter tuning is computationally intensive. We
validate our methodology through extensive applications across a range of
high-dimensional, large-scale NLP, Audio, and Computer Vision tasks,
demonstrating its practical effectiveness and robustness. This work not only
offers a significant advancement in parameter control for density-based
clustering but also broadens the understanding regarding the relations between
their guiding parameters. Our code is available at
https://github.com/oronnir/UnimodalStrategies.

</details>


<div id='cs.LO'></div>

# cs.LO [[Back]](#toc)

### [86] [On the role of connectivity in Linear Logic proofs](https://arxiv.org/abs/2506.21678)
*Raffaele Di Donna,Lorenzo Tortora de Falco*

Main category: cs.LO

TL;DR: 通过引入几何条件，将线性逻辑证明结构中一个已知必要性质在MELL的某些片段中转变为充分的正确性判据。


<details>
  <summary>Details</summary>
Motivation: 旨在扩展Danos-Regnier线性逻辑证明结构的正确性判据，特别是解决在MELL中一个已知必要但不充分的性质问题。

Method: 提出了一个关于无类型证明结构的几何条件。

Result: 所提出的几何条件能将该必要性质转变为充分性质，从而可以分离出MELL的几个片段，使得该性质成为正确的判据。同时，也作为副产品恢复了一些已知结果。

Conclusion: 本文通过引入几何条件，成功地将线性逻辑证明结构中一个已知的必要性质提升为MELL（乘法指数线性逻辑）的某些片段的充分正确性判据，从而加深了对线性逻辑证明结构的理解。

Abstract: We investigate a property that extends the Danos-Regnier correctness
criterion for linear logic proof-structures. The property applies to the
correctness graphs of a proof-structure: it states that any such graph is
acyclic and that the number of its connected components is exactly one more
than the number of nodes bottom or weakening. This is known to be necessary but
not sufficient in multiplicative exponential linear logic (MELL) to recover a
sequent calculus proof from a proof-structure. We present a geometric condition
on untyped proof-structures allowing us to turn this necessary property into a
sufficient one: we can thus isolate several fragments of MELL for which this
property is indeed a correctness criterion. We also recover as by-product some
known results.

</details>


### [87] [Negated String Containment is Decidable (Technical Report)](https://arxiv.org/abs/2506.22061)
*Vojtěch Havlena,Michal Hečko,Lukáš Holík,Ondřej Lengál*

Main category: cs.LO

TL;DR: 本文证明了字符串不包含（not-contains）谓词在正则语言约束下是可判定的，解决了长期存在的开放问题。


<details>
  <summary>Details</summary>
Motivation: 解决字符串不包含谓词（not-contains string predicate）的可判定性这一长期存在的开放问题，该谓词在符号执行字符串操作程序中具有实际应用意义。

Method: 通过证明当字符串变量序列受正则语言约束时，notContains(x1 ... xn, y1 ... ym)谓词是可判定的。

Result: 结合无链词方程和正则成员约束的not-contains谓词的可判定性也随之确立。

Conclusion: 成功证明了不包含字符串谓词的可判定性，并进一步扩展到结合无链词方程和正则成员约束的情况。

Abstract: We provide a positive answer to a long-standing open question of the
decidability of the not-contains string predicate. Not-contains is practically
relevant, for instance in symbolic execution of string manipulating programs.
Particularly, we show that the predicate notContains(x1 ... xn, y1 ... ym),
where x1 ... xn and y1 ... ym are sequences of string variables constrained by
regular languages, is decidable. Decidability of a not-contains predicate
combined with chain-free word equations and regular membership constraints
follows.

</details>


### [88] [Wait-Only Broadcast Protocols are Easier to Verify](https://arxiv.org/abs/2506.22144)
*Lucie Guillou,Arnaud Sangnier,Nathalie Sznajder*

Main category: cs.LO

TL;DR: 分析了广播网络中同步和重复可覆盖性问题的复杂性，发现它们在 Wait-Only 协议下分别是 Ackermann-complete 和 EXPSPACE/PSPACE-hard。


<details>
  <summary>Details</summary>
Motivation: 研究具有参数化进程数量的广播网络中的同步问题和重复可覆盖性问题。由于这两个问题在一般情况下是不可判定的，因此研究它们在 Wait-Only 协议下的情况。

Method: 研究当协议为 Wait-Only 时（即没有状态允许进程同时广播和接收消息）这些问题的可判定性。

Result: 同步问题变为 Ackermann-complete；重复可覆盖性问题在 EXPSPACE 中，并且是 PSPACE-hard。

Conclusion: 本文确定了在 Wait-Only 协议下，同步问题和重复可覆盖性问题的复杂性。

Abstract: We study networks of processes that all execute the same finite-state
protocol and communicate via broadcasts. We are interested in two problems with
a parameterized number of processes: the synchronization problem which asks
whether there is an execution which puts all processes on a given state; and
the repeated coverability problem which asks if there is an infinite execution
where a given transition is taken infinitely often. Since both problems are
undecidable in the general case, we investigate those problems when the
protocol is Wait-Only, i.e., it has no state from which a process can both
broadcast and receive messages. We establish that the synchronization problem
becomes Ackermann-complete, and the repeated coverability problem is in
EXPSPACE, and PSPACE-hard.

</details>


### [89] [Scott's Representation Theorem and the Univalent Karoubi Envelope](https://arxiv.org/abs/2506.22196)
*Arnoud van der Leer,Kobe Wullaert,Benedikt Ahrens*

Main category: cs.LO

TL;DR: 本文在univalent foundations中形式化了Scott表示定理，实现了两种证明，并探讨了Karoubi包的作用和λ-项约简的自动化。


<details>
  <summary>Details</summary>
Motivation: 本文旨在univalent foundations中的(Rocq-)UniMath库中形式化无类型λ演算的Scott表示定理，并解释Karoubi包的作用以及所选基础对其构造的影响，同时报告为λ-项约简实现的一些自动化。

Method: 作者在(Rocq-)UniMath库中实现了Scott表示定理的两种证明（Scott和Hyland的证明），并讨论了Karoubi包在证明中的作用以及所选基础对其构造的影响，还实现了λ-项约简的自动化。

Result: 成功在univalent foundations中使用(Rocq-)UniMath库形式化并实现了Scott表示定理的两种证明，解释了Karoubi包的作用以及所选基础对其构造的影响，并提供了λ-项约简的自动化。

Conclusion: 本文在univalent foundations中成功形式化并实现了Scott表示定理，提供了多种证明实现，并讨论了像Karoubi包这样的基础方面。此外，还为λ-项约简做出了自动化贡献。

Abstract: Lambek and Scott constructed a correspondence between simply-typed lambda
calculi and Cartesian closed categories. Scott's Representation Theorem is a
cousin to this result for untyped lambda calculi. It states that every untyped
lambda calculus arises from a reflexive object in some category. We present a
formalization of Scott's Representation Theorem in univalent foundations, in
the (Rocq-)UniMath library. Specifically, we implement two proofs of that
theorem, one by Scott and one by Hyland. We also explain the role of the
Karoubi envelope -- a categorical construction -- in the proofs and the impact
the chosen foundation has on this construction. Finally, we report on some
automation we have implemented for the reduction of $\lambda$-terms.

</details>


### [90] [Computation by infinite descent made explicit](https://arxiv.org/abs/2506.22206)
*Sebastian Enqvist*

Main category: cs.LO

TL;DR: 介绍了一个非良基证明系统，用于扩展了归纳和协同归纳定义的直觉逻辑。该系统表明所有有效证明都是可计算的，并导出了一个范畴模型，将不动点公式与初始代数和最终余代数联系起来。


<details>
  <summary>Details</summary>
Motivation: 引入一个用于扩展了普通归纳和协同归纳定义的直觉逻辑的非良基证明系统，并探索该系统的计算内容。

Method: 该系统基于带有序数显式变量标注不动点公式的语法。引入了可计算性概念。从证明系统中推导出了一个范畴模型。

Result: 每个有效证明都是可计算的。获得了对所谓的有限公式证明的归一化结果。适当形式的序列的每个证明都代表自然数上的一个独特函数。最小和最大不动点公式分别对应于初始代数和最终余代数。

Conclusion: 该文成功引入了一个非良基证明系统，展示了其计算内容（可计算性和归一化），并建立了一个范畴模型，显示了不动点公式与初始代数和最终余代数的对应关系。

Abstract: We introduce a non-wellfounded proof system for intuitionistic logic extended
with ordinary inductive and co-inductive definitions, based on a syntax in
which fixpoint formulas are annotated with explicit variables for ordinals. We
explore the computational content of this system, in particular we introduce a
notion of computability and show that every valid proof is computable. As a
consequence, we obtain a normalization result for proofs of what we call
finitary formulas. A special case of this result is that every proof of a
sequent of the appropriate form represents a unique function on natural
numbers. Finally, we derive a categorical model from the proof system and show
that least and greatest fixpoint formulas correspond to initial algebras and
final coalgebras respectively.

</details>


### [91] [On the role of connectivity in Linear Logic proofs](https://arxiv.org/abs/2506.21678)
*Raffaele Di Donna,Lorenzo Tortora de Falco*

Main category: cs.LO

TL;DR: 通过引入一个几何条件，本文将线性逻辑证明结构中一个必要的性质（关于正确性图的非循环性和连通分量数量）转化为充分的正确性判据，适用于乘法指数线性逻辑（MELL）的某些片段。


<details>
  <summary>Details</summary>
Motivation: 研究一个扩展Danos-Regnier线性逻辑证明结构正确性准则的性质，旨在将一个必要但不充分的性质（无环正确性图，特定数量的连通分量）转变为充分条件，以便在MELL中从证明结构中恢复序列演算证明。

Method: 提出了一个关于无类型证明结构的几何条件。

Result: 该几何条件使得必要性质变为充分条件，从而分离出几个MELL片段，其中该性质确实是一个正确性判据。此外，还重新发现了一些已知结果。

Conclusion: 本文成功地定义了几何条件，使得一个先前在乘法指数线性逻辑（MELL）中必要但不充分的性质，成为某些MELL片段的正确性判据，从而能够从证明结构中恢复序列演算证明。

Abstract: We investigate a property that extends the Danos-Regnier correctness
criterion for linear logic proof-structures. The property applies to the
correctness graphs of a proof-structure: it states that any such graph is
acyclic and that the number of its connected components is exactly one more
than the number of nodes bottom or weakening. This is known to be necessary but
not sufficient in multiplicative exponential linear logic (MELL) to recover a
sequent calculus proof from a proof-structure. We present a geometric condition
on untyped proof-structures allowing us to turn this necessary property into a
sufficient one: we can thus isolate several fragments of MELL for which this
property is indeed a correctness criterion. We also recover as by-product some
known results.

</details>


### [92] [On the role of connectivity in Linear Logic proofs](https://arxiv.org/abs/2506.21678)
*Raffaele Di Donna,Lorenzo Tortora de Falco*

Main category: cs.LO

TL;DR: 通过引入几何条件，我们将一个在MELL中仅是必要的证明结构正确性性质，转化为在某些MELL片段中的充分判据。


<details>
  <summary>Details</summary>
Motivation: 本文研究了一个扩展 Danos-Regnier 线性逻辑证明结构正确性判据的性质。现有性质在 MELL 中是必要但不充分的，本文旨在将其转化为充分条件。

Method: 提出了一种关于无类型证明结构的几何条件。

Result: 成功地分离出MELL的几个片段，对于这些片段，所研究的性质确实是一个正确性判据。此外，还恢复了一些已知结果。

Conclusion: 结合几何条件，扩展的 Danos-Regnier 性质在MELL的特定片段中可作为充分的正确性判据。

Abstract: We investigate a property that extends the Danos-Regnier correctness
criterion for linear logic proof-structures. The property applies to the
correctness graphs of a proof-structure: it states that any such graph is
acyclic and that the number of its connected components is exactly one more
than the number of nodes bottom or weakening. This is known to be necessary but
not sufficient in multiplicative exponential linear logic (MELL) to recover a
sequent calculus proof from a proof-structure. We present a geometric condition
on untyped proof-structures allowing us to turn this necessary property into a
sufficient one: we can thus isolate several fragments of MELL for which this
property is indeed a correctness criterion. We also recover as by-product some
known results.

</details>


### [93] [Negated String Containment is Decidable (Technical Report)](https://arxiv.org/abs/2506.22061)
*Vojtěch Havlena,Michal Hečko,Lukáš Holík,Ondřej Lengál*

Main category: cs.LO

TL;DR: 该论文证明了“不包含”字符串谓词是可判定的，尤其是在字符串变量受正则语言约束时，这对程序分析很重要。


<details>
  <summary>Details</summary>
Motivation: 解决长期未决的“不包含”字符串谓词的可判定性问题，该问题在符号执行等字符串操作程序中具有实际意义。

Method: 他们证明了谓词 notContains(x1 ... xn, y1 ... ym) 是可判定的，其中 x1 ... xn 和 y1 ... ym 是受正则语言约束的字符串变量序列。

Result: 证明了在字符串变量受正则语言约束的情况下，“不包含”谓词是可判定的。

Conclusion: 基于他们的发现，结合无链词方程和正则成员约束的不包含谓词的可判定性也随之确定。

Abstract: We provide a positive answer to a long-standing open question of the
decidability of the not-contains string predicate. Not-contains is practically
relevant, for instance in symbolic execution of string manipulating programs.
Particularly, we show that the predicate notContains(x1 ... xn, y1 ... ym),
where x1 ... xn and y1 ... ym are sequences of string variables constrained by
regular languages, is decidable. Decidability of a not-contains predicate
combined with chain-free word equations and regular membership constraints
follows.

</details>
